...
- 管理员也可以在创建共享资源时一次性填写初始凭据，系统只保存加密摘要，不会把凭据自动转发到 Sub2。
...
- 当前能力默认只负责安全保存和管理凭据；无论凭据是在创建资源时同步保存，还是在资源详情中后续轮换，都需要管理员显式点击“应用到 Sub2”才会把 `openai_refresh_token` 转发给 Sub2API。
- 已保存凭据不会通过任何后台响应回显明文。
