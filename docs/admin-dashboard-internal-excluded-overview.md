...
## 2026-06-13 共享资源全量入口生产复查

- Commit：`36912f07b88763d7ac9ce40fede79d71eb48d1bb`
- Release marker：`deployed_at=20260613T041814Z`
- Systemd 服务：`zyz-api`、`zyz-admin`、`zyz-web`、`sub2api` 均为 `active`。
- 发布脚本内验证：
  - API typecheck：通过。
  - Admin typecheck：通过。
  ...
  - `/health`、`/ready`、Web、Admin 探针均返回 200。
- 线上共享资源列表复查：
  - `GET /api/admin/resources`：默认经营口径 `total=0`。
  - `GET /api/admin/resources?action=all`：全量口径 `total=1`。
  - 全量口径中的内部资源 `sub2AccountId=admin-disabled-smoke-resource`，默认经营口径已排除。
  - Dashboard 同步显示 `managementOverview.sharing.total=0`、`managementOverview.internalExcluded.sharing=1`。
- Admin 前端产物复查：
  - `http://127.0.0.1:3101/` 返回 200。
  - 当前 Admin JS 产物包含“经营口径”和“包含内部巡检”两个筛选文案。
- 实时系统健康仍为 `24 ok / 2 warning / 3 error`，剩余阻断仍是有效 OpenAI refresh token、active Sub2 OpenAI 账号和 online Codex 共享资源缺失。
