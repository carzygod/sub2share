...
- API CORS 配置新增 `exposedHeaders: ["x-proxy-request-id"]`。
- 浏览器端 JavaScript 调用本地 `/v1/*` OpenAI/Codex 反代时，可以读取响应头中的 `x-proxy-request-id`。
- `x-proxy-request-id` 暴露头配置集中在 `user/apps/api/src/modules/openai-proxy/helpers.ts`，与代理响应头常量保持一致。
...
- 测试验证响应包含 `access-control-expose-headers: x-proxy-request-id`，并能同时返回本地 `x-proxy-request-id`。
- 更新独立说明文档 `docs/openai-proxy-request-correlation.md` 与 `docs/openai-proxy-public-endpoint.md`。

当前结论：

- `x-proxy-request-id` 的浏览器可读性不再只依赖静态常量检查，已经由真实 Fastify CORS 响应验证覆盖。
...
- `/v1/*` OpenAI/Codex 本地代理将 RPM/TPM 窗口处理拆为“检查”和“提交”两个阶段。
- `checkRentalRateLimits()` 只评估请求是否会超过租赁 RPM/TPM，不立即写入窗口。
- 只有 `acquireProxyConcurrency()` 成功取得租赁级并发租约后，才调用 `rateLimitCheck.record()` 记录本次 RPM/TPM 占用。
- 并发失败的请求仍返回 `429 concurrency_limit_exceeded` 并写入 `ProxyRequestLog`，但不再污染 RPM/TPM 额度。
- 新增 `evaluateProxyRateLimitWindow()` helper，把窗口过期清理、限额判断和延迟提交变成可测试逻辑。
- 自动化测试覆盖“检查不修改窗口，commit 后才计入 RPM/TPM”的核心行为。
...
- 该能力继续补强 OpenAI/Codex 反代可用性和售出套餐限额的一致性；多实例部署场景仍需将进程内窗口迁移到共享限流器。
...
- 多实例部署场景仍需要 Redis 或网关级共享限流器，才能让 RPM/TPM 在所有 API 实例之间一致。
...
- 生产环境下如果限流器仍是进程内作用域，巡检标记 warning，提示保持单实例部署或迁移到 Redis/网关级共享限流器。
...
- 新增测试覆盖生产环境进程内限流器的 warning 判定。

当前结论：

- 管理员可以在 `可用性巡检` 中直接看到当前 API 进程的 OpenAI/Codex 反代并发和 RPM/TPM 运行态。
- 该能力把历史反代请求日志和实时限流状态接入同一个管理入口，继续补强系统性可用性复查。
- 本次变更只暴露和提示当前进程内状态，不改变请求拦截行为；完整多实例一致限流仍需要后续把并发租约和速率窗口迁移到共享存储或网关级限流器。
