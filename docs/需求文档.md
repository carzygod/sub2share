...
### 18.170 反代请求日志支持 multipart 模型提取

#### 背景

本地 OpenAI/Codex `/v1/*` 反代已经用原始 Buffer 接收请求体，避免 JSON、multipart、二进制和大请求体被业务 parser 改写或拒绝。此前 `ProxyRequestLog.model` 只从 JSON 顶层 `model` 字段提取模型名；当客户端使用 multipart/form-data 发送 Responses/Codex 上传类或混合输入类请求时，请求可以被原样转发，但管理员在 `反代请求` 列表、订单/租赁详情和 CSV 中看不到模型名，按模型聚合错误率或搜索排障会缺一块。

#### 新增能力

- `proxyRequestModel()` 在 JSON 顶层提取之外，新增 multipart/form-data `model` part 识别。
- 支持 `Content-Disposition` 中 `name="model"` 和 `name=model` 两种形式。
- multipart part 可包含 `Content-Type` 等标准 part header，提取时只读取该 part 的字段值。
- 模型名继续统一 trim 并截断到 160 字符。
- 缺少 `model` part、无效 JSON、非对象 JSON、空模型名或其他字段中的模型文本仍记录为空，避免误把 prompt/input 内容当成模型。
- `openAiProxyForwardingContract` 与系统巡检 summary 新增 `extractsMultipartModelForLogs=true`。
- API 单元测试新增 multipart 模型提取与非 model part 不误判覆盖。

#### 价值

- 管理员可以在 `反代请求` 页面按模型筛选 multipart 上传类请求，继续追踪用户、租赁、API Key、订单和用量上下文。
- Codex/Responses 代理仍保持原始请求体透传，不保存 prompt、input、文件内容或响应体，只保存模型名这一条低敏排障线索。
- 该能力补强完整 OpenAI/Codex 反代的可观测性，不改变计费、余额、售出订单或 Sub2API 上游请求内容。

### 18.171 管理员首页共享资源巡检入口优先

#### 背景

当前线上可用性阻断同时暴露 `sub2`、`resourceCredentials`、`localProxySmoke` 和 `resources` 问题。其中 `resources` 的直接含义是没有可用的生产 Codex 共享资源，但该 issue 也会携带 `repairAction=apply_openai_refresh_token_to_sub2_account` 与 `sub2AccountId`，此前首页按钮会优先进入 `反代状态`，管理员补共享资源时需要再手动切换到 `共享资源` 页面。

#### 新增能力

- Admin 首页 `resources` 关键巡检项优先打开 `共享资源`。
- 点击 `resources` 巡检项时继续读取 `primaryIssue` / `primarySample` 中的 `supplierEmail`、`resourceType`、`resourceStatus`、`resourceScope` 和 `sub2AccountId`。
- 共享资源列表会带入上述筛选条件。
- 创建共享资源表单会复用供应方邮箱、资源类型和 Sub2 账号默认值，便于管理员直接补齐生产 Codex 资源。
- `sub2`、`resourceCredentials` 和 `localProxySmoke` 仍保持反代维修优先，继续打开 `反代状态` 并预填目标 Sub2/OpenAI 账号。
- 新增独立文档 `docs/admin-dashboard-resource-health-routing.md`，并更新 `docs/admin-dashboard-health-snapshot.md`。

#### 价值

- “共享资源缺失”和“Sub2/OpenAI 账号失效”在首页进入更贴近问题本身的管理入口。
- 管理员可以从 `No online production Codex shared resource` warning 直接进入共享资源创建/筛选路径，补齐用户要求的共享情况管理闭环。
- 该能力只改变 Admin 导航与默认筛选，不自动写入 refresh token、不修改 Sub2 账号、不触发真实 OpenAI/Codex smoke test。

### 18.172 反代请求日志补齐非 JSON 模型来源

#### 背景

18.170 已经让 JSON 顶层 `model` 和 multipart `model` part 进入 `ProxyRequestLog.model`。但 OpenAI 兼容生态里仍可能出现 `application/x-www-form-urlencoded` 请求、带 `model` query 的调试请求，或者 `/v1/models/:model` 模型元数据请求。此前这些请求可以被本地 `/v1/*` 反代原样转发，但管理员在反代请求日志中仍看不到模型名，排查模型元数据、兼容客户端或轻量表单请求时会缺少一条低敏线索。

#### 新增能力

- `proxyRequestModel()` 新增可选 `url` 和 `contentType` 上下文。
- 明确 `Content-Type=application/x-www-form-urlencoded` 时，从表单字段 `model` 提取模型名。
- URL query 携带 `model` 时，从 query 参数提取模型名。
- 路径为 `/v1/models/:model` 时，从路径段提取模型名。
- `text/plain` 等非表单请求体不会被当作 form-urlencoded 解析，避免误把 prompt 文本中的 `model=` 当成模型名。
- `openAiProxyContract.summary` 新增：
  - `extractsFormUrlEncodedModelForLogs=true`
  - `extractsUrlModelForLogs=true`
- API 单元测试覆盖 form-urlencoded、query、路径模型提取和非表单误判保护。

#### 价值

- 管理员在 `反代请求` 页面可以更完整地按模型检索 JSON、multipart、form-urlencoded、query 和模型元数据路径请求。
- 模型日志继续保持低敏边界：只保存模型名，不保存请求体、响应体、prompt、messages、input、文件内容或工具参数。
- 该能力继续补强完整 OpenAI/Codex 反代的可观测性，不改变计费、余额、售出订单、Sub2API 上游请求内容或真实生成行为。

### 18.173 共享资源修复创建默认执行 Sub2 应用与自检

#### 背景

当前线上 `resources` warning 代表没有可用的生产 Codex 共享资源，并且巡检问题已经能携带供应方、资源类型、Sub2 账号和 `repairAction=apply_openai_refresh_token_to_sub2_account`。管理员从首页进入共享资源页后，需要粘贴有效 OpenAI refresh token 并创建资源。此前表单虽然能预填供给方和 Sub2 账号，但 `创建后应用到 Sub2` 与 `应用后端到端自检` 默认未勾选，容易出现“资源已创建但凭据未应用”或“凭据已应用但 `/v1/responses` 未验收”的空窗。

#### 新增能力

- `ResourceCreateDefaults` 扩展保存资源巡检上下文：
  - `repairAction`
  - `checkId`
  - `resourceScope`
  - `resourceStatus`
  ...
  - `proxyRequestStatusCode`
  - `proxyRequestErrorCode`
- Admin 首页 `resources` 巡检项打开共享资源页时，会把上述字段传入创建表单默认值。
...
- 如果巡检上下文携带 smoke 模型，自动填入 `自检模型`；没有模型时保持为空，由后端使用默认 smoke 模型。
- Admin 单元测试锁定资源创建默认值会延续 OpenAI credential 修复流程。

#### 价值

- 管理员从 `No online production Codex shared resource` warning 进入共享资源页后，只需粘贴有效 refresh token 并提交，即可按默认路径完成“保存凭据 -> 应用到 Sub2 -> 账号测试 -> 端到端 smoke”。
- 修复路径更贴近完整 OpenAI/Codex 反代恢复要求，不再停留在资源登记或账号刷新层面。
- 该能力只调整表单默认值，不会在没有管理员提交时自动写入 refresh token、修改 Sub2 账号或触发 smoke test。

### 18.174 OpenAI/Codex 反代精确 `/v1` 基路径透传

#### 背景

本地 OpenAI/Codex 反代已经覆盖 `/v1/*` 下所有具体子路径，例如 `/v1/models`、`/v1/responses`、`/v1/responses/:id/input_items` 和 `/v1/chat/completions`。但公开 endpoint 本身配置为 `https://api.example.com/v1`，部分客户端、探针或人工验收会直接访问精确 `/v1` 或 `/v1/`。此前这些基路径不会进入本地反代 catch-all，容易被 API 服务自己的 404 提前截断，缺少本地 `x-proxy-request-id`、反代日志和 Sub2API 上游响应证据。

#### 新增能力

- 新增 `openAiProxyRouteBasePath=/v1` 和 `openAiProxyRoutePaths=/v1,/v1/*`。
- `registerOpenAiProxyRoutes()` 对 `/v1` 与 `/v1/*` 注册同一套 handler。
- 精确 `/v1` 请求会复用现有本地售出 Key 鉴权、余额准入、租赁状态检查、并发/RPM/TPM 限流、请求日志、上游 request id 捕获和 Sub2API 透传。
- `isOpenAiProxyRoutedPath()` 现在识别 `/v1` 与 `/v1/`，但不会误把 `/v10/responses` 识别为 OpenAI v1 路径。
- `openAiProxyContract.summary` 新增：
  - `routePaths=/v1,/v1/*`
  - `supportsV1BasePath=true`
  - `routesV1BasePath=true`
- API 单元测试锁定 `/v1`、`/v1/`、`/v1/*`、`/v10/responses` 和 `/api/admin/*` 的路由判定边界。

#### 价值

- 公开 endpoint 的基路径本身也成为完整 OpenAI/Codex 反代的一部分，不再只覆盖子路径。
- 管理员或客户端直接探测 `endpointUrl` 时，可以拿到本地代理请求 ID 和可审计反代日志，便于区分“本地入口未接住”和“Sub2API/OpenAI 上游响应”的差异。
- 该能力不改变余额、售出订单、租赁限额、用量同步或 Sub2API 上游请求内容，只补齐本地反代入口覆盖面。

### 18.175 Smoke 失败证据过期提示

#### 背景

`localProxySmoke` 已经能把最近一次 `/v1/models` 与 `/v1/responses` 端到端自检证据纳入系统巡检，并在失败时携带请求 ID、代理日志、HTTP 状态、模型和修复账号。但当最新失败证据已经超过 freshness 窗口时，管理员只能看到失败本身和 `ageMinutes`，不容易判断这条证据是否还代表当前状态，修复后也容易忘记重新跑端到端自检刷新 live 证据。

#### 新增能力

- `LocalProxySmokeEvidenceIssue` 新增 `stale` 字段。
- 最新 smoke 失败且证据超过 freshness 窗口时：
  - 巡检状态仍为 `error`。
  - `message` 会说明该失败证据已经过期，可能不再代表当前 `/v1/responses` 可用性。
  - `actionHint` 会要求修复后重新运行本地端到端自检刷新证据。
- 最新 smoke 通过但证据过期时，原有 `local_proxy_smoke_stale` warning 也会携带 `stale=true`。
- `GET /api/admin/dashboard` 的关键巡检预览保留 `stale`。
- Admin 完整巡检页的对象摘要展示 `stale`。
- Admin `反代状态 -> 修复定位` 在失败请求摘要中显示“证据已过期”。

#### 价值

- 管理员看到历史 `/v1/responses` 失败时，可以区分“当前刚失败”和“旧失败证据仍未刷新”。
- 应用有效 OpenAI refresh token 后，管理员更容易意识到还需要重新运行端到端 smoke，避免凭据已修但可用性证据仍停留在旧失败。
- 该能力只增强巡检证据表达和管理员入口上下文，不自动触发真实 OpenAI/Codex 请求，也不修改 Sub2/OpenAI 账号或资源凭据。

### 18.176 巡检候选样本管理入口补齐

#### 背景

`可用性巡检` 页面已经把 `detail.issues` 问题样本做成了可操作 drilldown，可以直接进入用户、共享、余额、售出、反代请求等管理入口。但部分健康检查会把可修复对象放在 `detail.samples` 中，例如 Sub2/OpenAI 候选账号、资源凭据候选、支付充值流水或 smoke 相关证据。此前候选样本的操作入口较少，管理员仍可能需要复制 ID 再去对应页面检索。

#### 新增能力

- `SystemHealthSampleRow` 扩展支持请求证据、售出对象、钱包对象、共享资源、Sub2 账号和治理对象的 drilldown 字段。
- 候选样本可以直接打开：
  - 反代请求
  - 共享资源列表或资源详情
  - Sub2/OpenAI 反代状态
  ...
  - 提现
  - 审计日志
- 候选样本支持 `refType/refId` 映射到 `order`、`usage`、`wallet_transaction`、`settlement` 和 `withdrawal`。
- 候选样本支持字符串布尔值，例如 `resourceList="true"`、`walletTransactionList="true"` 和 `salesList="true"`。
- 候选样本打开 Sub2/OpenAI 反代状态时，会携带 `actionHint`、请求 ID、代理日志 ID、上游 request id、失败路径、状态码、模型、smoke 分段结果、`ageMinutes` 和 `stale`。
- 共享摘要字段增加候选样本常用的请求、订单、租赁、Key、用量、商品、钱包流水、结算、提现、审计和资源定位字段。

#### 价值

- 管理员在巡检页看到候选样本时，也能像处理问题样本一样直接跳到对应管理入口。
- “完整管理员入口”从单纯列表覆盖进一步扩展到巡检驱动的运营闭环，减少跨页面复制 ID。
- 该能力只增强管理后台前端和摘要字段，不改变健康判定、不写业务数据，也不触发额外 Sub2API 或 OpenAI/Codex 请求。

### 18.177 管理后台能力矩阵可视化入口

#### 背景

后端已经通过 `GET /api/admin/capabilities` 和 `adminCapabilities` 系统巡检项证明管理员 API 覆盖了用户、共享资源、余额、售出和 OpenAI/Codex 反代核心范围。但管理员此前只能在系统健康摘要中看到覆盖结果，无法在后台直接浏览每个范围、每个操作、允许角色和缺失路由问题。

#### 新增能力

- 共享导航新增 `capabilities` / `入口能力`。
- Admin 前端新增 `入口能力` 页面。
- 页面读取 `GET /api/admin/capabilities`，展示：
  - 核心范围覆盖数。
  - 声明操作总数。
  - 关键操作总数。
  - 已注册操作总数。
  - 缺失路由数。
  - 每个能力范围的操作 ID、名称、HTTP 方法、路径、角色和关键级别。
  - 覆盖问题的范围、操作、路由、说明和修复建议。
- Admin 表面测试新增 `capabilities` 入口断言，防止侧边栏误删。
- 新增独立说明文档 `docs/admin-capability-matrix-view.md`。

#### 价值

- 管理员可以在后台直接确认用户、共享、余额、售出和 OpenAI/Codex 反代管理能力是否完整，而不必调用接口或只看健康摘要。
- 能力矩阵页面补齐“完整管理员入口”的治理视角，让上线验收、交接和故障排查更直观。
- 该能力只读取现有能力矩阵，不写业务数据，不触发 Sub2API 或 OpenAI/Codex 请求。

### 18.178 管理后台能力矩阵操作直达

#### 背景

18.177 已经让管理员能在后台浏览 `GET /api/admin/capabilities` 返回的能力矩阵，确认用户、共享、余额、售出和 OpenAI/Codex 反代核心范围的 API 路由覆盖。但页面仍偏“审计清单”：管理员看到某个能力项后，还需要自己判断应该切到哪个管理页面继续处理。对于完整管理员入口而言，能力矩阵应该既能证明覆盖，也能把管理员带到对应业务入口。

#### 新增能力

- 新增 `admin-capability-navigation.ts`，集中维护能力操作 ID 到 Admin 视图的映射。
- `入口能力` 表格新增“入口”列。
- 每个已知能力操作都提供只读跳转按钮：
  - 用户能力 -> `用户管理`。
  - 共享资源/供给方能力 -> `共享资源` 或 `供给方`。
  - 余额能力 -> `余额管理`、`余额流水` 或 `账务对账`。
  - 售出能力 -> `售出情况`、`订单管理`、`租赁通道`、`API Key`、`用量记录` 或 `商品配置`。
  - OpenAI/Codex 反代能力 -> `反代状态` 或 `反代请求`。
  - 治理能力 -> `总览`、`可用性巡检`、`巡检历史`、`入口能力` 或 `审计日志`。
- 点击入口按钮只打开对应管理页面，不直接执行能力项背后的写操作。
- Admin 单元测试锁定关键能力项的目标入口，覆盖用户、共享、余额、售出、用量同步、反代请求、Sub2 凭据应用、系统巡检和审计日志。

#### 价值

- 能力矩阵从“能看见后端覆盖”升级为“能进入实际管理入口”，管理员处理用户、共享、余额、售出和反代问题时少一次人工判断。
- `sub2.applyOpenAiRefreshToken`、`sub2.proxySmokeTest` 等 OpenAI/Codex 反代能力可直接进入 `反代状态`，与当前 `/v1/responses` 修复闭环保持一致。
- 该能力只改变 Admin 前端跳转，不写业务数据，不修改 Sub2API/OpenAI 上游，也不触发真实 smoke test。

### 18.179 管理后台能力入口目标后端契约化

#### 背景

18.178 让 `入口能力` 页面具备了能力操作直达入口，但入口映射最初维护在 Admin 前端。这样虽然能改善操作体验，却会让后端能力矩阵与前端入口目标形成两份声明：新增、重命名或删除能力项时，后端 `GET /api/admin/capabilities` 可以仍然通过路由覆盖检查，但前端跳转目标可能遗漏。为了让“完整管理员入口”具备机器可验证的契约，需要把入口目标下沉到后端能力矩阵，并纳入系统巡检。

#### 新增能力

- `AdminCapabilityOperation` 新增 `target` 字段：
  - `target.view`：对应 Admin 视图。
  - `target.label`：按钮文案。
- `GET /api/admin/capabilities` 直接返回每个能力操作的入口目标。
- `inspectAdminCapabilityRouteCoverage()` 新增入口目标覆盖检查：
  - `operationsWithTargets`
  - `missingTargets`
  - `operation_target_missing`
- `adminCapabilities` 系统巡检项会在缺少入口目标时返回 error。
- Admin 前端 `入口能力` 页面改为消费 API 返回的 `operation.target`，不再维护前端本地映射表。
- 能力矩阵页面新增 `可达入口` 指标，显示 `operationsWithTargets/totalOperations`。
- API 单元测试锁定 65 个能力操作全部存在入口目标，并覆盖用户、共享、余额、售出、用量同步、反代请求、Sub2 凭据应用、系统巡检和审计日志等关键目标。

#### 价值

- 管理员入口覆盖从“前端可点击”进一步升级为“后端契约可证明、系统巡检可发现漂移”。
- 后续新增 OpenAI/Codex 反代、售出、余额或共享资源能力时，如果没有同步提供管理入口，`adminCapabilities` 会直接暴露缺口。
- 该能力不执行任何写操作，不触发 Sub2API/OpenAI 请求，只增强能力矩阵契约、前端消费和系统可用性复查证据。

### 18.180 生产 mock 充值安全闸

#### 背景

支付巡检已经能识别生产环境 `PAYMENT_PROVIDER=mock` 并把余额、充值流水和售出情况入口串起来，但此前接口层仍会在生产环境写入 mock 充值流水。对于“管理员管理余额情况、售出情况”的目标而言，后台能提示风险还不够；生产默认行为应避免继续产生伪充值入账，除非运维显式授权内部验收。

#### 新增能力

- 后端环境变量新增 `ALLOW_PRODUCTION_MOCK_RECHARGE`，默认 `false`。
- 新增充值入口策略 helper：
  - 非生产环境 `PAYMENT_PROVIDER=mock` 仍允许 mock 充值。
  - 生产环境 `PAYMENT_PROVIDER=mock` 默认阻断 mock 充值。
  - 生产环境只有在 `ALLOW_PRODUCTION_MOCK_RECHARGE=true` 时才允许 mock 充值。
  - `PAYMENT_PROVIDER=disabled` 始终禁用充值入口。
- `POST /api/wallet/recharge` 改为使用该策略；生产默认阻断时返回 `503 recharge_unavailable`，不写入钱包余额和充值流水。
...
- 支付巡检语义调整：
  - 生产 mock 充值被默认阻断且没有近期充值流水时，`payments.status=ok`。
  - 生产显式允许 mock 充值时，`payments.status=warning`，issue type 为 `production_mock_recharge`。
  - 生产已阻断 mock 充值但仍发现近期充值流水时，`payments.status=warning`，issue type 为 `production_mock_recharge_recent_ledger`。
- `user/.env.example` 增加 `ALLOW_PRODUCTION_MOCK_RECHARGE=false`。
- API 测试新增生产默认阻断、显式允许、近期流水影响和 disabled provider 覆盖。

#### 价值

- 线上余额和售出收入不再因为默认 mock 充值入口开放而继续生成伪收款流水。
- 管理员在系统健康页不仅能看到支付配置模式，还能看到充值入口是否真的开放、是否被生产安全闸阻断，以及是否仍有近期流水需要复核。
- 该能力不替代真实支付渠道；真实公开收费仍需实现支付单、渠道回调、验签、幂等入账和退款对账。

### 18.181 共享资源创建展示巡检修复上下文

#### 背景

当前线上剩余阻断集中在 `resources`、`resourceCredentials`、`sub2` 和 `localProxySmoke`。管理员从系统健康或首页点击共享资源缺失问题时，后台已经可以带入供给方邮箱、资源类型、生产范围、Sub2 账号、推荐修复动作和失败 smoke 证据，并让共享资源创建表单默认勾选“创建后应用到 Sub2”和“应用后端到端自检”。但这些默认值此前只体现在表单字段和勾选状态里，管理员不容易在提交前确认它们来自哪条巡检问题、为什么会默认执行凭据应用和 smoke。

#### 新增能力

- Admin `共享资源` 创建表单在存在巡检修复上下文时展示只读诊断条。
- 诊断条由 `resourceCreateDefaultsContextItems()` 生成，展示：
  - 来源检查项与生产资源范围。
  - 推荐修复动作。
  - 供给方邮箱。
  - 资源类型与资源范围。
  ...
  - 创建后是否应用初始凭据到 Sub2。
  - 应用后是否运行端到端 smoke。
  - 失败请求路径、HTTP 状态和本地/上游错误码。
- Admin 单元测试新增 `resource create defaults expose repair context for operators`，锁定共享资源创建上下文摘要。
- 该能力只读展示上下文，不改变后端创建资源、保存凭据、应用到 Sub2 或 smoke test 的执行条件。

#### 价值

- 管理员从 `resources` warning 创建生产 Codex 资源时，可以在提交前确认表单默认值对应的 Sub2 账号和维修动作。
- 从 `resourceCredentials`、`sub2` 或 `localProxySmoke` 派生出的 OpenAI/Codex 反代修复上下文在共享资源创建表单中保持可见，减少误操作。
- 这进一步补齐“完整管理员入口”的可解释性：入口不仅能跳到页面，还能把巡检证据带进具体操作表单。

### 18.182 共享资源巡检区分 online 与 ready online

#### 背景

`resources` 巡检此前用 `status=online` 判断生产 Codex 共享资源是否存在。这个口径可以发现“完全没有线上 Codex 资源”的问题，但也存在误判空间：管理员如果手工把一个 Codex 资源切到 `online`，但没有绑定 Sub2 账号或没有 active OpenAI refresh token 凭据，资源仍无法支撑 OpenAI/Codex 反代交付，却可能让共享资源检查看起来正常。为了让系统可用性复查更贴近真实 `/v1/responses` 可交付性，需要把“在线状态”和“可交付在线资源”拆开。

#### 新增能力

- `GET /api/admin/system-health` 的 `resources.metrics` 新增：
  - `readyOnlineCodexResources`
  - `incompleteOnlineCodexResources`
- `readyOnlineCodexResources` 必须同时满足：
  - 生产 Codex 资源。
  - `status=online`。
  - 已绑定 `sub2AccountId`。
  - 拥有 `status=active` 且 `credentialType=openai_refresh_token` 的资源凭据。
- 如果没有 ready online Codex 资源，`resources` 继续保持 `warning`。
- 新增或细化 issue：
  - `codex_online_resource_missing`：没有任何 online 生产 Codex 资源。
  - `codex_ready_resource_missing`：有 online 生产 Codex 资源，但没有任何资源同时具备 Sub2 账号和 active OpenAI refresh token 凭据。
  - `codex_online_resource_incomplete`：已有 ready 资源，但仍存在不完整 online Codex 资源，需要管理员修复。
- `resources.detail.samples` 对资源样本新增 `credentialType` 与 `credentialStatus`，便于管理员直接判断资源缺少凭据、凭据状态错误，还是缺少 Sub2 绑定。
- API 单元测试补充 ready/incomplete 指标语义。

#### 价值

- 管理员不能仅靠把资源状态切成 `online` 就让共享资源巡检误报正常。
- 生产 Codex 资源的健康口径更贴近真实 OpenAI/Codex 反代交付链路：资源状态、Sub2 账号绑定和 active refresh token 凭据必须同时存在。
- 剩余真实阻断仍由 `sub2`、`resourceCredentials`、`localProxySmoke` 和真实反代请求证据共同证明，不会因为一个状态字段而被掩盖。

### 18.183 共享资源手动上线保护

#### 背景

18.182 已经让系统巡检把 `online` 和 ready online 拆开，避免资源只靠状态字段误报可交付。但如果管理员仍可以在共享资源状态接口里把一个不完整的 Codex 资源手工切成 `online`，系统就会留下一个“看起来在线、实际不可用”的中间状态，后续排障仍需要靠巡检再发现。为了把问题前移到写入口，需要在状态变更时直接校验资源是否具备真实 OpenAI/Codex 交付前提。

#### 新增能力

- `PATCH /api/admin/resources/:id/status` 在把 Codex 资源切换为 `online` 前，会读取资源类型、当前 Sub2 账号绑定和资源凭据摘要。
- Codex 资源手动上线必须同时满足：
  - 已存在或本次请求提供 `sub2AccountId`。
  - 已绑定 `credentialType=openai_refresh_token` 且 `status=active` 的资源凭据。
- 不满足条件时接口返回 `400 codex_resource_not_ready_for_online`，并在 `issues` 中列出：
  - `sub2_account_missing`
  - `active_openai_refresh_token_missing`
- 非 Codex 资源、以及 Codex 资源切到 `paused`、`disabled`、`abnormal` 等非 `online` 状态时不受该闸门影响。
- 状态变更审计继续记录资源状态、等级、Sub2 账号，并补充资源凭据摘要，便于追踪资源上线时的交付前提。
- API 单元测试覆盖缺少 Sub2/凭据时拦截、具备 active OpenAI refresh token 时放行，以及非 Codex/非上线动作不受影响。

#### 价值

- 管理员不能绕过资源凭据和 Sub2 绑定要求，把不可交付的 Codex 资源直接标为 `online`。
- `resources` 巡检的 ready online 口径与管理员状态写入口保持一致，减少“先写入错误状态、再由巡检发现”的运维往返。
- 该能力不替代真实 Sub2 账号测试或 `/v1/responses` 端到端 smoke；最终真实可用性仍必须由有效 OpenAI refresh token、active Sub2 OpenAI 账号和本地反代自检共同证明。

### 18.184 共享资源测试自动上线保护

#### 背景

18.183 已经拦截了管理员手动把不完整 Codex 资源切为 `online` 的路径，但 `POST /api/admin/resources/:id/test` 仍会在 Sub2 账号测试通过后，根据旧状态收敛规则自动把 `pending`、`testing` 或 `abnormal` 资源切为 `online`。这会形成另一条绕过 ready online 口径的路径：Sub2 账号测试通过并不等同于平台共享资源已经保存 active OpenAI refresh token 凭据，也不等同于本地 OpenAI/Codex 反代已经具备可交付证据。

#### 新增能力

- 新增共享资源测试状态转移 helper：
  - 先按既有测试规则计算目标状态。
  - 当目标状态为 `online` 且资源类型为 Codex 时，再校验 Sub2 账号绑定和 active OpenAI refresh token 凭据。
  - 不满足 ready 条件时保持原状态，并返回 `blockedOnline=true` 与 `onlineReadiness.issues`。
- `POST /api/admin/resources/:id/test` 使用该 helper，测试通过但资源不 ready 时不会自动上线。
- `POST /api/admin/resources/:id/apply-credential-to-sub2` 使用同一 helper；已存凭据应用、账号测试通过后，只有资源自身具备 active OpenAI refresh token 凭据时才自动上线。
- 从 `反代状态` 直接保存 refresh token 到资源的路径也使用同一 helper。该路径会在同一次更新中写入 active openai_refresh_token，因此在账号测试通过时可以安全收敛为 `online`。
- 资源测试和凭据应用审计载荷新增 `statusTransition`，记录：
  - 原计划目标状态。
  - 实际应用状态。
  - 是否阻止自动上线。
  - online readiness 问题列表。
- API 单元测试覆盖不完整 Codex 资源测试通过不自动上线、ready Codex 测试通过自动上线，以及测试失败仍收敛为 `abnormal`。

#### 价值

- Codex 共享资源无法再通过“测试通过自动上线”绕过 Sub2 账号与 active refresh token 凭据要求。
- 手动状态变更、资源测试、凭据应用和系统巡检共享同一套 ready online 语义，管理员看到的资源状态更接近真实 OpenAI/Codex 交付能力。
- 该能力仍不伪造真实上游成功；最终恢复 `/v1/responses` 仍必须提供有效 OpenAI refresh token / active Sub2 OpenAI 账号，并重新运行本地端到端 smoke。

### 18.185 共享资源凭据失效状态收敛

#### 背景

18.183 和 18.184 已经覆盖了“资源进入 online”的路径，但还需要覆盖“资源失去 ready 条件”的路径。管理员删除资源凭据、把凭据状态改为 `disabled/rotated`，或把凭据类型改成非 `openai_refresh_token` 时，一个原本 `online` 的 Codex 资源会立刻失去 OpenAI/Codex 反代交付前提。如果资源状态继续停留在 `online` 或 `busy`，管理员和调度侧仍可能误判它可用。

#### 新增能力

- 新增凭据变更状态收敛 helper：
  - 当资源类型为 Codex，且当前状态为 `online` 或 `busy` 时，重新检查 ready 条件。
  - 如果变更后的凭据不再是 active `openai_refresh_token`，或资源缺少 `sub2AccountId`，资源状态自动收敛为 `abnormal`。
  - 非 Codex 资源、以及非在线交付状态的资源保持原状态。
- `PUT /api/admin/resources/:id/credential` 在保存新凭据后执行该状态收敛。
- `DELETE /api/admin/resources/:id/credential` 在删除凭据后执行该状态收敛。
- 状态被收敛时同步清空 `lastCheckedAt`，避免保留旧测试时间误导管理员。
- 凭据创建/更新/删除审计载荷新增：
  - 变更前后的资源状态。
  - `statusTransition.changed`
  - `statusTransition.reason=codex_resource_not_ready_after_readiness_change`
  - `onlineReadiness.issues`
- API 单元测试覆盖在线 Codex 凭据禁用/删除后降级、busy Codex 降级、ready Codex 和非 Codex 资源不受影响。

#### 价值

- Codex 资源现在不仅进入 `online` 需要 ready，失去 ready 条件时也会退出在线交付状态。
- 管理员维护凭据不会留下“状态在线、凭据不可用”的脏资源，资源池与系统巡检更一致。
- 该能力不会删除资源或凭据历史，只修正资源可交付状态；真实恢复仍需要保存有效 OpenAI refresh token、应用到 Sub2、测试账号并运行 `/v1/responses` smoke。

### 18.186 共享资源创建与配置变更 ready 保护

#### 背景

18.183 到 18.185 已经覆盖了资源状态入口、资源测试入口和凭据入口，但管理员仍可能通过两个通用入口留下不 ready 的在线资源：创建资源时直接传入 `status=online`，或在资源配置页清空/修改 `sub2AccountId`，让一个原本在线的 Codex 资源失去 Sub2 绑定。为了让完整管理员入口的所有写路径都对齐 ready online 语义，需要把资源创建和配置修改也纳入同一套校验。

#### 新增能力

- `POST /api/admin/resources` 创建 Codex 资源时，如果请求初始 `status=online`，必须同时提供：
  - `sub2AccountId`
  - 初始 active `openai_refresh_token` 凭据
- 创建请求不满足 ready 条件时返回 `400 codex_resource_not_ready_for_online`，不会写入一个不可交付的 online 资源。
- 创建请求若使用 `busy` 等在线交付状态但不满足 ready 条件，会通过 readiness 状态转移收敛为 `abnormal`，并在审计中记录 `statusTransition`。
- `PATCH /api/admin/resources/:id` 更新资源配置时：
  - 显式把 Codex 资源改为 `online` 仍会执行 ready 校验。
  - 如果修改 `sub2AccountId` 或其他配置后导致在线/忙碌 Codex 资源不再 ready，资源状态自动收敛为 `abnormal`。
  - 状态收敛时清空 `lastCheckedAt`。
- `admin.resource.create` 与 `admin.resource.update` 审计载荷记录 `statusTransition`，便于管理员复盘资源为什么没有保持在线。
- API 单元测试补充 Sub2 绑定移除导致在线 Codex 降级，以及创建 online Codex 缺少初始凭据时的 ready 拒绝语义。

#### 价值

- 共享资源创建、配置维护、手动状态变更、资源测试、凭据维护和系统巡检全部对齐到同一套 Codex ready 条件。
- 管理员不能通过通用资源编辑入口绕过 Sub2 账号绑定和 active OpenAI refresh token 凭据要求。
- 资源池的 online/busy 状态更可信；真实 `/v1/responses` 恢复仍以有效 OpenAI refresh token、Sub2 active 账号、账号测试和本地反代 smoke 为最终证据。

### 18.187 Codex 售出交付 ready 资源准入

#### 背景

18.182 到 18.186 已经把生产 Codex 共享资源的 ready 条件纳入巡检、上线、测试、凭据维护、创建和配置修改入口。但售出交付链路仍需要在创建 Sub2 key 前确认资源池是否真的存在 ready Codex 资源，否则可能出现“订单和租赁已售出，但没有可交付生产资源”的偏差。

#### 新增能力

- 新增 `resource-delivery-readiness` 通用 helper，供非 admin 业务路径复用。
- Codex 交付前必须存在生产 ready online 共享资源：
  - `resourceType=codex`
  - `status=online`
  - `sub2AccountId` 非空
  - 排除内部巡检资源 `admin-disabled-smoke-resource`
  - 资源凭据为 active `openai_refresh_token`
- `POST /api/orders` 在钱包扣款和订单/租赁写入前执行该准入。
- `POST /api/admin/orders/:id/retry-provision` 在订单状态切换、钱包重新扣款和 Sub2 key 创建前执行该准入。
- 租赁 API Key rotation 在创建新 Sub2 key 前执行该准入。
- 没有 ready Codex 资源时返回 `503 codex_resource_not_ready_for_delivery`，并返回 required status、credential type、credential status 和内部巡检资源排除标识。
- 非 Codex 资源类型暂不受该闸门影响。
- 订单状态历史记录匹配到的 `supplierResourceId`，便于售出交付与共享资源审计交叉核查。
- 新增文档 `docs/codex-delivery-readiness-gate.md`。

#### 价值

- 新售出和重试交付不会在生产 Codex 资源不可交付时继续制造不可用租赁。
- 售出交付入口与 `resources.readyOnlineCodexResources`、共享资源 ready 闸门保持同一套可用性语义。
- 该能力不替代真实 `/v1/responses` smoke；最终可用性仍以有效 OpenAI refresh token、active Sub2 OpenAI 账号、账号测试和本地反代请求证据共同证明。

### 18.188 商品目录交付资源 readiness 巡检

#### 背景

18.187 已经让用户下单、管理员重试交付和租赁换钥匙在创建 Sub2 key 前检查 ready Codex 共享资源。但如果后台仍有 active Codex 商品和可购买价格，系统健康中的 `productCatalog` 只显示“公开商品目录可购买性正常”，管理员会在用户真正下单失败前缺少经营风险提示。因此商品目录巡检需要与交付闸门共享同一套 ready 资源语义。

#### 新增能力

- `GET /api/admin/system-health` 的 `productCatalog` 巡检新增指标：
...
- 该 issue 携带共享资源修复入口字段：
  - `resourceType=codex`
  - `resourceList=true`
  - `resourceScope=production`
  - `resourceStatus=online`
  - `repairAction=apply_openai_refresh_token_to_sub2_account`
- 新增 `codexCatalogDeliveryReadinessIssueFields()` helper，供系统健康和单元测试复用。
- API 单元测试覆盖“可售 Codex 商品但无 ready 资源会产生 warning 字段”，以及“已有 ready Codex 资源或非 Codex 商品不产生该问题”。

#### 价值

- 管理员会在用户购买失败前看到“商品可售但交付资源缺失”的风险，不再只靠下单接口返回 `503 codex_resource_not_ready_for_delivery` 才发现。
- 商品目录、售出交付、共享资源 ready 巡检和真实交付闸门共享同一套 Codex ready 资源语义。
- 该能力不自动下架商品，也不替代真实 OpenAI/Codex smoke；最终可用性仍以有效 OpenAI refresh token、active Sub2 OpenAI 账号、ready Codex 共享资源和 `/v1/responses` smoke 共同证明。

### 18.189 买家商品目录交付状态

#### 背景

18.187 和 18.188 已经让后端下单闸门和管理员系统健康都能识别“Codex 商品可售但没有 ready 交付资源”。但买家侧套餐页仍只读取 active 商品和 active 价格，用户可能看到可购买按钮，点击后才被下单接口拒绝。为了让公开体验与交付闸门一致，需要在商品接口和 Web UI 上提前暴露交付状态。

#### 新增能力

- `GET /api/products` 为每个商品新增：
  - `deliveryRequired`
  - `deliveryReady`
  - `readyDeliveryResources`
  - `deliveryBlockedReason`
- Codex 商品复用 ready production Codex shared resource 口径；没有 ready 资源时返回：
  - `deliveryRequired=true`
  - `deliveryReady=false`
  - `readyDeliveryResources=0`
  - `deliveryBlockedReason=codex_resource_not_ready_for_delivery`
- 非 Codex 商品返回 `deliveryRequired=false`、`deliveryReady=true`。
- Web 套餐页在 `deliveryReady=false` 时展示资源池暂不可交付提示。
- Web 开通按钮在 `deliveryReady=false` 时禁用，并展示“暂不可开通”。
- 新增文档 `docs/buyer-product-delivery-readiness.md`。
- API 单元测试覆盖公开商品 readiness 字段的 Codex 阻断、Codex ready 放行和非 Codex 放行。

#### 价值

- 买家不会在 Codex 资源不可交付时继续触发注定失败的下单请求。
- 买家侧、管理员商品目录巡检、售出交付闸门和共享资源 ready 口径保持一致。
- 该能力不自动下架商品，也不伪造真实 OpenAI/Codex 可用性；最终恢复仍需要有效 OpenAI refresh token、active Sub2 OpenAI 账号、ready Codex 共享资源和 `/v1/responses` smoke。

### 18.190 管理员商品交付状态

#### 背景

18.187 到 18.189 已经让下单、系统健康和买家商品目录都能识别“Codex 商品可售但没有 ready 交付资源”。但管理员商品管理页仍只展示商品上下架、资源类型、价格和售出统计，运营需要切换到系统健康或买家页才能判断商品是否真的可交付。商品管理入口需要直接展示同一套 delivery readiness。
...
- 管理员商品表新增“交付”列：
  - 可交付显示 `active`。
  - 不可交付显示 `blocked`。
  - Codex 商品显示 ready 资源数量。
  - 阻塞时显示 `codex_resource_not_ready_for_delivery`。
  - 非 Codex 商品显示 `not required`。
- 商品 CSV 导出新增 `deliveryReady`、`readyDeliveryResources`、`deliveryBlockedReason`，便于运营离线排查商品可售与交付资源是否一致。
- 新增文档 `docs/admin-product-delivery-readiness.md`。

#### 价值

- 管理员不需要离开商品管理页即可判断 Codex 商品是否可售且可交付。
- 系统健康中的 `active_codex_product_without_ready_delivery_resource` 可以直接回到商品列表确认影响面。
- 管理员商品管理、买家商品目录、售出交付闸门和共享资源 ready 巡检共享同一套 Codex ready 语义。
- 该能力不自动下架商品，也不替代真实 `/v1/responses` smoke；最终恢复仍需要有效 OpenAI refresh token、active Sub2 OpenAI 账号、ready Codex 共享资源和代理请求成功证据。

### 18.191 API 错误响应收敛

#### 背景

管理员维修 Sub2/OpenAI 账号、运行反代 smoke 或调整系统配置时，如果请求体格式、参数或框架解析出现 4xx 问题，接口不应该返回模糊的 `500 internal_error`。此前全局错误处理只识别业务 `AppError`，Fastify 框架级 4xx 和 Zod 校验错误会落入兜底 500 分支，容易把客户端输入问题误判为服务端故障。

#### 新增能力

- 全局错误处理继续保留 `AppError` 的显式 status、code、message 和 details。
- Zod 参数校验错误统一返回 HTTP 400：
  - `code=validation_error`
  - `details[]` 包含字段路径、校验 code 和 message。
- Fastify 框架级 4xx 错误保留原始 HTTP status。
- Fastify 错误 code 会归一为小写下划线格式，例如 `FST_ERR_CTP_EMPTY_JSON_BODY` 返回 `fst_err_ctp_empty_json_body`。
- 未识别错误和 5xx 仍返回脱敏 `500 internal_error`，并写入服务端日志。
- 新增单元测试覆盖：
  - 空 JSON body 的 Fastify 解析错误返回 400 而不是 500。
  - Zod 校验错误返回结构化 400。
  - 业务 `AppError` 状态保持不变。
...
- 管理员执行 `POST /api/admin/sub2/accounts/:id/refresh`、`POST /api/admin/sub2/accounts/:id/test` 等维修动作时，客户端输入问题会被准确呈现为 4xx，不再伪装成服务内部故障。
- 系统可用性复查更容易区分“请求格式错误”“参数错误”和“真实 Sub2/OpenAI 上游失效”。
- 该能力不恢复已经失效的 OpenAI refresh token；当前真实 `/v1/responses` 仍需要有效 OpenAI refresh token 或 active Sub2 OpenAI 账号。

### 18.192 管理员首页商品目录巡检定位

#### 背景

18.188 到 18.190 已经让系统健康、买家目录和管理员商品页都能识别 Codex 商品“可购买但没有 ready production Codex shared resource”的风险。完整可用性巡检页也能在 `productCatalog` issue 中看到 `productId`、`productName` 和 `priceId`。但管理员首页 `latestSystemHealth.criticalChecks` 对问题详情做了轻量白名单裁剪，商品定位字段没有进入首页预览，导致首页只能打开商品配置页，不能直接定位到受影响商品。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth.criticalChecks[].primaryIssue` 新增保留：
  - `productId`
  - `productName`
  - `priceId`
- `productCatalog` warning 纳入首页关键巡检优先级，避免被普通 warning 挤出最多 8 条的首页列表。
- Admin 首页关键巡检摘要新增展示商品定位字段。
- 当 `productCatalog` 问题携带商品定位字段时，首页按钮显示为“打开商品”，并按以下优先级打开商品管理列表：
  - `productId`
  - `priceId`
  - `productName`
- 若 `productCatalog` 没有商品定位字段，仍回退到商品配置入口。
- API 单元测试锁定 dashboard 预览会保留商品目录巡检的商品 ID、商品名、价格 ID、资源类型、资源范围和维修动作。
- 更新 `docs/admin-dashboard-health-snapshot.md` 与 `docs/product-catalog-purchasability.md`。

#### 价值

- 管理员从首页即可知道是哪一个 active Codex 商品受 ready 交付资源缺失影响。
- `productCatalog` warning 不再只是模块级跳转，而是能直接落到商品管理搜索结果。
- 商品可售性、售出交付、共享资源 ready 状态和首页可用性巡检形成更短的运营闭环。

### 18.193 管理员首页商品目录共享资源修复入口

#### 背景

18.192 让 `productCatalog` warning 能稳定进入管理员首页关键巡检并直达受影响商品。但当前线上该问题的真实修复动作不是修改商品本身，而是创建或修复 ready production Codex shared resource，并绑定有效 Sub2/OpenAI 账号与 active OpenAI refresh token。此前商品目录 issue 没有继承 `sub2AccountId` 等修复候选字段，管理员从首页看到商品风险后仍需要切换到共享资源页并手动填写目标账号。

#### 新增能力

- 系统健康报告在生成最终 `checks` 前，会把 Sub2 修复候选账号补充到所有 `repairAction=apply_openai_refresh_token_to_sub2_account` 且尚未携带 `sub2AccountId` 的 issue/sample。
- `productCatalog` issue 因此会继承：
  - `sub2AccountId`
  - `sub2AccountName`
  - `accountStatus`
  - `credentialsStatus`
  - `schedulable`
- `GET /api/admin/dashboard` 的关键巡检预览新增保留 `resourceList`。
- Admin 首页 `productCatalog` 行在具备资源修复上下文时，同时提供：
  - “打开商品”
  - “打开共享资源”
- “打开共享资源”会带入 `supplierEmail`、`resourceType`、`resourceScope`、`resourceStatus`、`sub2AccountId` 与 `repairAction`。
- 共享资源创建表单在 `repairAction=apply_openai_refresh_token_to_sub2_account` 且存在 Sub2 账号时，会继续默认勾选“创建后应用到 Sub2”和“应用后端到端自检”。
- API 单元测试覆盖商品目录修复候选字段继承；Admin 单元测试覆盖 productCatalog 默认值会继续凭据应用与 smoke 验收。
- 更新 `docs/admin-dashboard-health-snapshot.md` 与 `docs/product-catalog-purchasability.md`。

#### 价值

- 管理员从首页看到可售商品交付风险后，可以直接进入共享资源修复入口，不必手工复制 Sub2 账号。
- 商品确认入口和交付资源修复入口同时存在，既能核对经营风险，也能执行真实恢复动作。
- 当前线上 `productCatalog`、`resources`、`resourceCredentials`、`sub2` 和 `localProxySmoke` 的推荐修复账号进一步统一。

### 18.194 共享资源修复表单保留商品上下文

#### 背景

18.193 已经让管理员从 `productCatalog` 风险直接进入共享资源修复入口，并自动带入供给方、生产范围、目标 Sub2 账号和推荐维修动作。但共享资源创建表单的修复诊断条仍只展示来源、资源、账号、凭据应用和 smoke 信息，没有显示这次修复是为了恢复哪一个商品的交付能力。管理员在多个 Codex 商品共存时仍需要返回商品页确认影响对象。

#### 新增能力

- `ResourceCreateDefaults` 新增商品定位字段：
  - `productId`
  - `productName`
  - `priceId`
- Dashboard `productCatalog` 的“打开共享资源”会把商品定位字段传入共享资源创建默认值。
- 完整 `可用性巡检` 页的 `productCatalog` issue/sample 点击“打开共享资源”时，也会传递商品定位字段。
- `resourceCreateDefaultsContextItems()` 新增 `Product` 诊断项，显示商品名、商品 ID 和价格 ID。
- Admin 单元测试覆盖 productCatalog 修复上下文会展示 `Product`，并继续默认启用凭据应用和 smoke 验收。
- 更新 `docs/admin-dashboard-health-snapshot.md` 与 `docs/product-catalog-purchasability.md`。

#### 价值

- 管理员进入共享资源创建表单后，不必回到商品页即可确认受影响商品。
- 商品可售风险、共享资源修复、Sub2/OpenAI 凭据应用和 smoke 验收在同一个操作界面中保持上下文连续。
- 该能力不改变交付闸门或资源 ready 判定，只减少人工修复时的对象混淆风险。

### 18.195 管理员首页巡检快照新鲜度提示

#### 背景

管理员首页已经能够读取最近一条 `SystemHealthSnapshot` 并展示 Sub2/OpenAI 上游、共享资源、余额、售出交付和商品目录等关键巡检项。但首页此前只显示快照创建时间，没有明确告诉管理员这条快照是否仍然新鲜。如果最近巡检已经过去较久，管理员可能把历史问题或历史正常状态误认为当前可用性。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth` 新增快照新鲜度字段：
  - `ageMinutes`
  - `stale`
  - `staleThresholdMinutes`
- dashboard 新鲜度阈值为 60 分钟；快照年龄达到或超过 60 分钟时返回 `stale=true`。
- Admin 首页 `系统状态` 面板会展示：
  - 最近巡检的相对年龄，例如 `59 分钟前`、`2 小时 10 分钟前`。
  - 快照状态：`有效` 或 `已过期，建议重新巡检`。
  - 过期快照会以 warning 色调提示，但不会改写原始巡检 `status`。
- API 单元测试锁定 59 分钟仍有效、60 分钟起过期的边界，并确认关键巡检预览仍随快照返回。
- 更新 `docs/admin-dashboard-health-snapshot.md`。

#### 价值

- 管理员进入首页时可以区分“当前巡检结果”和“历史巡检快照”，减少对 Sub2/OpenAI、共享资源、余额和售出情况的误判。
- 首页继续只读取最近快照，不自动触发巡检写入，避免产生额外历史噪音。
- 当快照过期时，管理员可以直接进入 `可用性巡检` 手动刷新，形成更清晰的排障闭环。

### 18.196 OpenAI/Codex 本地错误 payload 补齐 param

#### 背景

本地 `/v1/*` 反代已经能够把 API Key、余额、租赁状态、请求量、RPM/TPM、并发和上游不可用等本地拦截错误映射为 OpenAI 风格的 `error.message`、`error.type` 与 `error.code`。但常见 OpenAI 错误对象还包含 `error.param` 字段；部分 SDK、日志解析器或调用方会按该字段做统一结构化处理。此前本地错误缺少 `param`，会让本地拦截错误和上游 OpenAI 错误对象存在轻微结构差异。

#### 新增能力

- `openAiProxyErrorPayload()` 生成的本地错误对象新增：
  - `error.param=null`
- 本地拦截错误继续保留原 HTTP 状态码、`error.message`、`error.type` 和 `error.code`。
- `inspectOpenAiProxyContract()` 摘要新增：
  - `localErrorPayloadIncludesParam`
- 当本地错误 payload 缺少 `error.param` 时，`openAiProxyContract` 会产生 `local_error_param_missing` error。
- API 单元测试覆盖 429 限流错误 payload 的 `param: null`，并锁定反代契约摘要会返回 `localErrorPayloadIncludesParam=true`。
- 更新 `docs/openai-proxy-error-compatibility.md` 与 `docs/openai-proxy-test-coverage.md`。

#### 价值

- 本地准入失败、余额不足、限流和上游不可用等错误更贴近 OpenAI 客户端期望的结构。
- 管理员系统健康页可以提前发现本地 OpenAI 错误格式回归，而不是等用户 SDK 解析失败后再排查。
- 该能力不改变反代准入规则、Sub2API 透传行为或真实 `/v1/responses` 可用性，只补齐兼容字段。

### 18.197 反代请求支持全部 request id 响应头搜索

#### 背景

本地 OpenAI/Codex 反代已经在 CORS 中暴露 `x-proxy-request-id` 和常见上游 request id 响应头，便于浏览器、SDK 或用户把诊断 ID 提交给管理员。但后台搜索规范化此前只识别 `x-proxy-request-id` 和 `x-request-id`。如果用户复制的是 `openai-request-id`、`x-openai-request-id` 或 `request-id` 完整响应头行，管理员仍需要手动删掉头名再搜索，容易在 `/v1/responses` 排障时漏掉上游证据。

#### 新增能力

- 新增统一 `proxyRequestLookupHeaderNames`，覆盖：
  - `x-proxy-request-id`
  - `x-request-id`
  - `openai-request-id`
  - `x-openai-request-id`
  - `request-id`
- `normalizeProxyRequestLookup()` 会从上述完整响应头行中提取纯 request id。
- `GET /api/admin/proxy-requests` 搜索继续复用同一个规范化 helper，因此后台 `反代请求` 页面可直接粘贴任一种响应头。
- `inspectOpenAiProxyContract()` 摘要新增：
  - `proxyRequestLookupHeaders`
  - `normalizesProxyRequestLookupHeaders`
- 当任一 request id 响应头不能被规范化时，`openAiProxyContract` 返回 `proxy_request_lookup_header_normalization_incomplete`。
- API 单元测试覆盖每个响应头名称的规范化结果。
- 更新 `docs/proxy-request-logs.md` 与 `docs/openai-proxy-test-coverage.md`。

#### 价值

- 管理员拿到用户或上游返回的完整 request id 响应头后，可以直接定位 `ProxyRequestLog`，减少手动清洗 ID 的步骤。
- CORS 暴露头、上游 request id 捕获、代理请求日志和管理员搜索入口共享同一套诊断口径。
- 该能力不改变 Sub2API 透传、鉴权、限流或真实上游调用，只缩短 OpenAI/Codex 反代排障路径。

### 18.198 共享资源创建凭据应用确认保留商品上下文

#### 背景

18.193 和 18.194 已经让 `productCatalog` 可交付性风险可以从管理员首页或完整巡检页进入共享资源修复入口，并在创建表单诊断条中展示商品名、商品 ID 和价格 ID。但真正会写入外部状态的动作发生在“创建共享资源后应用初始凭据到 Sub2”这一提交确认里。此前确认弹窗只展示供给方、资源类型、Sub2 账号、Client ID、Proxy ID 和 smoke 设置，管理员在最后确认时仍可能失去“这次修复对应哪一个商品”的上下文。

#### 新增能力

- Admin 新增 `resourceCreateDefaultsProductText()` 纯 helper，统一格式化资源创建默认值中的商品上下文：
  - 商品名
  - 商品 ID
  - 价格 ID
- helper 会清理空白字符串，避免确认信息出现空分隔符。
- 共享资源创建表单的诊断条继续复用同一商品上下文格式。
- 当管理员勾选 `创建后应用到 Sub2` 时，确认弹窗会在供给方后展示：
  - `关联商品：商品名 / 商品 ID / 价格 ID`
- 只有修复上下文中存在商品字段时才展示 `关联商品` 行；普通共享资源创建不增加额外提示。
- Admin 单元测试锁定完整商品上下文、仅商品 ID、空白值三种格式化结果。

#### 价值

- 管理员在最后一次高风险确认前仍能看到这次 OpenAI/Codex 凭据应用是为了恢复哪一个可售商品。
- 商品可售风险、共享资源创建、Sub2 凭据应用和 `/v1/responses` smoke 验收之间的操作上下文更连续。
- 该能力不改变资源创建、凭据保存、Sub2API 调用、ready 闸门或真实 OpenAI/Codex 反代行为，只降低人工维修时误选对象的风险。

### 18.199 商品目录 ready 资源风险定位到可购买价格

#### 背景

线上 `productCatalog` 巡检已经能识别 active Codex 商品可购买但没有 ready production Codex shared resource，并把管理员带到商品或共享资源修复入口。但实际线上 issue 仍可能只携带 `productId/productName`，`priceId` 为空，导致 Dashboard 预览、共享资源修复诊断条和最终凭据应用确认只能定位到商品，不能定位到触发可购买风险的具体价格档位。

#### 新增能力

- `codexCatalogDeliveryReadinessIssueFields()` 新增可选 `priceId` 入参。
- `inspectProductCatalogReadiness()` 在确认商品存在可购买 active price 后，会把首个可购买价格 ID 写入 Codex delivery readiness issue。
- `productCatalog` issue id 从 `active_codex_product_without_ready_delivery_resource:<productId>:product` 收敛为包含具体 price 的 `active_codex_product_without_ready_delivery_resource:<productId>:<priceId>`。
- 现有 Dashboard 关键巡检预览、商品页搜索、共享资源修复默认值、诊断条和凭据应用确认弹窗会继续沿用该 `priceId`。
- API 单元测试覆盖 Codex delivery readiness issue 会保留价格定位字段。

#### 价值

- 管理员看到“商品可售但无法交付”时，可以直接确认是哪一个可购买价格档位触发风险。
- 商品配置、共享资源修复、Sub2/OpenAI 凭据应用和售出交付排障的定位粒度更一致。
- 该能力不改变商品是否可购买、资源 ready 判定、下单闸门或真实 `/v1/responses` 行为，只补齐系统健康证据字段。

### 18.200 Sub2 修复上下文继承商品价格定位

#### 背景

18.199 已经让 `productCatalog` issue 能带出具体 `priceId`。但同一线上阻断还会同时出现在 `resources`、`resourceCredentials`、`sub2` 和 `localProxySmoke` 等巡检项中。管理员如果从这些更高优先级 error/warning 进入修复入口，仍可能只能看到 Sub2 账号和资源类型，看不到触发售卖风险的商品与价格档位。

#### 新增能力

- `enrichSub2RepairContextChecks()` 会从 `productCatalog` 的 `active_codex_product_without_ready_delivery_resource` issue 中提取：
  - `productId`
  - `productName`
  - `priceId`
- 对同一个 `apply_openai_refresh_token_to_sub2_account` 修复动作，若目标记录缺少商品字段且资源类型为空或为 `codex`，会自动补齐上述商品价格上下文。
- 该补齐覆盖 `resources`、`resourceCredentials`、`sub2`、`localProxySmoke` 等共享的 Sub2/OpenAI 修复问题。
- 已有字段不会被覆盖；非 Codex 资源类型不会被强行注入商品上下文。
- API 单元测试覆盖资源缺失和资源凭据缺失 issue 会继承商品名、商品 ID、价格 ID 和 Sub2 修复候选账号。

#### 价值

- 管理员从任意相关巡检项进入共享资源或 Sub2 修复入口时，都能看到同一个商品价格定位。
- `productCatalog`、`resources`、`resourceCredentials`、`sub2` 和 `localProxySmoke` 的修复上下文不再漂移。
- 该能力不改变巡检状态、资源 ready 判定、Sub2API 调用或真实 `/v1/responses` 行为，只补齐跨巡检项的操作证据。

### 18.201 OpenAI/Codex 浏览器端限流响应头暴露

#### 背景

本地反代已经通过 CORS 暴露 `x-proxy-request-id` 和常见上游 request id，方便浏览器端排查 `/v1/responses` 请求。但 OpenAI/Codex 客户端在遇到本地 RPM/TPM、套餐限额或上游 429 时，还需要读取 `retry-after` 与 `x-ratelimit-*` 头来判断重试间隔和剩余额度。此前这些响应头即使由本地或上游返回，也不会被浏览器 JavaScript 读取到。
...
- `openAiProxyCorsExposedHeaders` 同时暴露：
  - 本地 `x-proxy-request-id`
  - 常见上游 request id
  - 限流/重试头
- `inspectOpenAiProxyContract()` 摘要新增：
  - `rateLimitHeaders`
  - `corsExposesRateLimitHeaders`
- 当 CORS 没有暴露上述限流/重试头时，`openAiProxyContract` 会返回 `rate_limit_headers_not_exposed`。
- API 单元测试覆盖真实 `Access-Control-Expose-Headers` 中包含 `retry-after` 和 `x-ratelimit-*`。

#### 价值

- 浏览器端 OpenAI/Codex 客户端可以在 429 或额度受限时读取重试间隔与剩余额度，减少只能依赖后台日志定位限流问题。
- 系统健康能提前发现 CORS 暴露头与反代兼容契约漂移。
- 该能力不改变鉴权、余额、限流规则、Sub2API 透传或真实 `/v1/responses` 可用性，只补齐浏览器端可观测响应头。

### 18.202 OpenAI/Codex 本地限流响应头写出

#### 背景

18.201 已经让浏览器端可以读取 `retry-after` 与 `x-ratelimit-*` 头，但这只解决了 CORS 暴露问题。若 429 是本地反代自身拦截产生，例如并发租约、RPM/TPM 或套餐请求量耗尽，本地错误响应也必须实际写出兼容头，否则 OpenAI/Codex 客户端仍无法根据响应判断重试间隔和剩余额度。

#### 新增能力

- 新增 `openAiProxyRateLimitHeaders()`，统一生成本地 429 限流响应头。
- `concurrency_limit_exceeded` 返回：
  - `retry-after`
  - `retry-after-ms`
- `rpm_limit_exceeded` / `tpm_limit_exceeded` 返回：
  - `retry-after`
  - `retry-after-ms`
  - `x-ratelimit-limit-requests`
  - `x-ratelimit-remaining-requests`
  ...
  - `x-ratelimit-remaining-tokens`
  - `x-ratelimit-reset-tokens`
- `request_limit_exceeded` 返回：
  - `x-ratelimit-limit-requests`
  - `x-ratelimit-remaining-requests=0`
  - 不伪造分钟级 `retry-after`，因为套餐请求量不等同于滚动窗口限流。
- 内存 limiter 在 RPM/TPM 失败时返回当前用量和基于滚动窗口计算的 `retryAfterMs`。
- Redis limiter 在 RPM/TPM 失败时返回当前用量，并使用 60 秒窗口作为保守 `retryAfterMs`。
- `inspectOpenAiProxyContract()` 摘要新增：
  - `setsLocalRateLimitHeaders`
- 当本地 429 响应头契约不完整时，系统健康返回：
  - `local_rate_limit_headers_incomplete`
- API 单元测试覆盖：
  - helper 生成完整限流头。
  - 并发耗尽返回 `retryAfterMs`。
  - RPM 耗尽返回 `rpmLimit`、`rpmUsed` 与 `retryAfterMs`。
  - 反代契约摘要返回 `setsLocalRateLimitHeaders=true`。

#### 价值

- 浏览器端与 SDK 客户端在遇到本地 429 时不再只能看到错误 payload，还能读取重试间隔与额度余量。
- 管理员可在系统健康里区分 CORS 暴露是否正确，以及本地限流响应是否真正写出兼容头。
- 该能力不改变鉴权、余额、租赁状态、请求量/RPM/TPM/并发规则、Sub2API 透传或真实 `/v1/responses` 可用性，只补齐本地限流响应的兼容性与可观测性。

### 18.203 可用性巡检详情保留商品名称

#### 背景

线上 Codex 可用性阻断通常会同时出现在 `productCatalog`、`resources`、`resourceCredentials`、`sub2` 和 `localProxySmoke` 等巡检项中。前序能力已经让这些问题继承 `productId`、`productName` 和 `priceId`，并能把管理员带到商品或共享资源修复入口。但完整 `可用性巡检` 页的 issue ref / sample summary 仍由共享白名单控制；若白名单只包含 `productId` 与 `priceId`，管理员在详情行里仍需要凭 UUID 反查商品名称。

#### 新增能力

- `adminSystemHealthIssueRefFields` 新增：
  - `productName`
- `adminSystemHealthSampleSummaryFields` 新增：
  - `productName`
- 完整巡检页展示 issue 和 sample 时，商品上下文会同时包含：
  - 商品名称
  - 商品 ID
  - 价格 ID
- Admin 单元测试锁定 issue ref 与 sample summary 都会保留 `productName`。

#### 价值

- 管理员排查“Codex 商品可售但无 ready 交付资源”时，可以直接看到受影响商品名，而不是只看 ID。
- `productCatalog`、`resources`、`resourceCredentials`、`sub2` 与 `localProxySmoke` 的修复上下文在首页、完整巡检页、共享资源修复表单和 Sub2 修复入口之间更一致。
- 该能力不改变系统健康判定、商品可售规则、共享资源 ready 规则、Sub2API 调用或真实 `/v1/responses` 可用性，只提升管理员入口的可读性和排障效率。

### 18.204 可用性巡检商品跳转支持商品名兜底

#### 背景

18.203 已经让完整 `可用性巡检` 页在 issue ref 和 sample summary 中展示 `productName`。但行内“打开商品”动作此前仍只按 `productId` 或 `priceId` 生成商品列表搜索词。线上某些跨链路证据可能只保留商品名，导致管理员能看到商品名，却仍需要手动复制到商品管理页搜索。

#### 新增能力

- 共享包新增 `adminProductLookupCandidate()`，统一输出商品管理页搜索关键词。
- 商品跳转关键词优先级为：
  - `productId`
  - `priceId`
  - `productName`
- Admin 首页关键巡检项的商品跳转继续复用该 helper。
- 完整 `可用性巡检` 页 issue 行与 sample 行的 `productLookup` 也改为复用该 helper。
- Admin 单元测试覆盖 ID、价格 ID、商品名和空商品名四种情况。

#### 价值

- 管理员在 `productCatalog`、`resources`、`resourceCredentials`、`sub2` 或 `localProxySmoke` 问题只携带商品名时，仍然可以直接跳到商品管理筛选入口。
- 首页和完整巡检页的“打开商品”行为保持一致，减少排障时入口差异造成的误判。
- 该能力不改变商品可售规则、资源 ready 规则、系统健康判定或真实 `/v1/responses` 调用，只补齐人工排障入口的兜底搜索能力。

### 18.205 本地反代 smoke 证据暴露过期阈值

#### 背景

线上 `localProxySmoke` 已经返回 `ageMinutes` 与 `stale`，但管理员看到类似 `ageMinutes=1278`、`stale=false` 时，仍需要知道当前 freshness 窗口是多少，才能判断这条 `/v1/responses` 失败证据为什么尚未过期。此前 24 小时阈值只体现在后端逻辑和文档里，没有跟随最新证据、问题样本和首页关键巡检上下文一起返回。

#### 新增能力

- `localProxySmoke.detail.latest` 新增 `staleThresholdMinutes`。
- `localProxySmoke.detail.issues[]` 新增 `staleThresholdMinutes`。
- `localProxySmoke.summary` 在有无 smoke 证据时都会返回 `staleThresholdMinutes`。
- Dashboard 关键巡检预览保留 `staleThresholdMinutes`。
- Admin 完整巡检 issue/sample 字段白名单保留 `staleThresholdMinutes`。
- Admin 从首页、完整巡检 issue 或 sample 打开 `反代状态` 时，会把 `staleThresholdMinutes` 带入修复上下文。
- `反代状态 -> 修复定位 -> 失败请求` 会同时展示证据年龄和阈值，例如 `12 分钟前 / 阈值 1440 分钟`。

#### 价值

- 管理员能直接区分“证据很旧但仍在 24 小时窗口内”和“证据已经过期”，减少对 `stale=false` 的误判。
- 修复 OpenAI refresh token 后，管理员能更清楚地判断是否需要立即重新运行端到端 smoke 刷新 live `/v1/responses` 证据。
- 该能力不改变 smoke freshness 判定、不触发真实 OpenAI/Codex 请求、不写入 Sub2API 或业务数据，只提升可用性巡检证据的解释性。

### 18.206 反代请求健康分类排除本地准入误报

#### 背景

线上 `proxy` 巡检会统计最近 1 小时 OpenAI/Codex `/v1/*` 反代请求。此前缺少 Bearer Key、无效 Key、余额不足、租赁不可用、请求量/RPM/TPM/并发限流等本地准入拒绝，会与真正的平台可用性故障一起触发 warning。公网探测 `/v1/models` 未带 Key 时虽然是符合预期的 401，也会让健康面板看起来像本地反代异常。

#### 新增能力

- 后端新增本地客户端准入拒绝分类，覆盖 `missing_api_key`、`invalid_api_key`、`insufficient_balance`、`rental_not_active`、`request_limit_exceeded`、`rpm_limit_exceeded`、`tpm_limit_exceeded`、`concurrency_limit_exceeded` 等错误码。
- `GET /api/admin/system-health` 的 `proxy.metrics` 新增：
  - `proxyRecentClientRejections`
  - `proxyRecentActionableClientErrors`
- `proxyRecentLocalErrors` 改为只统计本地可用性错误，例如 limiter 不可用、Sub2API 超时或不可达。
- `proxy.status` 规则调整：
  - 只有本地准入拒绝时保持 `ok`。
  - 需复查 4xx 或客户端断开时标记 `warning`。
  - 5xx、本地可用性错误或上游流异常标记 `error`。
- `proxy.detail.issues` 只返回需要排查的反代请求样本，不再把本地准入拒绝作为健康问题样本。
- 新增 API 单元测试 `admin-proxy-health.test.ts`，锁定本地准入拒绝不会误触发 warning，真实 4xx/5xx/流异常仍会升高状态。
- 新增文档 `docs/system-health-proxy-request-classification.md`，并更新 `docs/system-health-check.md`、`docs/system-health-issue-samples.md` 与 `docs/proxy-request-logs.md`。

#### 价值

- 未认证公网探测或客户错误 Key 不再污染系统健康状态，管理员看到的 `proxy` warning/error 更接近真实平台可用性问题。
- 管理员仍能通过 `proxyRecentClientRejections` 观察本地准入拒绝数量，用于定位调用噪音、限额触发或客户配置错误。
- 真正影响交付的上游不可用、限流器不可用、上游流异常和非准入类 4xx 仍会进入巡检问题样本，并可跳转到 `反代请求` 日志继续排障。

### 18.207 Sub2 修复定位显示账号诊断摘要

#### 背景

当前线上真实阻断已经明确集中在 Sub2/OpenAI 账号无 active 上游，候选账号样本中能看到 OpenAI 返回的 `token_invalidated` / `token revoked` 摘要。但管理员从首页或完整 `可用性巡检` 页点击“打开反代状态”后，`反代状态 -> 修复定位` 此前主要展示目标账号、账号状态、凭据状态、资源和 smoke 失败请求，具体账号失败摘要仍需要切回巡检样本查看。

#### 新增能力

- Admin `Sub2RepairContext` 新增账号诊断字段：
  - `schedulable`
  - `tempUnschedulableReason`
  - `accountUpdatedAt`
  - `accountMessage`
- 完整 `可用性巡检` 的 issue/sample 行会保留 `sub2AccountName`、`accountStatus`、`credentialsStatus`、`schedulable`、`tempUnschedulableReason`、`updatedAt` 和原始 `message`。
- Dashboard 关键巡检预览白名单保留 `tempUnschedulableReason` 与 `updatedAt`，并把 `message` 作为账号诊断带入反代状态。
- `反代状态 -> 修复定位` 新增 `账号诊断` 行，展示是否可调度、临时不可调度原因、账号更新时间和 OpenAI/Sub2 返回的失败摘要。
- Admin 单元测试更新 `sub2 repair context summarizes operator drilldown targets`，锁定账号诊断行不会丢失。
- 更新 `docs/admin-sub2-repair-context-prefill.md`、`docs/admin-dashboard-health-snapshot.md` 与 `docs/system-health-issue-samples.md`。

#### 价值

- 管理员从首页或完整巡检页进入 `反代状态` 后，可以直接看到目标账号为何不可用，例如 `schedulable=false` 与 `token_invalidated`，无需在巡检样本和修复页之间来回切换。
- 对当前 `openai_group_has_no_active_accounts` 阻断，修复入口可以同时呈现“补有效 refresh token”的动作、目标账号和失败原因，降低误选账号或误判故障层级的风险。
- 该能力不展示或缓存 OpenAI refresh token，不改变 Sub2API 调用、账号刷新、资源凭据保存或真实 `/v1/responses` 可用性，只增强管理员修复上下文的可解释性。

### 18.208 Sub2 主修复问题继承账号诊断摘要

#### 背景

18.207 已经让反代修复定位显示账号诊断字段，但线上 `sub2` 关键巡检默认优先使用 `primaryIssue`。当主 issue 只携带通用阻断 `message`，而账号级 `token_invalidated`、`updatedAt` 等信息只存在于候选 sample 时，管理员从首页直接进入 `反代状态` 仍可能看不到最具体的失败原因。

#### 新增能力

- `resourceCredentialRepairCandidateFields()` 会从首个可修复 Sub2 OpenAI 账号继承：
  - `tempUnschedulableReason`
  - `accountMessage`
  - `updatedAt`
- `Sub2/OpenAI 上游` 的 `openai_group_has_no_active_accounts` 主 issue 会继续保留通用阻断 `message`，同时通过 `accountMessage` 暴露账号级失败摘要。
- Dashboard 关键巡检预览白名单保留 `accountMessage`。
- Admin 从首页、完整巡检 issue 或 sample 打开 `反代状态` 时，账号诊断优先读取 `accountMessage`，缺失时再回退到旧的 `message`。
- 完整巡检对象摘要白名单补齐 `accountMessage`，便于管理员直接区分“分组无 active 账号”和“某个账号 token 已失效”。
- API 单元测试锁定主 issue 不覆盖阻断 `message`，并同时保留账号级 `accountMessage`、`tempUnschedulableReason` 和 `updatedAt`。

#### 价值

- 管理员从首页 `sub2` 检查进入修复页时，可以直接看到具体账号失败原因，不必先打开完整巡检候选样本。
- `message` 与 `accountMessage` 语义分离后，巡检列表仍能展示聚合阻断摘要，修复定位则展示账号级诊断，降低混淆。
- 该能力不改变 Sub2API 调用、账号测试、凭据应用、资源保存或真实 `/v1/responses` 验收，只补齐健康证据在管理入口之间的透传。

### 18.209 本地反代 smoke 证据暴露剩余新鲜时间

#### 背景

线上 `localProxySmoke` 已经返回 `ageMinutes`、`stale` 和 `staleThresholdMinutes`，管理员可以判断证据是否过期。但当前真实生产证据常处于“失败但尚未 stale”的边界状态，例如 `ageMinutes=1332`、`staleThresholdMinutes=1440`。此时管理员仍需要手工计算还剩多少分钟过期，才能决定是否先刷新 `/v1/responses` live smoke 证据。

#### 新增能力

- `localProxySmoke.summary` 新增 `freshMinutesRemaining`。
- `localProxySmoke.detail.latest` 新增 `freshMinutesRemaining`。
- `localProxySmoke.detail.issues[]` 新增 `freshMinutesRemaining`。
- 当缺少 smoke 证据时，`freshMinutesRemaining=null`。
- 当 smoke 证据已过期时，`freshMinutesRemaining=0`。
- Dashboard 关键巡检预览白名单保留 `freshMinutesRemaining`。
- 完整 `可用性巡检` 的 issue ref 与 sample summary 字段白名单保留 `freshMinutesRemaining`。
- Admin 从首页、完整巡检 issue 或 sample 打开 `反代状态` 时，会把 `freshMinutesRemaining` 带入修复上下文。
- `反代状态 -> 修复定位 -> 失败请求` 在证据未过期时展示 `剩余 X 分钟过期`；证据已过期时继续展示 `证据已过期`。

#### 价值

- 管理员无需心算即可知道当前失败 smoke 证据距离过期还有多久，尤其适合临近 24 小时窗口时做维修优先级判断。
- 修复 OpenAI refresh token 或 Sub2 账号后，管理员能更清楚地判断是否应该立即运行端到端自检刷新 `/v1/models` 与 `/v1/responses` 证据。
- 该能力不改变 freshness 判定、不触发真实 OpenAI/Codex 请求、不写入 Sub2API 或业务数据，只增强可用性巡检证据在管理员入口中的可解释性。

### 18.210 共享资源修复入口保留 Sub2 账号诊断

#### 背景

线上剩余阻断会同时暴露在 `resources`、`productCatalog`、`resourceCredentials`、`sub2` 和 `localProxySmoke`。其中 `resources` 与 `productCatalog` 的首页按钮会优先打开 `共享资源`，用于创建生产 Codex 共享资源。但此前共享资源创建诊断条只展示 Sub2 账号 ID、商品、失败请求和 smoke 默认行为，没有展示目标账号的状态、凭据状态和 token invalidated 摘要。管理员在共享资源入口仍可能需要切回 `反代状态` 或完整巡检样本确认账号根因。

#### 新增能力

- `ResourceCreateDefaults` 新增账号诊断字段：
  - `sub2AccountName`
  - `accountStatus`
  - `credentialsStatus`
  - `schedulable`
  ...
  - `accountMessage`
  - `accountUpdatedAt`
- `ResourceCreateDefaults` 同步保留 smoke 失败证据字段：
  - `smokeTestSkippedReason`
  - `ageMinutes`
  - `stale`
  - `staleThresholdMinutes`
  - `freshMinutesRemaining`
- Dashboard `resources` / `productCatalog` 共享资源跳转会传递上述上下文。
- 完整 `可用性巡检` issue/sample 行点击 `打开共享资源` 时也会传递上述上下文。
- 共享资源创建表单诊断条新增：
  - `Account status`
  - `Account diagnostics`
- 共享资源创建表单的 `Failure` 项会展示失败路径、HTTP 状态、代理错误码、证据年龄、过期阈值和剩余新鲜时间。
- Admin 单元测试锁定共享资源创建诊断条会保留账号状态、账号诊断和 smoke 剩余新鲜时间。

#### 价值

- 管理员从 `resources` 或 `productCatalog` 进入生产 Codex 共享资源创建时，可以直接确认目标账号为何不可调度，例如 `schedulable=false` 与 `token_invalidated`。
- 共享资源修复入口与 `反代状态` 修复入口看到的账号根因保持一致，减少入口切换和误判。
- 该能力不展示或缓存 OpenAI refresh token，不改变资源创建、凭据保存、Sub2 应用、账号测试或真实 `/v1/responses` smoke 逻辑，只增强管理员提交前的可解释性。

### 18.211 本地反代 smoke 证据暴露绝对过期时间

#### 背景

18.205 已经暴露 `staleThresholdMinutes`，18.209 已经暴露 `freshMinutesRemaining`。但线上真实证据经常处于“失败但尚未 stale”的临界窗口，例如 `freshMinutesRemaining=87`。管理员仍需要把 `createdAt + 24h` 或当前时间 + 剩余分钟换算成具体过期时间，才能判断是否应该优先刷新 `/v1/responses` live smoke 证据。

#### 新增能力

- `localProxySmoke.summary` 新增 `staleAt`。
- `localProxySmoke.detail.latest` 新增 `staleAt`。
- `localProxySmoke.detail.issues[]` 新增 `staleAt`。
- 当缺少 smoke 证据时，`staleAt=null`。
- `staleAt` 按最新 smoke 审计记录的 `createdAt + systemHealthLocalSmokeFreshMs` 计算，当前窗口为 24 小时。
- Dashboard 关键巡检预览白名单保留 `staleAt`。
- 完整 `可用性巡检` 的 issue ref 与 sample summary 字段白名单保留 `staleAt`。
- Admin 从首页、完整巡检 issue 或 sample 打开 `反代状态` 时，会把 `staleAt` 带入修复上下文。
- Admin 从首页或完整巡检打开 `共享资源` 修复入口时，资源创建默认值会继续保留 `staleAt`。
- `反代状态 -> 修复定位 -> 失败请求` 与共享资源创建表单 `Failure` 项会显示 `staleAt <ISO 时间>`。

#### 价值

- 管理员不需要手工换算即可知道 smoke 证据的具体过期时刻。
- 当 `/v1/responses` 失败证据尚未过期但接近窗口末尾时，管理员可以更准确地决定先刷新证据还是先补 OpenAI refresh token。
- 该能力不改变 freshness 判定、不触发真实 OpenAI/Codex 请求、不写入 Sub2API 或业务数据，只增强系统可用性证据的可解释性。

### 18.212 Sub2 修复问题继承最近 smoke 失败证据

#### 背景

线上剩余阻断通常会同时出现在 `productCatalog`、`resources`、`resourceCredentials`、`sub2` 和 `localProxySmoke`。此前 `localProxySmoke` 已经能展示 `/v1/models` 与 `/v1/responses` 的最新失败路径、HTTP 状态、代理错误码、证据年龄、剩余新鲜时间和 `staleAt`。但管理员如果从共享资源、商品或资源凭据入口进入修复流程，这些修复问题只携带商品、账号和资源上下文，缺少同一份 smoke 失败证据，仍需要切换到 `localProxySmoke` 才能确认真实失败阶段。

#### 新增能力

- `enrichSub2RepairContextChecks()` 会从 `localProxySmoke.detail.issues[]` 中提取最新可修复 smoke 证据。
- 对 `repairAction=apply_openai_refresh_token_to_sub2_account` 且资源类型为空或为 `codex` 的问题，系统会在字段缺失时继承：
  - `model`
  - `modelsOk`
  - `responsesOk`
  - `localProxyOk`
  ...
  - `freshMinutesRemaining`
  - `staleAt`
- 已存在的字段不会被覆盖，`message` 与 `actionHint` 等原问题语义保持不变。
- 该继承能力覆盖 `productCatalog`、`resources`、`resourceCredentials`、`sub2` 以及同类共享修复问题。
- API 单元测试锁定共享修复问题会保留 smoke 失败证据，同时不覆盖原有修复建议。

#### 价值

- 管理员从共享资源、商品、资源凭据或 Sub2 任一入口进入修复流程时，都能看到与 `localProxySmoke` 一致的 `/v1/responses` 失败证据。
- 共享资源创建表单、首页关键巡检和完整可用性巡检可以直接展示失败路径、HTTP 状态、代理错误码、请求 ID、证据年龄和 `staleAt`。
- 管理员无需在多个巡检项之间来回切换，即可判断当前阻断是否仍是 OpenAI refresh token 失效或 Sub2 账号不可调度。
- 该能力只复用已有巡检证据，不触发新的 OpenAI/Codex 请求、不写入 Sub2API、不改变资源创建、凭据保存或 smoke test 执行条件。

### 18.213 共享修复问题保留 smoke 审计追溯字段

#### 背景

18.212 已经让商品、共享资源、资源凭据和 Sub2 修复问题继承 `/v1/responses` 失败路径、HTTP 状态、代理错误码和 freshness 字段。但线上复查发现，`localProxySmoke` 自身有 `auditLogId`，而继承后的 `productCatalog`、`resources`、`resourceCredentials`、`sub2` 问题没有该审计定位。管理员从这些入口看到失败证据后，仍无法一键打开产生该证据的审计记录，排查链路还差最后一步。

#### 新增能力

- `localProxySmoke` 作为共享修复证据源时，额外继承：
  - `auditLogId`
  - `auditAction`
  - `keyDisabled`
  - `proxyRequestLogCount`
- 继承范围仍限定为 `repairAction=apply_openai_refresh_token_to_sub2_account` 且资源类型为空或 `codex` 的问题。
- 已存在字段不会被覆盖，原巡检项的商品、资源、账号、消息和修复建议保持不变。
- Dashboard 关键巡检预览已有 `auditLogId` 白名单，完整 `可用性巡检` issue/sample 行已有 `auditLogId` 与 `auditAction` 的审计打开能力，因此后端继承后可以直接出现“打开审计”入口。
- API 单元测试锁定共享修复问题会保留 `auditLogId`、`auditAction`、`keyDisabled` 和 `proxyRequestLogCount`。

#### 价值

- 管理员从 `productCatalog`、`resources`、`resourceCredentials` 或 `sub2` 任一修复入口看到 smoke 失败后，可以继续打开同一条审计记录确认完整脱敏结果。
- 失败路径、代理请求日志和审计日志之间形成闭环，适合复查 Sub2API/OpenAI 返回 `token_invalidated`、临时 Key 是否清理、代理日志数量是否足够等细节。
- 该能力只透传已有审计字段，不触发新的 smoke、不读取明文 refresh token、不改变系统健康判定或业务写入。

### 18.214 Sub2 账号候选样本继承 smoke 修复上下文

#### 背景

18.212 和 18.213 已经让 `sub2` 主问题继承 smoke 失败证据和审计定位。但线上复查发现，`sub2.detail.samples` 中的账号候选样本仍只展示账号状态、凭据状态和可调度状态，缺少 `repairAction`、`resourceType=codex`、`auditLogId`、失败路径和 freshness 字段。管理员从 Sub2 候选样本行进入 `反代状态` 时，仍可能丢失这次 `/v1/responses` 失败证据。

#### 新增能力

- `enrichSub2RepairContextChecks()` 会把 `sub2` 检查项的账号候选样本识别为 OpenAI refresh token 修复候选。
- 对带 `sub2AccountId` 且包含账号诊断字段的 `sub2.detail.samples[]`，系统会补齐：
  - `repairAction=apply_openai_refresh_token_to_sub2_account`
  - `sub2Status=true`
  - `resourceType=codex`
  - 最新 smoke 失败证据字段
  - 最新 smoke 审计追溯字段
- 样本原有账号名、账号状态、凭据状态、调度状态、错误摘要和更新时间不被覆盖。
- API 单元测试锁定 Sub2 账号候选样本会继承 smoke 失败证据、审计定位和修复动作。

#### 价值

- 管理员不仅从 Sub2 主问题行进入修复时有完整上下文，从候选账号样本行进入时也能看到同一条 `/v1/responses` 失败证据。
- 当默认 OpenAI 分组存在多个错误账号时，管理员可以逐个候选账号打开 `反代状态`，并保持相同 smoke/audit 追溯链。
- 该能力只补齐巡检样本上下文，不改变 Sub2API 账号状态、不执行账号测试、不触发真实 OpenAI/Codex 请求。

### 18.215 Dashboard 关键巡检保留完整 smoke 清理与审计字段

#### 背景

线上 `GET /api/admin/system-health` 已经在相关修复问题和 Sub2 候选样本中保留 `auditAction`、`keyDisabled`、`proxyRequestLogCount` 和 `sub2Status`。但 `GET /api/admin/dashboard` 的关键巡检预览使用独立字段白名单，仍只保留 `auditLogId`，过滤掉审计动作、临时 Key 清理状态、代理日志数量和 Sub2 修复标记。管理员从首页看到的证据因此弱于完整巡检。

#### 新增能力

- Dashboard 关键巡检预览白名单新增：
  - `sub2Status`
  - `auditAction`
  - `keyDisabled`
  - `proxyRequestLogCount`
- `primaryIssue` 和 `primarySample` 都会保留这些字段。
- API 单元测试锁定 dashboard 关键巡检预览会保留完整 smoke 审计与清理证据。

#### 价值

- 首页关键巡检现在能直接展示与完整巡检一致的 smoke 清理和审计上下文。
- 管理员可以从首页判断临时 Sub2 Key 是否已禁用、代理日志数量是否足够、审计动作是否来自 `admin.sub2.proxy_smoke_test`。
- 该能力只扩展 dashboard 摘要字段，不改变系统健康判定、关键巡检排序、Sub2API 调用或真实 OpenAI/Codex 请求。

### 18.216 Sub2API 原始路径与查询透传契约

#### 背景

本地 OpenAI/Codex 反代已经通过 `/v1` 与 `/v1/*` 统一承接售出 Key 的访问，并将请求转发到 Sub2API。但运行代码此前直接拼接 `SUB2_BASE_URL + request.raw.url`，虽然行为简单可用，仍缺少独立 helper 与单元测试来证明：Sub2 base 尾斜杠会被规整、`/v1/responses/:id/input_items` 这类 Codex/Responses 子路径不会被改写、query 参数不会丢失。为了让“基于 Sub2API 的完整 OpenAI/Codex 反代内核”具备更强验收证据，需要把这层拼接与路径覆盖做成显式契约。

#### 新增能力

- 新增 `buildSub2ProxyUrl(baseUrl, rawUrl)`：
  - 去掉 Sub2API base 尾部多余 `/`。
  - 兼容带 `/` 或不带 `/` 的原始 OpenAI 路径。
  - 保留完整 path 与 query。
- `registerOpenAiProxyRoutes()` 使用该 helper 构造上游 URL，运行路径与契约测试复用同一实现。
...
- 当核心样例路径没有全部进入本地 `/v1` 反代时，健康检查返回 `core_openai_path_samples_not_routed`。
- 当 Sub2API URL 拼接无法保留原始 path/query 或无法规整 base 尾斜杠时，健康检查返回 `sub2_proxy_url_forwarding_incomplete`。
- API 单元测试锁定上述 helper、样例路径和契约摘要。

#### 价值

- 管理员在 `OpenAI 反代契约` 巡检中可以直接看到 `/v1`、Responses、Responses input items、Chat Completions、files、uploads 和 batches 等样例路径处于同一条 Sub2API 转发契约内。
- 后续修改 `SUB2_BASE_URL` 或反代路由时，测试会更早发现双斜杠、漏 query、漏 `/v1` 基路径或 Codex 子路径未覆盖等问题。
- 该能力不改变鉴权、余额、租赁、限流、请求日志或真实 Sub2API/OpenAI 返回结果，只增强反代内核的可证明性。

### 18.217 Dashboard 关键巡检保留反代契约 metrics

#### 背景

18.216 已经让完整 `可用性巡检` 中的 `openAiProxyContract.metrics` 暴露核心 `/v1` 样例路径、Sub2API path/query 透传和 base 尾斜杠规整证据。但 `GET /api/admin/dashboard` 的关键巡检预览此前只保留 issue/sample 摘要；`openAiProxyContract` 当前为 `ok` 时没有 issue，也就无法在首页首屏展示这些正向契约证据。管理员需要进入完整巡检详情才能确认反代内核覆盖是否完整。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth.criticalChecks[]` 新增 `metrics` 预览。
- 预览字段使用白名单，只保留标量字段，忽略嵌套对象。
- 白名单覆盖：
  - 管理员能力覆盖计数。
  - OpenAI/Codex 反代契约核心字段。
  - OpenAI/Codex 反代运行态关键计数。
- `openAiProxyContract` 首页关键巡检现在可直接展示：
  - `corePathSamples`
  - `routesCorePathSamples`
  - `preservesRawPathAndQuery`
  - `normalizesSub2BaseTrailingSlash`
...
- API 单元测试锁定 ok 状态的 `openAiProxyContract` 会保留上述 metrics，且嵌套对象不会进入 dashboard 预览。

#### 价值

- 管理员在后台首屏就能看到“反代契约正常”背后的具体证据，而不是只看到一句 summary。
- 当前真实 `/v1/responses` 上游阻断存在时，管理员可以区分“本地反代契约 ok”和“Sub2/OpenAI 凭据 error”，降低误判本地代理代码故障的概率。
- 该能力只扩展 dashboard 摘要，不改变系统健康状态、关键巡检排序、Sub2API 调用或真实 OpenAI/Codex 请求。

### 18.218 Dashboard 稳定展示管理员入口覆盖

#### 背景

系统已经通过 `adminCapabilities` 和 `adminSurfaceCoverage` 证明 API 能力矩阵与 Admin 前端入口覆盖了用户、共享资源、余额、售出和 OpenAI/Codex 反代核心范围。但生产 dashboard 的 `criticalChecks` 只保留最多 8 个关键巡检；当当前真实风险集中在 Sub2/OpenAI、资源凭据、共享资源、商品、支付和反代运行态时，`adminCapabilities` 与 `adminSurfaceCoverage` 这两个 ok 检查会被挤出首页。这样“完整管理员入口”虽然在完整巡检和能力页中被证明，但首页首屏缺少稳定摘要。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth` 新增 `adminEntryCoverage`。
- 该字段独立于 `criticalChecks` top 8 切片，始终从最近巡检快照中提取：
  - `adminCapabilities` 的 API 能力覆盖摘要。
  - `adminSurfaceCoverage` 的 Admin 前端入口覆盖摘要。
- `adminEntryCoverage` 返回：
  - `ok`
  - `summary`
  - `api.status`
  - `api.metrics`
  - `frontend.status`
  - `frontend.metrics`
- Admin 首页系统状态表新增“管理员入口”行，展示 API 与前端入口覆盖摘要。
- API 单元测试覆盖：即使 `criticalChecks` 前 8 项已经被其他风险和反代运行态占满，`adminEntryCoverage` 仍会返回完整 API/前端入口覆盖结果。

#### 价值

- 管理员在首页就能稳定确认用户、共享、余额、售出和 OpenAI/Codex 反代的管理入口仍然完整。
- 当前真实 `/v1/responses` 仍被外部 token 阻断时，首页可以同时呈现“反代上游不可用”和“管理员维修入口完整”，维修路径更清楚。
- 该能力只扩展 dashboard 摘要，不改变能力矩阵、前端导航、系统健康判定、关键巡检排序或任何业务写入。

### 18.219 Dashboard 稳定展示 Sub2/OpenAI 上游阻断

#### 背景

当前生产可用性复查已经能证明本地 OpenAI/Codex 反代契约、管理员入口覆盖和关键运维视图均存在。但真实端到端 `/v1/responses` 仍被 Sub2/OpenAI 上游账号不可用阻断。`criticalChecks` 虽然会优先展示 `sub2`、`localProxySmoke` 和 `resourceCredentials`，但它仍然是最多 8 项的切片，而且同级错误中可能先展示只有阻断码、缺少维修动作的检查项。管理员需要一个稳定、可操作的首页摘要，直接告诉他当前上游阻断来自哪里，以及下一步应打开哪个维修入口。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth` 新增 `upstreamBlocker`。
- `upstreamBlocker` 从最近巡检快照的以下检查项中提炼：
  - `sub2`
  - `localProxySmoke`
  - `resourceCredentials`
  - `resources`
...
- 选择策略：
  - 仅选择 `warning` 或 `error`。
  - 优先选择更高严重级别。
  - 同级别下优先选择携带 `actionHint`、`repairAction`、`sub2AccountId`、`resourceId` 或 `resourceList` 的可操作详情。
  - 再按既有 dashboard 健康检查优先级排序。
- 返回字段包括：
  - `blocked`
  - `status`
  - `checkId`
  ...
  - `resourceScope`
  - `check`
- Admin 首页系统状态表新增“上游阻断”行。
- Admin 首页在关键巡检列表前展示 `upstreamBlocker` 摘要，并复用原有 `dashboardHealthCheckTarget()` 跳转逻辑打开 `反代状态`、`共享资源` 或完整巡检。
- API 单元测试覆盖：当 `sub2` 错误只有阻断码而 `resourceCredentials` 带有资源修复建议时，`upstreamBlocker` 会选择可操作的 `resourceCredentials`。

#### 价值

- 管理员进入首页即可判断真实不可用点是否来自 Sub2/OpenAI 上游凭据，而不必先进入完整巡检详情。
- 当本地反代契约为 ok、真实 `/v1/responses` 仍失败时，首页能把“代码路径可用”和“外部凭据阻断”拆开呈现。
- 该能力不触发新的 Sub2API 调用、不写入资源或凭据、不改变系统健康聚合和关键巡检切片，只增强首屏可用性诊断和维修入口明确性。

### 18.220 Sub2API 上游请求头透传契约

#### 背景

18.216 已经把 Sub2API 上游 URL 拼接、核心 `/v1` 路径覆盖和 query 透传做成可测试契约。但“完整 OpenAI/Codex 反代内核”还需要证明请求头侧也满足代理要求：本地售出 Key 的鉴权不能泄漏调用方原始 Authorization，hop-by-hop headers 不能透传到上游，同时 OpenAI/Codex SDK 常用的 `content-type`、`openai-beta`、trace header 和 request id 诊断上下文需要保留或补齐。此前这些行为存在于路由内部，缺少可复用 helper 和健康巡检证据。

#### 新增能力

- 新增 `buildOpenAiProxyUpstreamHeaders()`，`registerOpenAiProxyRoutes()` 与单元测试复用同一套上游请求头构造逻辑。
- 新增 `openAiProxyHopByHopHeaderNames` 与 `isOpenAiProxyHopByHopHeader()`，请求头转发与响应头回传共用同一份 hop-by-hop 判断。
- 上游请求头构造规则：
  - 剥离本地 `authorization`。
  ...
  - 保留 `content-type`、`openai-beta`、自定义 trace header 等 end-to-end headers。
...
- 当样例请求头无法证明上述规则时，`openAiProxyContract` 返回 `upstream_header_forwarding_incomplete`。
- `GET /api/admin/dashboard` 的 `openAiProxyContract.metrics` 白名单新增 `forwardsUpstreamHeaders`，首页可直接展示该证据。
- API 单元测试覆盖本地 Authorization 剥离、售出 Key 注入、hop-by-hop 剥离、诊断头保留与 Dashboard metrics 预览。

#### 价值

- 本地反代内核现在从“路径/query 转发正确”扩展到“请求头转发正确”，更接近完整 OpenAI/Codex 代理验收。
- 管理员在首页就能看到 `forwardsUpstreamHeaders=true`，确认当前不可用不是因为本地代理把错误 Authorization 或压缩头传给 Sub2API。
- 该能力不改变余额、租赁、限流、实际 Sub2API 调用结果或凭据状态，只把已存在的请求头转发规则抽成可测试、可巡检的契约。

### 18.221 Dashboard 上游阻断 smoke 证据新鲜度

#### 背景

生产复查显示，`latestSystemHealth` 快照本身未过期，但最近一次 `/v1/responses` 端到端 smoke 证据已经超过新鲜度阈值。`criticalChecks` 中的 `localProxySmoke` 会展示该信息，但首页稳定的 `upstreamBlocker` 摘要仍主要展示 Sub2/OpenAI 上游阻断和修复动作。管理员从首屏判断可用性时，需要同时知道“当前阻断是什么”和“支撑该判断的 smoke 证据是否已经过期”。
...
- `GET /api/admin/dashboard` 的 `latestSystemHealth.upstreamBlocker` 新增 smoke 证据字段：
  ...
  - `evidenceStaleAt`
- 字段来源为 `upstreamBlocker.check.primaryIssue` 或 `primarySample` 中已经存在的 smoke 证据，不新增外部请求。
- Admin 首页“上游阻断”表格行和阻断卡片会展示 smoke 路径、模型、证据状态、年龄、HTTP 状态和代理错误码。
- API 单元测试覆盖：`upstreamBlocker` 会直接返回 stale smoke 证据字段，而不只把这些字段藏在内层 `check.primaryIssue` 中。

#### 价值

- 管理员在首页即可看出当前 `/v1/responses` 阻断判断是否依赖已过期 smoke 证据。
- 当修复了 OpenAI refresh token 或 Sub2 账号后，管理员会更明确地知道需要重新运行端到端 smoke 来刷新可用性证据。
- 该能力只增强 Dashboard 摘要，不改变健康状态聚合、关键巡检排序、Sub2API 调用、凭据写入或真实 OpenAI/Codex 请求。

### 18.222 Dashboard 上游阻断资源凭据可用性

#### 背景

首页现在可以稳定展示 Sub2/OpenAI 上游阻断、修复动作和 smoke 证据新鲜度。但管理员要判断下一步是“直接应用已有资源凭据”还是“必须新建/粘贴 OpenAI refresh token”，仍需要展开 `resourceCredentials` 关键巡检。为了让首屏阻断摘要直接支撑运维决策，需要把资源凭据 readiness 摘要并入 `upstreamBlocker`。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth.upstreamBlocker` 新增 `credentialReadiness`。
- `credentialReadiness` 从最近巡检快照中的 `resourceCredentials` 检查提炼：
  - `status`
  - `summary`
  - `issueCount`
  - `sampleCount`
  - `metrics`
- Dashboard metrics 白名单新增资源凭据 readiness 计数字段：
  ...
  - `activeMissingSub2Account`
  - `inactiveOpenAiRefreshTokens`
- Admin 首页“上游阻断”表格行和阻断卡片会展示凭据状态、summary、active token 数、可应用候选数和缺 Sub2 账号数。
- API 单元测试覆盖 `upstreamBlocker.credentialReadiness` 会保留资源凭据 summary 与核心计数。

#### 价值

- 管理员在首页即可判断当前恢复路径：已有候选凭据时去应用，没有候选时先新建/粘贴 OpenAI refresh token。
- 当前生产真实阻断为 `openai_group_has_no_active_accounts` 且没有可应用资源凭据时，首屏不再只提示上游账号不可用，还能解释为什么不能直接自动恢复。
- 该能力只复用已有巡检快照，不新增 Sub2API 调用、凭据解密、数据库写入或真实 OpenAI/Codex 请求。

### 18.223 Dashboard 上游阻断 Sub2 账号诊断

#### 背景

生产环境当前 `/v1/responses` 阻断链路已经能够在首页显示：Sub2/OpenAI 上游无 active 账号、smoke 证据已过期、生产无可应用资源凭据。但具体账号层面的 `accountStatus`、`credentialsStatus`、`schedulable` 和 OpenAI `token_invalidated` 错误仍主要藏在完整巡检 detail 中。管理员首屏仍可能不知道要处理哪个 Sub2 账号。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth.upstreamBlocker` 顶层新增账号诊断字段：
  - `sub2AccountName`
  - `accountStatus`
  - `credentialsStatus`
  - `schedulable`
  ...
  - `accountMessage`
  - `accountUpdatedAt`
- 字段来源仍为 `upstreamBlocker.check.primaryIssue` 或 `primarySample` 中的已脱敏巡检详情。
- Admin 首页“上游阻断”表格行和阻断卡片会展示账号诊断摘要：Sub2 账号、账号状态、凭据状态、是否可调度、错误摘要和最近更新时间。
- 前端会把长 JSON 错误压缩成一行可扫描文本，例如 `HTTP 401 / token_invalidated / Your authentication token has been invalidated.`。
- API 单元测试覆盖账号诊断字段会从巡检 detail 保留到 Dashboard preview。

#### 价值

- 管理员在首页即可把 `openai_group_has_no_active_accounts` 进一步落到具体账号和认证错误，不需要先展开完整巡检。
- 当 OpenAI refresh token 被失效时，首页会直接显示 `token_invalidated`，下一步操作可以更明确地进入反代状态页应用新 token。
- 该能力不读取或展示 refresh token 明文，不新增外部调用，只增强管理员入口的信息密度。

### 18.224 Dashboard 完整 OpenAI/Codex 反代契约指标保留

#### 背景

`openAiProxyContract` 巡检已经能够证明本地 OpenAI/Codex 反代是否完整：包括 `/v1` 路径覆盖、Responses API、Chat Completions、文件/上传/批处理样例、原始 body 透传、Sub2API header 转发、stream idle timeout、CORS 暴露、OpenAI 兼容错误类型和 Redis 限流运行态。但 Dashboard preview 为了控制 payload，只白名单了一部分指标，管理员在首页或关键巡检摘要里看不到完整证据链。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth.criticalChecks[].metrics` 对 `openAiProxyContract` 保留更多契约字段：
  - request id、upstream request id、rate limit headers、proxy request lookup headers。
  - CORS 是否暴露 request id、upstream request id 与 OpenAI rate limit headers。
  - `requestBodyMode`、`bodyLimitBytes`、`upstreamTimeoutMs`、`streamIdleTimeoutMs`。
  - multipart、form-urlencoded、URL model 提取能力。
  - forwarded host/proto、客户端断开 abort、stream 完成/错误日志、stream idle timeout。
  - `insufficient_quota`、`rate_limit_error`、`api_error` 与 `error.param` 兼容字段。
- 同一白名单也补齐 OpenAI 反代运行态中的 Redis rate window 配置、token event 和清理周期指标。
- API 单元测试覆盖这些完整反代契约字段不会被 Dashboard preview 过滤。

#### 价值

- 管理员在后台即可确认反代内核是否真的具备完整 OpenAI/Codex 兼容能力，而不是只看到粗略的 `ok`。
- 当未来 `/v1/responses`、stream、multipart upload、rate limit 或浏览器端诊断出现问题时，Dashboard 可以直接给出契约级证据。
- 该能力只增强可观测性，不改变真实反代流量、Sub2API 调用、凭据处理或数据库写入。

### 18.225 Dashboard 稳定展示 Codex 交付阻断

#### 背景

当前首页已经有 `upstreamBlocker` 稳定展示 Sub2/OpenAI 上游阻断。但生产剩余问题还包括 `productCatalog` warning：Codex 商品可购买，却没有 ready production Codex shared resource。该问题关系到“售出情况”和“共享资源”闭环，不能只依赖 `criticalChecks` 的切片展示，否则管理员需要在商品、共享资源、售出交付多个页面之间切换确认影响对象。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth` 新增 `deliveryBlocker`。
- `deliveryBlocker` 从最近巡检快照中的以下检查项提炼：
  - `salesDelivery`
  - `productCatalog`
  - `resources`
- 同级别下优先选择携带商品、价格、资源、供给方或维修动作的可操作详情。
- `deliveryBlocker` 顶层保留：
  - `productId`
  - `productName`
  - `priceId`
  - `supplierEmail`
  ...
  - `repairAction`
  - `actionHint`
- Admin 首页系统状态表新增“交付阻断”行；上游阻断卡片后新增可点击的交付阻断卡片。
- API 单元测试覆盖：当 `productCatalog` 指出 active Codex 商品无 ready 交付资源时，Dashboard 顶层会保留商品、价格、资源、供给方和目标 Sub2 账号上下文。

#### 价值

- 管理员在首页即可判断当前销售/交付风险具体影响哪个 Codex 商品和价格档位。
- 从首页可以直接进入商品或共享资源修复入口，减少“看到上游问题但不知道售卖风险对象”的断点。
- 该能力只增强 Dashboard 摘要，不改变真实下单、交付、Sub2API 调用、凭据处理或数据库写入。

### 18.226 Dashboard 核心管理摘要与余额风险筛选

#### 背景

管理员入口已经覆盖用户管理、共享资源、余额管理和售出情况，也具备能力矩阵和系统巡检证明。但首页原有经营卡片主要是总量：用户数、有效租赁、在线资源、售出金额、可用余额、累计充值、累计消费、供给收益。管理员仍需要分别进入用户、余额、订单、租赁、共享资源列表，才能判断各状态下有多少待处理对象，尤其是负余额和冻结余额钱包缺少直接筛选入口。

#### 新增能力

- `GET /api/admin/dashboard` 新增 `managementOverview`。
- `managementOverview.users` 返回用户总数和 `active`、`disabled`、`banned` 状态分布。
- `managementOverview.wallets` 返回钱包总数和运营风险计数：
  - `negative`：可用余额小于 0。
  - `frozen`：冻结余额大于 0。
  - `available`：可用余额大于 0。
  - `spent`：累计消费大于 0。
- `managementOverview.sales` 返回所有订单状态分布，并保留每个状态下的 `totalAmount` 与 `paidAmount`。
- `managementOverview.rentals` 返回租赁通道状态分布。
- `managementOverview.sharing` 返回共享资源状态分布。
- Admin 首页新增“核心管理摘要”面板，按用户、余额、售出订单、租赁通道、共享资源分组展示状态计数。
- 每个非零状态计数都可以直接打开对应管理列表，并自动带上状态筛选。
- 余额列表新增 `negative`、`frozen`、`available`、`spent` 运营筛选。
- API 单元测试覆盖状态摘要排序、缺失状态补 0、未知状态保留和钱包风险计数归一化。

#### 价值

- 管理员首屏即可判断用户、共享、余额、售出四个核心范围的待处理状态，不再只看到总量。
- 负余额和冻结余额钱包可以直接筛选出来，补齐“余额情况”管理的可操作入口。
- 售出订单、租赁通道和共享资源的状态分布与列表筛选联动，减少巡检后手动切换页面和重复输入筛选条件。
- 该能力只新增 Dashboard 只读聚合和列表筛选条件，不改变下单、扣费、退款、资源交付或 Sub2API 调用链路。

### 18.227 Dashboard 交付阻断卡片直达共享资源修复

#### 背景

18.225 已经让 `latestSystemHealth.deliveryBlocker` 稳定进入管理员首页，能够显示 Codex 商品交付阻断、商品/价格、资源类型、供给方和目标 Sub2 账号。但 `productCatalog` 型交付阻断的主操作会优先打开受影响商品；完整关键巡检列表才会额外显示“打开共享资源”。这意味着管理员从首屏交付阻断卡片仍可能多点一次，才能进入真正的 ready production Codex shared resource 修复入口。

#### 新增能力

- Admin 首页 `deliveryBlocker` 卡片在原有主操作之外，新增共享资源修复直达按钮。
- 当 `deliveryBlocker.check.id=productCatalog` 且问题携带资源修复上下文时，展示“打开共享资源”。
- 资源修复上下文判断支持：
  - `resourceList=true`
  - `supplierEmail`
  - `resourceType`
  ...
  - `resourceScope`
  - `sub2AccountId`
- 该按钮复用既有 `dashboardHealthResourceFilter()`，继续传递商品、价格、供给方、资源类型、资源状态、生产范围、Sub2 账号、账号状态、凭据状态和 smoke 失败证据。
- 前端纯函数测试覆盖：
  - `productCatalog + resourceList=true` 可打开共享资源。
  - `productCatalog + resourceType/resourceScope/sub2AccountId` 可打开共享资源。
  - 非 `productCatalog` 或缺少资源上下文时不误显示。

#### 价值

- 管理员在首页即可同时获得“定位商品”和“维修共享资源”两条动作，不必进入完整巡检页寻找第二个按钮。
- Codex 商品可购买但无 ready production shared resource 时，恢复路径更短：从首页直接进入共享资源创建/凭据应用/smoke 验收入口。
- 该能力只增强 Admin 前端跳转和可测试判断，不改变 API、数据库、真实下单、Sub2API 或 OpenAI/Codex 反代请求。

### 18.228 反代请求日志支持运维失败类别筛选

#### 背景

管理员已经可以在 `反代请求` 页面搜索本地 request id、上游 request id、用户、租赁、API Key、模型、路径和错误码，也可以按具体 HTTP 状态码筛选。但真实排查 OpenAI/Codex 反代可用性时，管理员更常问的是“所有失败请求在哪里”“哪些是上游错误”“哪些是本地准入拒绝”“哪些是 Redis/上游可用性问题”“哪些是流式异常”。只靠 `400/401/500/503` 逐个筛选会让排查变慢，也不利于区分本地代理问题和 Sub2/OpenAI 上游问题。

#### 新增能力

- `GET /api/admin/proxy-requests` 的 `status` 参数继续支持精确 HTTP 状态码，例如 `503`。
- `status` 新增运维失败类别：
  - `failed`：`statusCode >= 400` 或存在 `errorCode`。
  - `client_error`：`400 <= statusCode < 500`。
  - `server_error`：`statusCode >= 500`。
  - `upstream_error`：`upstreamStatusCode >= 400` 或 `errorCode` 以 `upstream_` 开头。
  - `local_rejection`：本地准入拒绝，例如缺 API Key、余额不足、租赁不可用、RPM/TPM/并发限制。
  - `local_availability`：本地代理或上游可用性错误，例如 limiter 不可用、上游超时、上游不可达。
  - `stream_error`：上游 stream 异常、关闭或 idle timeout。
- 后端筛选条件改为 `AND` 组合，确保类别筛选、错误码 contains 和关键字搜索可以同时生效。
- Admin `反代请求` 页面状态下拉新增上述类别，同时保留常用精确状态码。
- API 单元测试覆盖每个类别生成的 Prisma 查询条件。

#### 价值

- 管理员可以一键筛出“所有失败”“上游失败”“本地准入拒绝”“流式异常”等关键集合。
- 当 `/v1/responses` 或 Codex stream 失败时，排查路径可以直接落到反代请求日志，不必凭多个状态码手动拼凑。
- 该能力只增强管理员查询和日志可观测性，不改变真实 OpenAI/Codex 反代转发、限流、扣费或 Sub2API 调用链路。

### 18.229 Dashboard 阻断卡片直达反代失败日志

#### 背景

18.228 已经让 `反代请求` 页面可以按 `failed`、`upstream_error`、`local_rejection`、`local_availability` 和 `stream_error` 等运维类别筛选日志。但管理员在首页看到 `upstreamBlocker` 或 `deliveryBlocker` 时，仍需要先进入反代请求列表再手动选择筛选项，无法从首屏阻断摘要直接落到对应失败日志集合。

#### 新增能力

- `GET /api/admin/dashboard` 的 `latestSystemHealth.upstreamBlocker` 与 `deliveryBlocker` 新增：
  - `proxyRequestFilterStatus`
  - `proxyRequestFilterLookup`
- 后端会优先保留精确定位字段：`proxyRequestLookup`、`requestId`、`proxyRequestLogId`、`upstreamRequestId`。
- 当没有精确定位字段时，后端会根据 `proxyRequestErrorCode`、`proxyRequestStatusCode`、`upstreamStatusCode` 和检查项类型推导失败类别：
  - 上游 HTTP 或 `upstream_*` 错误映射到 `upstream_error`。
  - 本地准入拒绝映射到 `local_rejection`。
  - limiter、上游超时或不可达映射到 `local_availability`。
  - stream 错误映射到 `stream_error`。
  - 4xx / 5xx 分别映射到 `client_error` / `server_error`。
  - Sub2/OpenAI 上游阻断在缺少具体日志字段时默认给出 `upstream_error` 入口。
- Admin 首页 `upstreamBlocker` 与 `deliveryBlocker` 卡片新增“打开失败日志”按钮。
- 按钮优先使用 `proxyRequestFilterLookup` 做精确搜索；没有 lookup 时使用 `proxyRequestFilterStatus` 打开对应失败类别列表。
- API 单元测试覆盖 Dashboard 失败类别推导；Admin 单元测试覆盖 lookup 优先于 status fallback。

#### 价值

- 管理员从首页即可完成“看到阻断 -> 打开相关失败日志 -> 查看用户/租赁/售出/用量关联”的排障闭环。
- 当前生产 Sub2/OpenAI 无 active 账号时，首页上游阻断可直接打开 `upstream_error` 反代日志集合，减少手动切换筛选项。
- 该能力只新增 Dashboard 可观测字段和 Admin 跳转，不改变真实 OpenAI/Codex 反代、Sub2API 调用、计费、限流或凭据处理。

### 18.230 反代请求日志行直达 Sub2/OpenAI 修复上下文

#### 背景

18.229 已经让管理员从首页阻断卡片直达相关反代失败日志。线上复查显示，命中的日志通常是 `/v1/responses` 的 `503 upstream_http_503`，但日志行此前只提供用户、订单、租赁、API Key、商品和用量入口。管理员还需要回到首页或系统巡检页才能带着该失败证据进入 Sub2/OpenAI 修复面板。
...
- 点击后打开 `反代状态` 页面，并带入修复上下文：
  ...
  - `model`
  - `repairAction=apply_openai_refresh_token_to_sub2_account`
  - `/v1/responses` 失败时标记 `responsesOk=false`
- 本地准入拒绝，例如 `missing_api_key`、无效 Key、余额不足或租赁不可用，不显示该 Sub2 修复动作，避免误导管理员把用户侧或本地准入问题当作上游账号问题。
- Admin 单元测试覆盖 `upstream_http_503` 会生成完整修复上下文，`missing_api_key` 不生成 Sub2 修复上下文。

#### 价值

- 管理员从首页进入失败日志后，可以在同一行继续打开 Sub2/OpenAI 修复入口，形成“首页阻断 -> 失败日志 -> 上游账号/凭据修复”的连续路径。
- `/v1/responses` 失败证据会跟随进入修复面板，管理员应用新 refresh token 后更容易立即运行 smoke 验收。
- 该能力只增强 Admin 前端跳转和上下文预填，不读取或展示 refresh token 明文，不改变真实反代转发、限流、计费、Sub2API 调用或数据库写入。

### 18.231 反代失败日志携带租赁 Sub2 交付上下文

#### 背景

18.230 已经让管理员能从一条上游失败的 `反代请求` 日志直接打开 `反代状态` 修复面板。但线上排障时，管理员还需要确认这次失败对应哪条已售租赁通道、哪个 Sub2 用户、哪个 Sub2 Key，以及用户实际拿到的 endpoint。此前 `GET /api/admin/proxy-requests` 只返回租赁的订单、商品、资源类型和状态，不返回这些交付字段，导致管理员仍需要再打开租赁详情二次确认。

#### 新增能力

- `GET /api/admin/proxy-requests` 的租赁关联返回新增：
  - `rental.sub2UserId`
  - `rental.sub2KeyId`
  - `rental.endpointUrl`
- 反代请求搜索新增覆盖租赁交付字段：
  ...
  - Sub2 Key ID
  - endpoint URL
- Admin `反代请求` 列表的“租赁 / Key”列会展示 Sub2 Key 或 endpoint 作为低敏交付线索。
- Admin `反代请求` CSV 导出新增 `sub2UserId`、`sub2KeyId`、`endpointUrl` 列。
- 从失败日志点击“反代状态”时，修复上下文新增：
  - `rentalId`
  - `resourceType`
  - `sub2UserId`
  - `sub2KeyId`
  - `endpointUrl`
- `反代状态 -> 修复定位` 新增展示：
  - `租赁通道`：租赁 ID、资源类型和 endpoint。
  - `Sub2 Key`：Sub2 用户 ID 与 Sub2 Key ID。

#### 价值

- 管理员从首页阻断进入失败日志后，可以在同一条路径中看到“请求失败证据 + 已售租赁通道 + Sub2 交付对象”。
- 排查 `/v1/responses` 上游 503、Sub2 账号失效、租赁 Key 异常或 endpoint 配置问题时，不再必须先跳到租赁详情复制字段。
- 该能力只增加管理员侧低敏排障字段和搜索/导出能力，不保存 API Key 明文，不保存请求体/响应体，不改变真实 OpenAI/Codex 反代、限流、计费或 Sub2API 写入链路。

### 18.232 租赁持久化共享资源交付来源

#### 背景

Codex 商品开通前已经会检查是否存在可交付的生产共享资源，并在订单状态历史中记录本次选择的 `supplierResourceId`。但租赁本体此前没有持久化该字段，导致管理员从已售租赁、反代请求日志或 API Key 轮换继续追踪到“是哪一个共享资源、哪个供给方、哪个 Sub2 OpenAI 账号交付了这条租赁”时，需要依赖历史审计或用量记录间接推断。

#### 新增能力

- `Rental` 新增可空字段 `supplierResourceId`，关联到 `SupplierResource`。
- 数据库迁移会尽力从 `OrderStatusHistory.meta.supplierResourceId` 回填历史租赁。
- 用户下单开通 Codex 租赁时，租赁创建阶段写入当前 `deliveryReadiness.resource.id`。
- Sub2 Key 创建完成后，租赁交付字段继续保持同一 `supplierResourceId`。
- 管理员重试失败订单开通时，会在重试开始和完成阶段写入本次选中的共享资源。
- 租赁 API Key 轮换时，会重新确认可交付共享资源并把本次资源 ID 保存到租赁。
- Admin 租赁列表、租赁详情、订单详情中的租赁交付表新增共享资源展示与资源直达操作。
- Admin 租赁搜索新增覆盖共享资源 ID、Sub2 Account ID 和供给方邮箱。
- Admin 反代请求日志返回并搜索租赁关联的共享资源、Sub2 Account ID 和供给方邮箱。
- 上游失败日志打开 `反代状态` 时，修复上下文优先带入共享资源 ID、Sub2 Account ID、资源状态和供给方邮箱。
- 租赁 CSV 与反代请求 CSV 导出新增共享资源、Sub2 Account ID 和供给方邮箱列。

#### 价值

- “售出订单 -> 租赁 -> 反代失败请求 -> 共享资源 -> Sub2/OpenAI 账号 -> 供给方”形成稳定归因链路。
- 管理员处理 OpenAI/Codex 可用性故障时，可以直接确认失败请求对应的供应资源和上游账号，不必在订单历史、用量、结算和资源页之间手工拼接。
- 该能力只持久化资源归因 ID 和低敏运维字段，不保存 API Key 明文、请求体、响应体或 refresh token 明文，不改变真实反代转发、限流或扣费顺序。

### 18.233 售出交付巡检覆盖租赁共享资源归因
#### 背景

18.232 已经把 Codex 租赁交付时选中的 `supplierResourceId` 持久化到 `Rental`，但全局 `salesDelivery` 巡检仍只检查 endpoint、Sub2 Key 和本地 API Key。历史数据、迁移缺口或人工修复不完整时，租赁可能已经 active，却无法从售出交付视角确认它实际由哪个生产共享资源、哪个供应方和哪个 Sub2/OpenAI 账号支撑。

#### 新增能力

- `GET /api/admin/system-health` 的 `salesDelivery` 检查在扫描 Codex 租赁时同步读取 `supplierResourceId` 与关联 `SupplierResource`。
- 当 Codex 租赁缺少共享资源归因时，新增 issue：
  ...
  - `resourceStatus=online`
  - `resourceScope=production`
- 当 Codex 租赁已关联共享资源，但资源不为 `online` 或缺少 `sub2AccountId` 时，新增 issue：
  - `type=rental_supplier_resource_not_ready`
  - `repairAction=repair_supplier_resource_delivery_readiness`
  - 返回 `resourceId`、`resourceStatus`、`supplierEmail` 与 `sub2AccountId` 作为修复定位。
- `salesDelivery.metrics.counters` 新增：
  - `rentalsMissingSupplierResource`
  - `rentalsWithUnavailableSupplierResource`
- `GET /api/admin/dashboard` 的健康预览白名单新增 `orderId`、`rentalId`、`userId` 和 `userEmail`，`deliveryBlocker` 也直接返回这些字段。
- Admin 首页中，当 `salesDelivery` 问题携带共享资源上下文时，主按钮优先打开 `共享资源`，并保留资源类型、生产范围、目标状态和订单/租赁/用户定位摘要。
- 前端共享资源修复入口的可测试判断从 `productCatalog` 扩展到 `salesDelivery`。

#### 价值

- 售出交付巡检不再只证明“用户拿到了 endpoint/key”，也能证明“这条 Codex 租赁有可追溯、可维修的生产共享资源来源”。
- 迁移后仍缺少历史资源归因的租赁会在全局可用性巡检中显性暴露，避免隐藏在订单详情或代理日志里。
- 管理员从首页交付阻断卡片可以直接进入共享资源修复入口，补齐或恢复生产 Codex 资源，再回到订单/租赁侧复查交付闭环。
- 该能力只增加低敏归因巡检和修复入口，不保存 refresh token 明文、API Key 明文、请求体或响应体，不改变真实 OpenAI/Codex 反代、限流、扣费或 Sub2API 调用顺序。

### 18.234 管理员租赁共享资源归因修复

#### 背景

18.233 已经让 `salesDelivery` 能发现 `rental_missing_supplier_resource` 与 `rental_supplier_resource_not_ready`，但修复路径仍偏间接：管理员需要先进入共享资源页确认生产资源，再回到租赁或订单侧人工补齐归因。历史迁移、人工开通或异常重试留下的 Codex 租赁，如果只是缺少 `supplierResourceId`，需要一个受控的后台写入口把“这条已售租赁由哪个共享资源交付”明确落库。

#### 新增能力

- 后端新增 `PATCH /api/admin/rentals/:id/supplier-resource`，仅 `admin` 可调用。
- 请求体支持：
  - `supplierResourceId`：共享资源 ID，传 `null` 表示清空归因。
  - `requireReady`：默认 `true`，Codex 租赁会要求目标资源是 online、生产可交付、存在 Sub2 Account，并具备 active OpenAI refresh token 凭据。
  - `note`：可选维修备注，仅进入审计上下文。
- 接口会校验共享资源存在且 `resourceType` 与租赁一致；类型不一致返回 `rental_supplier_resource_type_mismatch`。
- Codex 默认 ready 校验失败时返回 `rental_supplier_resource_not_ready`，并返回资源状态、Sub2 Account、凭据类型和凭据状态，帮助管理员先修资源再绑定。
- 清空归因使用 `supplierResourceId=null` 与 `requireReady=false`，用于撤销错误历史归因。
- 写入成功后记录审计动作 `admin.rental.supplier_resource`，保存变更前后的资源 ID、资源类型、状态、Sub2 Account、供应方邮箱和操作备注。
- Admin 租赁列表与租赁详情新增共享资源归因表单，可输入共享资源 ID、选择是否要求 ready，并支持清空归因。
- 能力矩阵新增 `rentals.assignSupplierResource`，管理员入口覆盖从 65 个操作扩展到 66 个操作。

#### 价值

- `salesDelivery` 发现租赁缺少共享资源归因后，管理员可以直接在租赁管理页完成补齐，不再只停留在“打开共享资源”。
- 对生产 Codex 租赁，默认要求绑定可交付资源，避免把 disabled、缺 Sub2 Account 或缺 active OpenAI refresh token 的资源误标为交付来源。
- 历史修复仍保留强制归因通道，管理员可以在明确风险时关闭 ready 校验，但所有变更都会留审计。
- 该能力只写入租赁的低敏资源归因字段，不保存 refresh token 明文、API Key 明文、请求体或响应体，不改变真实 OpenAI/Codex 反代、限流、扣费或 Sub2API 调用顺序。

### 18.235 Codex 商品上架交付就绪闸门

#### 背景

18.188 到 18.234 已经让下单、公开商品目录、管理员商品页、系统健康和售出交付巡检都能识别“Codex 商品可购买但没有 ready production Codex shared resource”的风险。线上临时下线 `Codex 标准租赁` 后，`productCatalog` 从 warning 变为 ok，但如果管理员后续误把商品重新上架，系统又会回到“商品 active、价格 active、但真实 `/v1/responses` 不可交付”的状态。因此商品上架本身也需要复用同一套 ready 资源闸门。

#### 新增能力

- `POST /api/admin/products` 创建 `resourceType=codex` 且 `status=active` 的商品时，默认要求存在 ready production Codex shared resource。
- `PATCH /api/admin/products/:id` 把 Codex 商品更新为 `active` 时，默认要求存在 ready production Codex shared resource。
- `POST /api/admin/products/:id/prices` 在 active Codex 商品下创建 active 且可购买的价格时，默认要求存在 ready production Codex shared resource。
- `PATCH /api/admin/product-prices/:id` 把 active Codex 商品的可购买价格更新为 active 时，默认要求存在 ready production Codex shared resource。
- 默认校验失败返回 `codex_resource_not_ready_for_product_activation` 或 `codex_resource_not_ready_for_price_activation`，并携带 required status、credential type/status、ready 资源数量和商品/价格定位。
- 管理员确需做历史或特殊运营配置时，可以显式传入 `allowUnavailableDelivery=true` 覆盖该闸门；Admin 商品和价格表单新增 `override delivery` 勾选项。
- 单元测试覆盖默认阻止、非 Codex/非 active/已有 ready 资源不阻止，以及显式覆盖不阻止。

#### 价值

- 商品目录可交付性不再只靠事后健康告警发现，后台上架动作本身会默认拒绝不可交付的 Codex 商品。
- 当前生产临时下线策略被固化为程序约束，降低后续误上架导致买家看到不可开通套餐的风险。
- 显式覆盖仍保留给管理员，但需要在请求里明确声明，审计日志也会记录这次覆盖。
- 该能力不改变真实 OpenAI/Codex 反代、Sub2API 调用、计费或租赁交付逻辑，只加强商品配置进入公开目录前的准入。

### 18.236 前端入口巡检覆盖构建资产

#### 背景

管理员入口覆盖已经从 API 路由、前端导航、系统健康和部署运行态多个角度补齐。但此前 `frontendRuntime` 只检查 Web/Admin 入口是否返回 HTML。若部署目录切换后 `index.html` 可达、但 Vite 构建出的 `assets/index-*.js` 或 `assets/index-*.css` 丢失，健康页仍可能显示前端入口 ok，而用户和管理员实际看到的是空白应用。

#### 新增能力

- `frontendRuntime` 读取 Web/Admin 的 HTML 后，会解析：
  - `<script src="...">`
  - `<link rel="stylesheet" href="...">`
- 对解析出的 JavaScript 与 stylesheet 资产逐个发起请求。
- 资产需要返回 `2xx/3xx`，且 JavaScript / stylesheet MIME 需要符合预期。
- HTML 没有引用任何构建资产时返回 `frontend_assets_missing`。
- 资产不可达、状态码异常或 MIME 异常时返回 `frontend_asset_unreachable`、`frontend_asset_bad_status` 或 `frontend_asset_bad_content_type`。
- `frontendRuntime.metrics` 新增 `totalAssets`、`okAssets`、`failedAssets` 与 `endpointsWithoutAssets`。

#### 价值

- 系统健康可以证明 Web/Admin 不只是入口 HTML 可达，也能加载当前 release 的真实 JS/CSS 构建产物。
- 管理员入口完整性从“路由和导航存在”进一步延伸到“页面能被浏览器实际运行”。
- 该能力只增加内部健康探测，不改变前端服务、反代转发、Sub2API 调用或用户业务数据。

### 18.237 OpenAI v1 反代契约扩展到更多代表路径

#### 背景

18.216 已经把 `/v1`、Responses、Chat Completions、files、uploads 和 batches 纳入本地 OpenAI/Codex 反代契约样本。但“完整 OpenAI v1 反代”在实际运维中还会遇到 embeddings、Assistants、Threads/Runs、Vector Stores、Audio、Images 和 fine-tuning 等路径。虽然本地 `/v1/*` 通配路由理论上已经覆盖这些路径，系统健康仍需要把它们纳入机器可读证据，避免未来路由收窄时只剩 Responses smoke 能发现问题。

#### 新增能力

- `openAiProxyCorePathSamples` 新增代表路径：
  - `/v1/embeddings`
  - `/v1/assistants` 与 `/v1/assistants/:id`
  - `/v1/threads`、`/v1/threads/:id/messages`、`/v1/threads/:id/runs` 与 run step 路径
  - `/v1/vector_stores` 与 vector store file 路径
...
- 如果这些代表路径不再被 `/v1/*` 转发契约覆盖，系统健康会返回 `extended_openai_path_samples_not_routed`。
- Dashboard 健康预览白名单保留这些新增指标，管理员可以在首页关键巡检摘要中看到更完整的 OpenAI v1 覆盖证据。

#### 价值

- “基于 Sub2API 的完整 OpenAI/Codex 反代内核”从 Responses/Chat 重点路径扩展为更广泛的 OpenAI v1 代表面。
- 管理员能区分“本地通配代理契约覆盖完整”和“真实 Sub2/OpenAI 凭据不可用”这两类问题。
- 该能力只增强静态契约、健康指标、Dashboard 摘要和测试覆盖，不触发真实 OpenAI 请求，不改变 Sub2API 透传、鉴权、限流、计费或凭据处理。

### 18.238 Sub2 上游修复候选账号稳定排序

#### 背景

生产环境中 `sub2` 与 `localProxySmoke` 仍能稳定指出 `/v1/responses` 被真实 Sub2/OpenAI 上游阻断，但当 Sub2 返回多个非 active OpenAI 账号时，系统健康此前直接使用上游返回顺序生成修复候选。这样在存在“已配置凭据但 token/auth 失效”的账号、普通 error 账号和 disabled 账号混合出现时，首页 `upstreamBlocker` 与完整巡检可能把低优先级账号放到第一位，降低管理员直接应用 refresh token 或重新授权的效率。

#### 新增能力

- `sub2AccountHealthSamples` 在截取候选前先做稳定排序。
- 优先级从高到低为：
  - 错误信息或临时不可调度原因包含 auth、credential、token、invalidated、unauthorized、expired、revoked 的账号。
  - `status=error` 且 `credentialsStatus=configured(...)` 的账号。
  - 普通 `status=error` 账号。
  - 已配置凭据但非 active 的账号。
  - 其他非 active 账号。
  - active 但不可调度的账号。
- 同优先级候选按 `updatedAt` 新近程度倒序排列；时间相同再按账号 ID 稳定排序。
- `buildSub2UpstreamIssues` 继续复用第一条候选生成 `repairAction=apply_openai_refresh_token_to_sub2_account`、`sub2AccountId`、账号状态和错误摘要。
- 单元测试覆盖下划线形式的 `token_invalidated`、多个候选乱序输入以及 issue 修复入口仍指向最高优先级账号。

#### 价值

- 管理员在首页或系统健康页看到的 Sub2/OpenAI 修复入口更稳定地指向最可能需要重新授权或应用 refresh token 的账号。
- 当真实 `/v1/responses` 仍因上游凭据不可用返回 503 时，诊断不再依赖 Sub2 返回数组顺序。
- 该能力只调整只读健康巡检候选排序，不解密凭据、不写入 Sub2API、不改变真实 OpenAI/Codex 反代、限流、扣费或凭据应用流程。

### 18.239 Sub2 修复诊断字段空白归一化

#### 背景

线上复查显示，Sub2 状态接口有时会把 `tempUnschedulableReason`、`updatedAt` 或其他可选诊断字段返回为空字符串。Dashboard 预览层已经会把空字符串过滤掉，但完整 `GET /api/admin/system-health` payload 和共享资源/资源凭据修复 issue 仍可能保留 `""`。这会让管理员在完整巡检、共享资源创建默认值或二次集成里看到“字段存在但没有内容”的噪声，也不利于机器判断该字段是否真正提供了诊断含义。

#### 新增能力

- `sub2AccountHealthSamples` 对以下可选字段做统一归一化：
  - `credentialsStatus`
  - `rateLimitedAt`
  - `overloadUntil`
  ...
  - `updatedAt`
  - `errorMessage`
- 空白字符串会变为 `null`；非空字符串会 trim 后返回。
- 当 `errorMessage` 与 `tempUnschedulableReason` 都为空白时，账号样本会回落到默认可读 message，而不是返回空 message。
- `resourceCredentialRepairCandidateFields` 与 `resourceCredentialSub2AccountRepairSamples` 同步归一化账号名、状态、凭据状态、临时不可调度原因、group、message 和更新时间。
- 单元测试覆盖 Sub2 样本、资源凭据 issue 字段和资源凭据 sample 的空白字符串归一化。

#### 价值

- 完整系统健康 payload、Dashboard 预览和 Admin 操作页上下文在“空值”语义上保持一致。
- 管理员不会把空字符串误读为有效的 Sub2 不可调度原因。
- 该能力只清洗只读诊断字段，不改变真实 Sub2API 状态、不写入数据库、不解密或展示 refresh token，也不改变 OpenAI/Codex 反代转发链路。
### 18.240 Sub2/OpenAI 账号错误结构化诊断

#### 背景

系统已经具备基于 Sub2API 的 OpenAI/Codex 本地反代契约检查、真实 Sub2 上游状态检查、本地 `/v1/models` 与 `/v1/responses` smoke 证据，以及管理员系统健康入口。但生产复查中，Sub2 OpenAI 账号的真实错误此前主要藏在 `accountMessage` 长文本里，例如多行 `Authentication failed (401): {...}`。这会让 Dashboard、系统健康页和共享资源修复入口虽然能指向异常账号，却仍要求管理员人工解析 JSON 才能判断根因是 token invalidated、invalid api key、quota、rate limit 还是其他错误。

#### 新增能力

- Sub2/OpenAI 账号候选、资源凭据修复候选和 Sub2 上游 issue/sample 新增结构化错误字段：
  - `accountErrorStatusCode`
  - `accountErrorType`
  - `accountErrorCode`
  - `accountErrorMessage`
- 当 `accountMessage` 或临时不可调度原因包含 OpenAI JSON 错误体时，系统会解析：
  - HTTP 状态码，例如 `401`。
  - OpenAI error type，例如 `invalid_request_error`。
  - OpenAI error code，例如 `token_invalidated`。
  - OpenAI error message，例如 `Your authentication token has been invalidated. Please try signing in again.`
- 当文本没有 JSON 体但能识别常见错误码时，系统会提取 `token_invalidated`、`invalid_api_key`、`insufficient_quota`、`rate_limit_exceeded` 等已知错误码。
- `resources`、`resourceCredentials`、`sub2` 与 `localProxySmoke` 健康检查都会携带这些字段。
- Dashboard `latestSystemHealth.upstreamBlocker` 和 `deliveryBlocker` 预览会保留这些字段。
- Admin 系统健康页的 issue/sample 行、打开 Sub2 状态、打开共享资源、资源创建默认值和 Sub2 修复上下文都会保留这些字段。
- 系统健康摘要字段白名单会把这些字段纳入引用/样本摘要，便于管理员和二次集成快速定位上游账号错误。

#### 价值

- 管理员可以直接看到 `HTTP 401 / token_invalidated / invalid_request_error`，不再依赖人工阅读多行 JSON。
- 运维自动化可以用机器可读字段判断下一步动作，例如重新授权 OpenAI、应用新的 refresh token、检查额度或等待限流恢复。
- 本地 OpenAI/Codex 反代契约与真实上游凭据问题被清晰分离：契约正常时，`openAiProxyContract` 保持 ok；真实上游凭据失效时，`sub2`、`resourceCredentials` 和 `localProxySmoke` 以结构化账号错误说明阻断原因。
- 该能力只解析既有诊断消息，不读取、解密或展示 refresh token 明文，不写入 Sub2API，不改变限流、扣费、租赁或代理转发链路。

#### 生产验收证据

- 2026-06-13 部署 `4c76561b25c2a93a3096427061f4aa409f759a86` 后，`/health`、`/ready`、Web、Admin 均返回 200。
- `GET /ready` 显示 database、Sub2API、Redis OAuth 状态、Redis OpenAI proxy limiter 均 ok。
- `GET /api/admin/system-health` 总体仍为 `error`，但阻断已结构化为：
...
- `GET /api/admin/dashboard` 的 `latestSystemHealth.upstreamBlocker` 也保留同一组字段，并继续指向 `repairAction=apply_openai_refresh_token_to_sub2_account`。
- 管理员入口覆盖仍为 API 5/5 核心范围、66/66 路由、66/66 入口；前端 5/5 核心范围、16 个列表入口、5 个关键入口。

#### 剩余约束

该能力提升的是诊断与修复入口，不会伪造或绕过真实 OpenAI 上游能力。当前生产 `/v1/responses` 恢复仍需要有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource 共同成立。

### 18.241 本地 OpenAI/Codex smoke 阶段诊断

#### 背景

系统已经能够通过 `localProxySmoke` 证明本地 `/v1/models` 与 `/v1/responses` 的真实链路状态，但此前健康摘要更偏向单一失败结论。当生产环境出现“`/v1/models` 返回 200，但 `/v1/responses` 返回 503”的情况时，管理员需要继续展开审计日志或代理请求日志才能确认失败发生在哪个阶段。为了让 Dashboard、系统健康页和修复入口直接携带阶段级证据，需要把 smoke 审计中的 models/responses 状态码与错误摘要纳入健康 payload。

#### 新增能力

- `localProxySmoke` 健康证据新增 `/v1/models` 阶段字段：
  - `modelsStatusCode`
  - `modelsError`
- `localProxySmoke` 健康证据新增 `/v1/responses` 阶段字段：
  - `responsesStatusCode`
  - `responsesErrorType`
  - `responsesErrorMessage`
- smoke 失败摘要会拼接具体阶段、HTTP 状态、错误类型和错误消息，例如：`/v1/responses (HTTP 503 / api_error / Service temporarily unavailable)`。
- 系统健康完整 issue/sample、资源凭据修复候选、共享资源修复候选、Sub2 上游 issue/sample 和本地 smoke issue 都会保留这些字段。
- Dashboard `latestSystemHealth.upstreamBlocker` 新增：
  - `evidenceModelsStatusCode`
  - `evidenceModelsError`
  - `evidenceResponsesStatusCode`
  - `evidenceResponsesErrorType`
  - `evidenceResponsesErrorMessage`
- Admin 入口会在打开共享资源、打开 Sub2 状态、创建共享资源默认值和 Sub2 修复上下文时传递阶段级 smoke 证据。
- `sub2RepairContextShouldRunSmokeTest` 会同时参考 `modelsOk=false` 与 `responsesOk=false`，避免只根据整体失败状态判断是否需要重新 smoke。

#### 价值

- 管理员可以直接区分“本地模型列表代理可达”和“Responses 真实调用失败”，排障路径更短。
- 当 `/v1/models` 成功而 `/v1/responses` 失败时，系统可以证明本地 `/v1/*` 反代入口并非整体不可达，问题更可能集中在真实上游账号、模型调用或资源交付前提。
- Dashboard 首页 `upstreamBlocker` 可以同时展示账号错误结构化字段与 smoke 阶段状态，减少在系统健康、代理请求日志和 Sub2 状态页之间来回比对。
- 该能力只消费既有 smoke 审计证据，不读取、解密或展示 refresh token 明文，不写入 Sub2API，不改变鉴权、限流、扣费、租赁或代理转发链路。

#### 生产验收证据

- 2026-06-13 部署 `8d4ea4af053f43a389f67646cb76c3203fce4a01` 后，`/health`、`/ready`、Web、Admin 均返回 200。
- 服务器侧部署脚本通过 Prisma generate、migrate deploy、API/Admin typecheck、API 139/139 测试、Admin 15/15 测试和 workspace build。
- `GET /api/admin/system-health` 总体仍为 `error`，29 项检查中 25 ok、1 warning、3 error。
- 最新 smoke 阶段证据为：
  - `modelsOk=true`
...
- `GET /api/admin/dashboard` 的 `latestSystemHealth.upstreamBlocker` 保留同一组阶段证据，并继续指向 `checkId=sub2` 与 `accountErrorCode=token_invalidated`。

#### 剩余约束

该能力提升的是可观测性与修复定位，不会伪造真实 OpenAI 上游可用性。当前生产 `/v1/responses` 恢复仍需要有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource 共同成立。

### 18.242 OpenAI v1 反代契约代表路径继续扩展

#### 背景

18.237 已经把本地 OpenAI/Codex 反代契约从 Responses、Chat Completions、files、uploads、batches 扩展到 embeddings、Assistants、Threads/Runs、Vector Stores、Audio、Images 和 fine-tuning。继续复查当前 OpenAI API surface 后，系统还需要把 Realtime、Containers、Evals、Moderations、Videos、Conversations 和 Responses 生命周期端点纳入静态契约证据。虽然 `/v1/*` 通配路由已经能覆盖这些路径，但健康巡检需要显式样本，才能在未来代码收窄路由时及时发现回归。
...
- `openAiProxyCorePathSamples` 新增代表路径：
  ...
  - `/v1/evals`
  - `/v1/evals/:id/runs`
  - `/v1/evals/:id/runs/:runId/output_items`
  - `/v1/containers`
  - `/v1/containers/:id/files`
  - `/v1/containers/:id/files/:fileId/content`
  - `/v1/realtime/client_secrets`
  - `/v1/realtime/calls`
  - `/v1/realtime/calls/:id/accept`
- `inspectOpenAiProxyContract()` 摘要新增：
  ...
  - `routesContainersApi`
  - `routesRealtimeApi`
- `extended_openai_path_samples_not_routed` 的判断范围同步扩大，覆盖 Responses 生命周期、Conversations、audio/image/video、moderations、evals、containers 和 realtime。
- Dashboard 健康预览字段白名单同步保留新增指标，管理员不用展开完整 payload 也能看到更广的 OpenAI v1 反代覆盖证据。
- `admin-capabilities` 测试中的 `corePathSamples` 复用源码导出的样本集合，降低未来扩展时测试和实现分叉的风险。

#### 价值

- “完整 OpenAI/Codex 反代”不再只证明常规文本、文件和向量路径，也能覆盖 Realtime、Containers、Evals 等更接近 agentic/Codex 场景的 API surface。
- 该能力仍然保持单一 `/v1/*` Sub2API 反代内核，不引入逐端点分叉逻辑。
- 系统健康可以更早发现本地路由契约收窄，而不是等真实用户请求新端点失败后才从代理请求日志里排查。
- 该能力只增强静态契约、健康指标、Dashboard 摘要和测试覆盖，不主动调用 OpenAI 或 Sub2API，不改变鉴权、限流、扣费、租赁或凭据处理。
...
- 2026-06-13 部署 `986af2f83f12744de114bbac3f78cba03bdbfbaf` 后，服务器侧 API 139/139、Admin 15/15、workspace build 和 `/health`、`/ready`、Web、Admin 入口检查均通过。
- 生产 `GET /api/admin/system-health` 中 `openAiProxyContract.status=ok`，新增指标 `routesResponsesLifecycle`、`routesConversationsApi`、`routesVideoApis`、`routesModerationsApi`、`routesEvalsApi`、`routesContainersApi`、`routesRealtimeApi` 均为 `true`。
- 生产 Dashboard 关键巡检预览保留 `routesRealtimeApi=true`，说明新增指标已经进入管理员首页摘要。

#### 剩余约束

该能力证明的是本地反代契约覆盖范围，不证明真实上游账号可用。生产 `/v1/responses` 恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。

### 18.243 Dashboard 固定展示当前发布运行态

#### 背景

系统已经有 `deploymentRuntime` 巡检，用于证明 API/Web/Admin 进程是否运行在当前 release，并读取 `.release-marker` 中的 commit 与 deployedAt。但 Dashboard 首页的 `criticalChecks` 只保留排序后的前 8 个检查。当生产同时存在 Sub2/OpenAI 上游阻断、资源凭据、资源池和交付问题时，`deploymentRuntime.status=ok` 很容易被挤出首页摘要，管理员需要进入完整系统健康页才能确认当前运行 commit。

#### 新增能力

- `dashboardLatestSystemHealthPreview()` 新增固定字段 `deploymentRuntime`。
- 该字段从 `deploymentRuntime` 健康检查中提取：
  - `commit`
  - `deployedAt`
  - `releaseRoot`
  - `markerPath`
  ...
  - 原始检查预览 `check`
- Dashboard metrics 白名单新增部署运行态字段，确保首页摘要不会丢失 release marker 证据。
- Admin 首页“系统状态”表新增“当前发布”行，展示当前 commit、部署时间、release root 和旧/staging release 标志。
- 部署 marker 的紧凑时间格式如 `20260613T012534Z` 会在前端转换为可读时间；无法解析时保留原文。

#### 价值

- 管理员不需要展开完整巡检，就能确认当前页面和 API 正在运行哪个 Git commit。
- 当 `/v1/responses` 仍由上游凭据失效阻断时，可以排除“是不是旧 release 仍在服务”的干扰。
- 该能力只增强 Dashboard 摘要和前端展示，不改变部署脚本、不重启服务、不修改系统健康判断，也不触碰 OpenAI/Sub2API 凭据。

#### 验收证据

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-capabilities.test.ts tests/admin-deployment-runtime.test.ts`：23/23 通过。
- `pnpm.cmd --filter @zyz/api run typecheck`：通过。
- `pnpm.cmd --filter @zyz/admin run typecheck`：通过。

#### 剩余约束

该能力证明的是当前运行版本，不证明真实 OpenAI 上游账号可用。生产 `/v1/responses` 恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。

### 18.244 Dashboard 当前发布态必须实时覆盖旧健康快照

#### 背景

18.243 已要求 Dashboard 固定展示 `deploymentRuntime`。线上部署后进一步发现，Dashboard 的 `latestSystemHealth` 语义是“最新健康快照”。如果最新快照是在上一轮发布生成的，那么首页虽然有 `deploymentRuntime` 字段，但仍可能显示旧 commit，无法准确回答“当前进程到底跑的是哪个 release”。

#### 新增要求

- Dashboard 首页的 `latestSystemHealth.deploymentRuntime` 必须优先使用实时 `deploymentRuntimeHealthCheck()` 的结果。
- 当实时检查不可用时，才允许回退到健康快照里的 `deploymentRuntime`。
- `deploymentRuntime.metrics` 必须包含：
  - `cwd`
  - `releaseRoot`
  - `releaseRootName`
  - `markerPath`
  ...
  - `runningFromReplacedRelease`
  - `runningFromStagingRelease`
- Dashboard 的总体健康状态、阻断卡片、关键检查列表、快照时间和 stale 判断继续来自最新健康快照，避免首页请求触发完整巡检。

#### 管理价值

- 管理员看到 `/v1/responses` 503 时，可以立即排除“是否仍在旧 release 服务”的干扰。
- 部署后无需先手动刷新完整健康快照，Dashboard 首页也能显示当前 `.release-marker`。
- 发布态证据与上游可用性证据分离：当前发布态可以是 ok，同时系统总体仍因 Sub2/OpenAI 凭据失效保持 error。
...
- 2026-06-13 部署 `5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6` 后，服务器侧 API 140/140、Admin 15/15、workspace build 和 `/health`、`/ready`、Web、Admin 入口检查均通过。
- 生产 Dashboard：
...
该能力只证明当前运行版本。生产 `/v1/models` 已返回 200，但 `/v1/responses` 仍返回 `HTTP 503 / api_error / Service temporarily unavailable`；完整 OpenAI/Codex 反代恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。

### 18.245 已有 Codex 资源凭据应用必须默认带端到端 smoke 验收

#### 背景

当管理员从共享资源详情页修复已有 Codex 资源时，页面已经支持把保存的 `openai_refresh_token` 应用到 Sub2 账号。但此前“应用后端到端自检”默认未勾选，管理员可能只完成凭据应用，没有立即验证 `/v1/responses` 是否恢复。当前生产真实阻断正是 `/v1/responses` 503，因此已有资源修复路径也必须默认推动 smoke 验收。

#### 新增要求

- 共享资源详情页的“应用到 Sub2”表单，在以下条件同时满足时默认勾选“应用后端到端自检”：
  - `resourceType=codex`
  - 存在 `sub2AccountId`
  - 凭据类型为 `openai_refresh_token`
  - 凭据状态为 `active`
- 表单应尽量从最近凭据应用审计记录中提取 smoke 模型：
  - 优先读取 `after.model`
  - 其次读取 `after.smokeTest.model`
  - 都不存在时留空，由后端使用默认 `SUB2_SMOKE_MODEL`
- 对非 Codex 资源、无 Sub2 账号、非 active 凭据或非 OpenAI refresh token 凭据，不自动勾选 smoke。
- 该能力只改变管理员表单默认行为，不读取、不展示、不自动提交 refresh token 明文。

#### 管理价值

- 管理员修复已有资源时，系统默认把“应用凭据”和“真实反代 smoke 验收”放在同一个动作里。
- 减少 `openai_group_has_no_active_accounts` 修复后仍没有刷新 `/v1/responses` 证据的空窗。
- 与新建共享资源时的“创建后应用到 Sub2 + 应用后 smoke”保持一致。
...
- 2026-06-13 部署 `15482b8ba5fe09fcb1601363f15b89f56acbc429` 后，服务器侧 API 140/140、Admin 16/16、workspace build 和 `/health`、`/ready`、Web、Admin 入口检查均通过。
- 生产 Dashboard 显示当前发布 commit 为 `15482b8ba5fe09fcb1601363f15b89f56acbc429`，Admin 构建产物为 `index-CHuJx3t_.js`。

#### 剩余约束

该能力不能凭空恢复无效上游凭据。当前生产仍没有 active OpenAI refresh token resource credential，也没有 ready online production Codex shared resource；`/v1/responses` 仍返回 `HTTP 503 / api_error / Service temporarily unavailable`。

### 18.246 新建资源初始凭据应用必须先校验 Sub2 账号

#### 背景

管理员从共享资源页创建生产 Codex 资源时，可以同时填写初始 OpenAI refresh token，并勾选“创建后应用到 Sub2”和“应用后端到端自检”。此前后端已经要求初始凭据存在、类型为 `openai_refresh_token`、状态为 `active`，但如果管理员漏填 `sub2AccountId`，资源会先被创建，再在应用凭据阶段失败，留下半成品资源。

#### 新增要求

- `POST /api/admin/resources` 必须在创建资源事务前校验初始凭据应用请求。
- 当 `applyCredentialToSub2=true` 时，必须满足：
  - `credentialSecret` 已提供
  - `sub2AccountId` 已提供
  - `credentialType` 为 `openai_refresh_token`
  - `credentialStatus` 为 `active`
- 当 `credentialRunSmokeTest=true` 但 `applyCredentialToSub2=false` 时，继续返回 `initial_credential_apply_required`。
- 缺少 `sub2AccountId` 时返回 HTTP 400，错误码为 `initial_credential_sub2_account_required`。
- 该错误必须发生在资源创建前，不能写入半成品共享资源或凭据。

#### 管理价值

- 管理员不会因为漏填 Sub2 账号而留下无法直接应用凭据的 Codex 资源。
- “创建资源 + 保存凭据 + 应用到 Sub2 + 端到端 smoke”继续作为一个原子化维修意图处理。
- 当前 `openai_group_has_no_active_accounts` 修复链路会更清晰：必须先选定目标 Sub2 账号，再提交有效 refresh token。
...
- 2026-06-13 部署 `6dedb8460367c642a68a139a5d55256d9c10fdf6` 后，服务器侧 API 141/141、Admin 16/16、workspace build 和 `/health`、`/ready`、Web、Admin 入口检查均通过。
- 生产负向请求验证：不带 `sub2AccountId` 的初始凭据应用请求返回 HTTP 400 / `initial_credential_sub2_account_required`，且 Codex 资源数量请求前后均为 `1`。

#### 剩余约束

该能力防止错误配置写入半成品资源，但不会生成有效 OpenAI refresh token。生产仍需管理员提供有效 token 并绑定 Sub2 账号后，才能恢复 active OpenAI account、ready production Codex shared resource 与 `/v1/responses`。

### 18.247 Codex 交付必须受最新本地代理 smoke 失败证据保护

#### 背景

系统已经能证明本地 OpenAI/Codex 反代契约覆盖完整 `/v1/*` 路径，也能通过共享资源状态、Sub2 账号绑定和 active `openai_refresh_token` 凭据判断是否存在 ready production Codex shared resource。但生产复查中出现过更细的失败形态：资源/凭据修复链路正在推进，`/v1/models` 可以返回 200，而真实 `/v1/responses` 仍返回 `HTTP 503 / api_error / Service temporarily unavailable`。如果交付门禁只看资源表字段，可能出现商品目录显示可交付、管理员允许上架、用户下单或轮换 key 后才暴露真实代理路径失败。

#### 新增要求

- Codex 交付门禁必须读取最近的本地 OpenAI/Codex 代理 smoke 审计证据。
- 当最新 smoke 证据满足以下条件时，Codex 交付必须被阻断：
  - 证据仍在新鲜窗口内，默认 24 小时。
  - smoke 没有因为 `credential_apply_failed` 或 `sub2_account_test_failed` 等前置失败被跳过。
  - `modelsOk=false`、`responsesOk=false` 或 `localProxyOk=false` 任一成立。
- 被阻断时统一返回/暴露 `codex_proxy_smoke_failed_for_delivery`，并带上：
  - smoke 审计日志 ID、动作、资源 ID、Sub2 账号 ID。
  - `createdAt`、`ageMinutes`、`staleThresholdMinutes`、`freshMinutesRemaining`、`staleAt`。
  - `modelsOk`、`modelsStatusCode`、`responsesOk`、`responsesStatusCode`、`responsesErrorType`、`responsesErrorMessage`。
  - 代理请求日志 ID、请求 ID、上游请求 ID、失败路径、状态码和错误码。
- 缺少 smoke 证据、过期失败证据、或前置失败导致的跳过证据，不直接阻断用户交付；这些情况继续由系统健康页提示管理员补跑 smoke。
- 以下入口必须复用同一 Codex 代理 smoke 交付门禁：
  - 用户创建订单时的 `requireReadySupplierResourceForDelivery()`。
  - 租赁 API Key 轮换。
  - 管理员创建/更新 active Codex 商品。
  - 管理员创建/更新 active 且可购买的 Codex 价格。
  - 公开商品目录与管理员商品列表的 `deliveryReady` / `deliveryBlockedReason` 字段。
- 当 ready production Codex shared resource 缺失与 proxy smoke 失败同时存在时，`deliveryBlockedReason` 优先返回 `codex_resource_not_ready_for_delivery`；`codexProxySmokeDeliveryReady=false` 和 `codexProxySmokeDeliveryLatest` 继续保留最新 `/v1/responses` 失败证据。
- 管理员确需历史维护或特殊配置时，商品/价格激活仍可通过既有 `allowUnavailableDelivery=true` 显式覆盖。

#### 管理价值

- `deliveryReady=true` 不再只表示“资源表字段齐全”，还会避开已知新鲜的 `/v1/responses` 或本地代理失败证据。
- 当最新 smoke 已证明真实反代路径失败时，系统会在售卖、上架和 key 轮换前挡住风险，而不是把故障转嫁给真实买家请求。
- 前台商品目录、后台商品管理、订单交付和租赁 key 轮换使用同一判断，减少“页面显示可买但下单失败”的割裂状态。
- 该能力只消费已有审计与健康证据，不解密 refresh token，不保存 API Key 明文，不保存请求体/响应体，也不绕过 Sub2API/OpenAI 的真实可用性。
...
该能力只能在“最新且明确的本地代理 smoke 失败”存在时保护交付，不会凭空生成有效 OpenAI refresh token。生产 `/v1/responses` 的真实恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account、ready production Codex shared resource 和新一轮通过的端到端 smoke。

### 18.248 商品目录与售出交付巡检复用 Codex proxy smoke 交付门禁

#### 背景

18.247 已经让订单交付、租赁 Key 轮换、商品/价格激活、公开商品目录和 Admin 商品列表共同使用最新本地 OpenAI/Codex proxy smoke 失败证据。但系统健康里的 `productCatalog` 与 `salesDelivery` 仍需要继续复查：如果未来 ready production Codex shared resource 已经存在，而最新 `/v1/responses` smoke 仍在新鲜窗口内失败，健康巡检不能只因为资源字段齐全就显示商品目录或售出交付正常。

#### 新增要求

- `productCatalog` 巡检必须复用 `codexDeliveryReadinessSnapshot()`。
- active 且可购买的 Codex 商品在存在 ready resource 但最新 proxy smoke 失败时，必须新增 warning issue：
  - `type=active_codex_product_proxy_smoke_failed`
  - `resourceList=true`
  - `resourceScope=production`
  - `repairAction=apply_openai_refresh_token_to_sub2_account`
- `productCatalog.summary` 必须保留：
  - `readyCodexDeliveryResources`
  - `codexProductsWithoutReadyDeliveryResources`
  - `codexProductsBlockedByProxySmoke`
- `salesDelivery` 巡检必须复用同一 Codex 交付快照。
- active Codex 租赁在最新 proxy smoke 失败时，必须新增 error issue：
  - `type=active_codex_rental_proxy_smoke_failed`
  - 关联 `orderId`、`rentalId`、`userId`、`userEmail`
  - 尽量保留 `supplierResourceId`、`supplierEmail`、`sub2AccountId`
  - `resourceList=true`
  - `resourceScope=production`
  - `repairAction=apply_openai_refresh_token_to_sub2_account`
- 商品目录 issue 与售出交付 issue 必须携带最新 smoke 证据字段：
  - `auditLogId`、`auditAction`
  - `model`
  - `modelsOk`、`modelsStatusCode`、`modelsError`
  - `responsesOk`、`responsesStatusCode`、`responsesErrorType`、`responsesErrorMessage`
  - `localProxyOk`
  - `proxyRequestLogId`、`requestId`、`upstreamRequestId`
  - `proxyRequestPath`、`proxyRequestStatusCode`、`proxyRequestErrorCode`
  - `ageMinutes`、`stale`、`staleThresholdMinutes`、`freshMinutesRemaining`、`staleAt`
- Dashboard 健康预览必须保留上述字段，让首页阻断摘要、资源修复入口和 Sub2 修复入口能继续传递 `/v1/responses` 失败证据。

#### 管理价值

- 管理员看到 `productCatalog=ok` 时，可以相信公开可售 Codex 商品没有被新鲜 proxy smoke 失败证据阻断。
- 管理员看到 `salesDelivery=ok` 时，可以相信已售 active Codex 租赁没有被最新端到端反代失败证据影响。
- 健康巡检、商品目录、售出交付、订单交付和 Key 轮换使用同一交付门禁，减少“健康面板 ok 但实际交付失败”的割裂状态。
- 该能力只消费既有审计证据和只读资源状态，不解密 refresh token，不保存 API Key 明文，不保存请求体或响应体，不改变真实 Sub2API/OpenAI 转发链路。
...
该能力让健康巡检真实反映已知 proxy smoke 失败，不会自动修复失效 OpenAI refresh token 或 Sub2 OpenAI account。生产 `/v1/responses` 恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account、ready production Codex shared resource 和新一轮通过的端到端 smoke。

### 18.249 空 active Codex 商品目录必须进入同一条上游维修链路

#### 背景

生产复查显示，系统基础健康、反代契约和 Dashboard 运行态均正常，但真实业务仍存在一组相关阻断：`Codex 标准租赁` 仍为 `offline`，没有 ready production Codex shared resource，没有 active OpenAI refresh token credential，Sub2 OpenAI 账号报 `token_invalidated`，`/v1/responses` smoke 返回 `HTTP 503 / api_error`。此前 `productCatalog` 的 `empty_active_product_catalog` warning 只说明没有 active 商品和候选商品状态，管理员从商品入口无法直接看到同一条上游修复证据。

#### 新增要求

- 当 `empty_active_product_catalog` 的候选商品 `resourceType=codex` 时，该 issue 必须携带共享资源维修上下文：
  - `resourceList=true`
  - `resourceScope=production`
  - `resourceStatus=online`
  - `repairAction=apply_openai_refresh_token_to_sub2_account`
- 该 issue 的 `actionHint` 必须表达先创建或修复 ready production Codex shared resource，再激活候选商品。
- `enrichSub2RepairContextChecks()` 必须把 Codex 类型的 `empty_active_product_catalog` 作为商品上下文来源，向 `resources`、`resourceCredentials`、`sub2` 和同类上游维修问题补齐：
  - `productId`
  - `productName`
  - `priceId`
- 当同一轮巡检存在唯一 active 供给方、Sub2 账号修复候选或最新 local proxy smoke 失败证据时，Codex 空目录候选 issue 也必须继承这些字段，和其它上游维修问题保持一致。
- 非 Codex 候选商品或没有候选商品的目录级 fallback 不应被误标为 OpenAI refresh token 修复动作。

#### 管理价值

- 管理员看到“没有 active Codex 商品”时，可以同时知道真正的前置修复是 production Codex shared resource、Sub2 OpenAI account 和有效 refresh token，而不是只停留在商品激活。
- 商品目录、共享资源、资源凭据、Sub2 状态和本地 smoke 的问题上下文保持一致，减少跨页面复制商品 ID、价格 ID 和账号诊断的步骤。
- 当前 `openai_group_has_no_active_accounts` 与 `/v1/responses` 503 不会被离线商品 warning 掩盖，商品入口也能落到同一条维修闭环。

#### 验收证据

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-capabilities.test.ts`：24/24 通过。

#### 剩余约束

该能力只补齐管理员定位与修复上下文，不生成有效 OpenAI refresh token，也不会绕过 Sub2/OpenAI 的真实认证。生产恢复仍依赖管理员提供有效 refresh token，形成 active Sub2 OpenAI account、active resource credential、ready production Codex shared resource，并重新跑通 `/v1/responses` 端到端 smoke。
