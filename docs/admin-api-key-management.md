...
- 管理后台支持按当前筛选条件批量启用或停用 API Key，单次最多处理 500 条。
...
- 批量泄露或异常调用处置时，可以先筛选用户、商品、资源类型或状态，再一次性停用命中 Key。
...
- `POST /api/admin/api-keys/bulk-status` 仅允许 `admin` 操作。
- 列表仅展示 Key 前缀和元数据，不返回完整明文 Key。
- 批量启用会拒绝非 active 租赁下的 API Key，避免恢复不可用或已关闭租赁。
- 批量启停会写入审计日志 `admin.api_key.bulk_status`，并返回 Sub2 同步失败数量供管理员复查。
