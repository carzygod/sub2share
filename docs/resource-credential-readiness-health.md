...
- `openai_refresh_token_candidate_missing` 同时会带出 `resourceList=true` 与 `resourceType=codex`，管理后台可以从同一问题行直接打开 Codex 共享资源列表，用于创建生产资源或补录 active refresh token。
- 管理后台问题样本和候选样本摘要会直接展示 `repairAction=apply_openai_refresh_token_to_sub2_account`，避免管理员只看到账号状态却不知道当前推荐的修复路径。
