...
- 当 Sub2/OpenAI 上游问题是 `openai_group_has_no_active_accounts` 且巡检已经发现非 active 或不可调度的 OpenAI 账号时，主问题样本会携带首个修复候选的 `sub2AccountId`、账号状态、凭据状态、调度状态、`tempUnschedulableReason`、`updatedAt`、`accountMessage` 和 `repairAction=apply_openai_refresh_token_to_sub2_account`；点击 `打开反代状态` 会直接预选该账号，并显示账号失败摘要。
...
- 候选样本进入 `反代状态` 时会携带 `actionHint`、账号诊断、请求定位、smoke 分段结果、模型、失败路径、HTTP 状态码、代理错误码、`ageMinutes`、`stale`、`staleThresholdMinutes` 和 `freshMinutesRemaining`，避免 Sub2 修复页丢失证据上下文。
