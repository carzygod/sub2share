...
- `GET /api/admin/dashboard` 的关键巡检详情预览保留 `actionHint`、`proxyRequestPath`、`proxyRequestStatusCode`、`proxyRequestErrorCode`、`model`、`modelsOk`、`responsesOk`、`localProxyOk`、`smokeTestSkippedReason`、`ageMinutes` 和 `stale`。
...
- 管理员从首页进入反代修复页后，如果失败 smoke 证据已过期，会直接看到“证据已过期”，便于修复后立即重新运行端到端自检刷新当前 `/v1/responses` 证据。
