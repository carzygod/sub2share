...
## 2026-06-13 08:34 Sub2 修复诊断字段空白归一化发布与线上复查

### 本次发布

- 代码提交：`de52395 fix: normalize sub2 repair diagnostics`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - `sub2AccountHealthSamples` 将空白的可选诊断字段归一为 `null`。
  - `resourceCredentialRepairCandidateFields` 与 `resourceCredentialSub2AccountRepairSamples` 同步清洗空白账号名、账号状态、凭据状态、group、message 和更新时间。
  - 当 Sub2 账号没有有效错误摘要时，样本回落到默认可读 message，不再把空字符串透传到完整健康 payload。

### 本地验证

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-sub2-upstream-health.test.ts tests/admin-resource-credential-health.test.ts`：11/11 通过。
- `pnpm.cmd --filter @zyz/api run typecheck`：通过。
- `pnpm.cmd --filter @zyz/api test`：138/138 通过。
- `pnpm.cmd --filter @zyz/admin test`：15/15 通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。

### 生产部署

- 部署归档：`/tmp/sub2share-user-de52395.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-de52395.tar --commit de52395761d5dd46e0936b3174eabece55236dee`
- 部署脚本在服务器侧通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  - Admin typecheck：通过。
  - API tests：138/138 通过。
  - Admin tests：15/15 通过。
  - workspace build：通过。
  - `/health`、`/ready`、Web、Admin：均 200。
  - 端口 cwd 校验：4100、3100、3101 均运行在 `/opt/zhisuan-yizhan/user` 当前 release 下。
- release marker：
  - `commit=de52395761d5dd46e0936b3174eabece55236dee`
  - `deployed_at=20260613T003320Z`
...
- `POST /api/auth/login`：管理员账号 `admin@zhisuan.local` 登录成功。
- `GET /api/admin/system-health`：
  - release marker 为 `de52395761d5dd46e0936b3174eabece55236dee`。
  - 总体 `status=error`，摘要为 29 项检查：25 ok、1 warning、3 error。
  - `sub2.detail.issues[0].tempUnschedulableReason=null`。
  - `sub2.detail.samples[0].tempUnschedulableReason=null`。
  - `resourceCredentials.detail.issues[0].tempUnschedulableReason=null`。
  - `resources.detail.issues[0].tempUnschedulableReason=null`。
  - 账号错误摘要仍保留 OpenAI `token_invalidated` / 401 原因。
  - `openAiProxyContract.status=ok`，扩展 OpenAI v1 代表路径指标仍全部为 true。
  - `adminCapabilities.status=ok`，5/5 核心范围、66/66 路由、66/66 入口。
  - `adminSurfaceCoverage.status=ok`，5/5 核心范围、16 个列表入口、5 个关键入口。
  - `frontendRuntime.status=ok`，Web/Admin 共 2 个入口、4 个构建资产均可达。
- `GET /api/admin/dashboard`：
  - `latestSystemHealth.status=error`。
  - `upstreamBlocker.tempUnschedulableReason=null`。
  - `upstreamBlocker.accountMessage` 仍明确指向 OpenAI token invalidated / 401。
  - 管理员入口覆盖摘要仍为 API 5/5 核心范围、66/66 路由、66/66 入口；前端 5/5 核心范围、16 个列表入口、5 个关键入口。
...
- `sub2.status=error`：`openai_group_has_no_active_accounts`，Sub2 默认 OpenAI 分组有 2 个 OpenAI account，但 active account 为 0。
- `localProxySmoke.status=error`：当前 release 下 `/v1/models` 可用，但 `/v1/responses` 仍返回 503。
- 结论：本次发布清理了完整健康 payload 中的空白诊断噪声；真实 `/v1/responses` 恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。
## 2026-06-13 08:53 Sub2 账号错误结构化诊断发布与线上复查

### 本次发布

- 代码提交：`4c76561 feat: expose sub2 account error diagnostics`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - `sub2`、`resourceCredentials`、`resources` 与 `localProxySmoke` 健康结果新增结构化账号错误字段。
  - 新增字段为 `accountErrorStatusCode`、`accountErrorType`、`accountErrorCode`、`accountErrorMessage`。
  - 当 Sub2 返回 OpenAI JSON 错误体时，系统会从 `Authentication failed (401): {...}` 中解析 HTTP 状态、OpenAI error type/code/message。
  - Dashboard、系统健康页、共享资源创建默认值与 Sub2 修复上下文都会保留这些字段，管理员不再需要人工解析长 `accountMessage`。
  - 该能力只读取既有 Sub2 诊断输出，不读取、解密或展示 refresh token 明文，也不写入 Sub2API。

### 本地验证

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-resource-credential-health.test.ts tests/admin-sub2-upstream-health.test.ts tests/admin-capabilities.test.ts`：31/31 通过。
- `pnpm.cmd --filter @zyz/admin test`：15/15 通过。
- `pnpm.cmd --filter @zyz/api run typecheck`：通过。
- `pnpm.cmd --filter @zyz/admin run typecheck`：通过。
- `pnpm.cmd --filter @zyz/api test`：139/139 通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。

### 生产部署

- 部署归档：`/tmp/sub2share-user-4c76561.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-4c76561.tar --commit 4c76561b25c2a93a3096427061f4aa409f759a86`
- 部署脚本在服务器侧通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  - Admin typecheck：通过。
  - API tests：139/139 通过。
  - Admin tests：15/15 通过。
  - workspace build：通过。
  - `/health`、`/ready`、Web、Admin：均 200。
- release marker：
  - `commit=4c76561b25c2a93a3096427061f4aa409f759a86`
  - `deployed_at=20260613T005316Z`
...
- `GET /ready` 依赖：
  - database：ok。
  - sub2api：ok，HTTP 200。
  - oauthStateStore：ok，Redis shared，reachable。
  - openAiProxyLimiter：ok，Redis shared，reachable。
- `POST /api/auth/login`：管理员账号 `admin@zhisuan.local` 登录成功。
- `GET /api/admin/system-health`：
  - 总体 `status=error`，29 项检查：25 ok、1 warning、3 error。
  - `resources.status=warning`，原因是没有 online production Codex shared resource。
  - `resourceCredentials.status=error`，原因是 Sub2 上游无 active 账号，且没有可应用的资源凭据。
  - `sub2.status=error`，原因是 `openai_group_has_no_active_accounts`。
  - `localProxySmoke.status=error`，最新 `/v1/responses` smoke 仍失败。
  - `resources`、`resourceCredentials`、`sub2`、`localProxySmoke` 均已经透出结构化账号错误：
...
- `GET /api/admin/dashboard`：
  ...
  - `repairAction=apply_openai_refresh_token_to_sub2_account`。
  - `sub2AccountId=2`，`sub2AccountName=1`，`accountStatus=error`，`credentialsStatus=configured(3)`，`schedulable=false`。
  - `accountErrorStatusCode=401`，`accountErrorType=invalid_request_error`，`accountErrorCode=token_invalidated`。
  - 最新 smoke 证据仍指向 `/v1/responses`，`evidenceStatusCode=503`，`proxyRequestFilterStatus=upstream_error`，`proxyRequestFilterLookup=9f725728-665a-4059-aade-23ca872d6df2`。
  - `adminEntryCoverage.ok=true`，API 5/5 核心范围、66/66 路由、66/66 入口；前端 5/5 核心范围、16 个列表入口、5 个关键入口。
  - `managementOverview` 可展示用户、钱包、销售、租赁与共享资源概览。

### 剩余真实阻断

- Sub2 默认 OpenAI 分组有 2 个 OpenAI account，但 active account 为 0。
- 两个 OpenAI account 当前均返回 `HTTP 401 / token_invalidated`。
- 当前没有 active OpenAI refresh token resource credential 可自动应用。
- 当前没有 ready online production Codex shared resource。
- 结论：本次发布已经把真实阻断从长文本诊断提升为可机读字段；真实 `/v1/responses` 恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。

### 清理

- 远端 `/tmp/sub2share-user-4c76561.tar` 已清理。
- 本地 `.deploy/sub2share-user-4c76561.tar` 已清理，空 `.deploy` 目录已删除。

## 2026-06-13 09:12 本地反代 smoke 阶段诊断发布与线上复查

### 本次发布

- 代码提交：`8d4ea4af053f43a389f67646cb76c3203fce4a01 feat: expose proxy smoke phase diagnostics`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - `localProxySmoke` 健康证据新增 `/v1/models` 与 `/v1/responses` 的分阶段状态字段。
  - 新增字段包括 `modelsStatusCode`、`modelsError`、`responsesStatusCode`、`responsesErrorType`、`responsesErrorMessage`。
  - smoke 失败摘要现在会携带具体失败阶段，例如 `/v1/responses (HTTP 503 / api_error / Service temporarily unavailable)`。
  - Dashboard `latestSystemHealth.upstreamBlocker` 会保留 `evidenceModelsStatusCode`、`evidenceModelsError`、`evidenceResponsesStatusCode`、`evidenceResponsesErrorType`、`evidenceResponsesErrorMessage`。
  - Admin 系统健康页、共享资源创建默认值和 Sub2 修复上下文都会传递这些字段，方便管理员区分“模型列表可达”和“Responses 调用失败”。

### 本地验证

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-local-proxy-smoke-health.test.ts tests/admin-capabilities.test.ts`：27/27 通过。
- `pnpm.cmd --filter @zyz/admin test`：15/15 通过。
- `pnpm.cmd --filter @zyz/api test`：139/139 通过。
- `pnpm.cmd --filter @zyz/api run typecheck`：通过。
- `pnpm.cmd --filter @zyz/admin run typecheck`：通过。
...
- 部署归档：`/tmp/sub2share-user-8d4ea4a.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-8d4ea4a.tar --commit 8d4ea4af053f43a389f67646cb76c3203fce4a01`
- 部署脚本在服务器侧通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  ...
  - `/health`、`/ready`、Web、Admin：均 200。
- release marker：
  - `commit=8d4ea4af053f43a389f67646cb76c3203fce4a01`
  - `deployed_at=20260613T011236Z`
...
- `GET /api/admin/system-health`：
  - 总体 `status=error`，29 项检查：25 ok、1 warning、3 error。
  - `localProxySmoke.metrics.modelsOk=true`。
  - `localProxySmoke.metrics.modelsStatusCode=200`。
  - `localProxySmoke.metrics.modelsError=null`。
  - `localProxySmoke.metrics.responsesOk=false`。
  - `localProxySmoke.metrics.responsesStatusCode=503`。
  - `localProxySmoke.metrics.responsesErrorType=api_error`。
  - `localProxySmoke.metrics.responsesErrorMessage=Service temporarily unavailable`。
  - `localProxySmoke.metrics.localProxyOk=false`。
  - smoke issue 已明确显示：`Latest local OpenAI/Codex smoke test failed at /v1/responses (HTTP 503 / api_error / Service temporarily unavailable).`
- `GET /api/admin/dashboard`：
  - `latestSystemHealth.status=error`。
  - `upstreamBlocker.checkId=sub2`。
  - `evidenceModelsStatusCode=200`。
  - `evidenceModelsError=null`。
  - `evidenceResponsesStatusCode=503`。
  - `evidenceResponsesErrorType=api_error`。
  - `evidenceResponsesErrorMessage=Service temporarily unavailable`。
  - `accountErrorCode=token_invalidated`。

### 剩余真实阻断

- 当前没有 online production Codex shared resource。
- 当前没有 active OpenAI refresh token resource credential。
- Sub2 默认 OpenAI 分组有 2 个 OpenAI account，但 active account 为 0。
- 两个 OpenAI account 当前均返回 `HTTP 401 / token_invalidated`。
- `/v1/models` 已证明可达且返回 200；`/v1/responses` 仍返回 503，根因仍是上游账号凭据失效与缺少 ready production Codex shared resource。
- 结论：本次发布完成了本地反代 smoke 的阶段级诊断与线上验证；真实 `/v1/responses` 恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。

### 清理

- 远端 `/tmp/sub2share-user-8d4ea4a.tar` 已清理。
- 本地 `.deploy/sub2share-user-8d4ea4a.tar` 已清理，空 `.deploy` 目录已删除。

## 2026-06-13 09:25 OpenAI v1 代表路径扩展发布与线上复查

### 本次发布

- 代码提交：`986af2f83f12744de114bbac3f78cba03bdbfbaf feat: expand openai proxy contract surface`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - `openAiProxyCorePathSamples` 扩展到 Responses lifecycle、Conversations、Vector Store search、Uploads parts/complete、Audio speech/translation、Videos、Moderations、Evals、Containers 和 Realtime。
  - `openAiProxyContract.metrics` 新增 `routesResponsesLifecycle`、`routesConversationsApi`、`routesVideoApis`、`routesModerationsApi`、`routesEvalsApi`、`routesContainersApi`、`routesRealtimeApi`。
  - Dashboard 健康预览白名单同步保留这些指标，管理员首页可直接看到更完整的 OpenAI v1 反代契约证据。
  - 运行时仍由 `/v1` 与 `/v1/*` 统一转发到 Sub2API，不新增逐端点分叉代理逻辑。

### 本地验证

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/openai-proxy-helpers.test.ts tests/admin-capabilities.test.ts`：41/41 通过。
- `pnpm.cmd --filter @zyz/api run typecheck`：通过。
- `pnpm.cmd --filter @zyz/api test`：139/139 通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。
...
- 部署归档：`/tmp/sub2share-user-986af2f.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-986af2f.tar --commit 986af2f83f12744de114bbac3f78cba03bdbfbaf`
- 部署脚本在服务器侧通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
...
- `GET /api/admin/system-health`：
  ...
  - `localProxySmoke.metrics.responsesErrorMessage=Service temporarily unavailable`。
- `GET /api/admin/dashboard`：
  - `latestSystemHealth.upstreamBlocker.checkId=sub2`。
  - `latestSystemHealth.upstreamBlocker.accountErrorCode=token_invalidated`。
  - `latestSystemHealth.upstreamBlocker.evidenceResponsesStatusCode=503`。
  - `openAiProxyContract` 关键巡检预览中 `routesRealtimeApi=true`，说明新增指标已经进入首页预览白名单。
...
- 两个 OpenAI account 当前均返回 `HTTP 401 / token_invalidated`。
- 当前没有 ready online production Codex shared resource。
- `/v1/models` 已证明可达且返回 200；`/v1/responses` 仍返回 503。
- 结论：本次发布扩展的是本地 OpenAI v1 反代契约证明面；真实 `/v1/responses` 恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。

### 清理

- 远端 `/tmp/sub2share-user-986af2f.tar` 已清理。
- 本地 `.deploy/sub2share-user-986af2f.tar` 已清理，空 `.deploy` 目录已删除。

## 2026-06-13 09:50 Dashboard 当前发布摘要修复、部署与线上复查

### 本次发布

- 初始能力提交：`58199b7dfa79c83c2d61e323305a1439e8e4f1e0 feat: surface deployment runtime on dashboard`
- 快照旧值修复提交：`3a7c7b83e35d2b3566d8943ad694e888c218f837 fix: show live deployment runtime on dashboard`
- marker path 补齐提交：`5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6 fix: include deployment marker path in dashboard runtime`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - `GET /api/admin/dashboard` 的 `latestSystemHealth.deploymentRuntime` 固定返回当前发布态。
  - Dashboard 不再只从最新健康快照读取 deployment runtime；即使健康快照来自上一轮发布，Dashboard 当前发布行也会用实时 `deploymentRuntimeHealthCheck()` 覆盖。
  - `deploymentRuntime.metrics` 补齐 `cwd`、`releaseRootName`、`markerPath`，首页与完整系统健康都能显示 `.release-marker` 的真实路径。

### 本地验证

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-capabilities.test.ts tests/admin-deployment-runtime.test.ts`：24/24 通过。
- `pnpm.cmd --filter @zyz/api run typecheck`：通过。
- `pnpm.cmd --filter @zyz/api test`：140/140 通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。

### 生产部署

- 最终部署归档：`/tmp/sub2share-user-5419ec5.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-5419ec5.tar --commit 5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6`
- 服务器侧部署脚本通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  - Admin typecheck：通过。
  - API tests：140/140 通过。
  - Admin tests：15/15 通过。
  - workspace build：通过。
  - `/health`、`/ready`、Web、Admin：均 200。
- release marker：
  - `commit=5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6`
  - `deployed_at=20260613T015001Z`
...
- `GET /api/admin/dashboard`：
  - `ok=true`
  - `latestSystemHealth.status=error`
  - `latestSystemHealth.deploymentRuntime.status=ok`
  - `latestSystemHealth.deploymentRuntime.commit=5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6`
  - `latestSystemHealth.deploymentRuntime.deployedAt=20260613T015001Z`
  - `latestSystemHealth.deploymentRuntime.releaseRoot=/opt/zhisuan-yizhan/user`
  - `latestSystemHealth.deploymentRuntime.markerPath=/opt/zhisuan-yizhan/user/.release-marker`
  - `latestSystemHealth.deploymentRuntime.runningFromReplacedRelease=false`
  - `latestSystemHealth.deploymentRuntime.runningFromStagingRelease=false`
  - `upstreamBlocker.checkId=sub2`
  - `upstreamBlocker.accountErrorCode=token_invalidated`
  - `upstreamBlocker.evidenceModelsStatusCode=200`
  - `upstreamBlocker.evidenceResponsesStatusCode=503`
- `GET /api/admin/system-health`：
  - 总体 `status=error`，29 项检查：25 ok、1 warning、3 error。
  - `deploymentRuntime.status=ok`
  - `deploymentRuntime.metrics.commit=5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6`
  - `deploymentRuntime.metrics.markerPath=/opt/zhisuan-yizhan/user/.release-marker`
  - 错误检查仍为 `resourceCredentials`、`sub2`、`localProxySmoke`。
  - warning 检查仍为 `resources`。
- `POST /api/admin/sub2/proxy-smoke-test`：
  - API 调用本身 `ok=true`，smoke 结果 `resultOk=false`。
  - 模型：`gpt-5.3-codex`
  - `/v1/models`：`ok=true`，`statusCode=200`
  - `/v1/responses`：`ok=false`，`statusCode=503`，`errorType=api_error`，`errorMessage=Service temporarily unavailable`
  - 本地 smoke 临时 API key、rental、order、wallet 清理均完成。

### 剩余真实阻断

- Sub2 默认 OpenAI 分组仍有 2 个 OpenAI account，但 active account 为 0。
- 当前没有 active OpenAI refresh token resource credential 可自动应用。
- 当前没有 ready online production Codex shared resource。
- `/v1/models` 可达且返回 200，`/v1/responses` 仍返回 503。
- 结论：本次发布修复的是管理员首页对“当前运行版本”的可观测性；真实 OpenAI/Codex 反代恢复仍依赖有效 OpenAI refresh token、active Sub2 OpenAI account 和 ready production Codex shared resource。

### 清理

- 远端 `/tmp/sub2share-user-58199b7.tar`、`/tmp/sub2share-user-3a7c7b8.tar`、`/tmp/sub2share-user-5419ec5.tar` 已清理。
- 本地 `.deploy` 目录已删除。

## 2026-06-13 10:04 资源凭据应用默认 smoke 验收发布与线上复查

### 本次发布

- 代码提交：`15482b8ba5fe09fcb1601363f15b89f56acbc429 feat: default resource credential apply smoke checks`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - Admin 共享资源详情页的“应用到 Sub2”表单现在会在可验证的 Codex 凭据修复场景下默认勾选“应用后端到端自检”。
  - 默认条件为：`resourceType=codex`、存在 `sub2AccountId`、凭据类型为 `openai_refresh_token`、凭据状态为 `active`。
  - 如果最近凭据应用审计记录里有 `smokeTest.model` 或直接 `model` 字段，表单会预填该 smoke 模型；没有则留空并让后端使用默认 `SUB2_SMOKE_MODEL`。
  - 该能力只改变管理员表单默认值，不读取或展示 refresh token 明文，也不自动应用任何凭据。

### 本地验证

- `pnpm.cmd --filter @zyz/admin test`：16/16 通过。
- `pnpm.cmd --filter @zyz/admin run typecheck`：通过。
- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-capabilities.test.ts`：20/20 通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。

### 生产部署

- 部署归档：`/tmp/sub2share-user-15482b8.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-15482b8.tar --commit 15482b8ba5fe09fcb1601363f15b89f56acbc429`
- 服务器侧部署脚本通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  - Admin typecheck：通过。
  - API tests：140/140 通过。
  - Admin tests：16/16 通过。
  - workspace build：通过。
  - `/health`、`/ready`、Web、Admin：均 200。
- release marker：
  - `commit=15482b8ba5fe09fcb1601363f15b89f56acbc429`
  - `deployed_at=20260613T020403Z`
- Admin 构建产物：`apps/admin/dist/assets/index-CHuJx3t_.js`

### 线上复查

- `GET /api/admin/dashboard`：
  - `latestSystemHealth.status=error`
  - `deploymentRuntime.commit=15482b8ba5fe09fcb1601363f15b89f56acbc429`
  - `deploymentRuntime.deployedAt=20260613T020403Z`
  - `deploymentRuntime.markerPath=/opt/zhisuan-yizhan/user/.release-marker`
  - `upstreamBlocker.checkId=sub2`
  - `upstreamBlocker.repairAction=apply_openai_refresh_token_to_sub2_account`
  - `upstreamBlocker.sub2AccountId=2`
  - `upstreamBlocker.accountErrorCode=token_invalidated`
  - `upstreamBlocker.evidenceResponsesStatusCode=503`
  - `deliveryBlocker.checkId=resources`
  - `deliveryBlocker.resourceType=codex`
  - `deliveryBlocker.resourceScope=production`
- `GET /api/admin/system-health`：
  - 总体 `status=error`，29 项检查：25 ok、1 warning、3 error。
  - 错误检查仍为 `resourceCredentials`、`sub2`、`localProxySmoke`。
  - warning 检查仍为 `resources`。
  - `resourceCredentials.metrics.totalCredentials=0`
  - `resourceCredentials.metrics.activeOpenAiRefreshTokens=0`
  - `resourceCredentials.metrics.activeApplicableCredentials=0`
  - `localProxySmoke.metrics.responsesStatusCode=503`
  - `localProxySmoke.metrics.responsesErrorType=api_error`
  - `localProxySmoke.metrics.responsesErrorMessage=Service temporarily unavailable`
...
- 生产仍没有 active OpenAI refresh token resource credential。
- 生产仍没有 ready online production Codex shared resource。
- Sub2 默认 OpenAI 分组仍没有 active OpenAI account。
- 本次发布减少的是“已有资源凭据修复后忘记跑 smoke 验收”的风险；真实 `/v1/responses` 恢复仍依赖管理员提供有效 OpenAI refresh token 并完成 Sub2 账号与生产 Codex 共享资源修复。

### 清理

- 远端 `/tmp/sub2share-user-15482b8.tar` 已清理。
- 本地 `.deploy` 目录已删除。

## 2026-06-13 10:11 初始凭据应用 Sub2 账号前置校验发布与线上复查

### 本次发布

- 代码提交：`6dedb8460367c642a68a139a5d55256d9c10fdf6 fix: require sub2 account for initial credential apply`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - `POST /api/admin/resources` 在创建资源前统一校验初始凭据应用请求。
  - 当 `applyCredentialToSub2=true` 时，必须同时提供初始凭据、`sub2AccountId`、`openai_refresh_token` 类型和 `active` 凭据状态。
  - 当 `credentialRunSmokeTest=true` 但未启用 `applyCredentialToSub2` 时，继续返回 `initial_credential_apply_required`。
  - 新增错误码 `initial_credential_sub2_account_required`，避免管理员勾选“创建后应用到 Sub2”但未填 Sub2 账号时先创建半成品资源。

### 本地验证

- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/admin-capabilities.test.ts`：21/21 通过。
- `pnpm.cmd --filter @zyz/api run typecheck`：通过。
- `pnpm.cmd --filter @zyz/api test`：141/141 通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。

### 生产部署

- 部署归档：`/tmp/sub2share-user-6dedb84.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-6dedb84.tar --commit 6dedb8460367c642a68a139a5d55256d9c10fdf6`
- 服务器侧部署脚本通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  - Admin typecheck：通过。
  - API tests：141/141 通过。
  - Admin tests：16/16 通过。
  - workspace build：通过。
  - `/health`、`/ready`、Web、Admin：均 200。
- release marker：
  - `commit=6dedb8460367c642a68a139a5d55256d9c10fdf6`
  - `deployed_at=20260613T021155Z`

### 线上复查

- `GET /api/admin/dashboard`：
  - `latestSystemHealth.status=error`
  - `deploymentRuntime.commit=6dedb8460367c642a68a139a5d55256d9c10fdf6`
  - `deploymentRuntime.deployedAt=20260613T021155Z`
  - `deploymentRuntime.markerPath=/opt/zhisuan-yizhan/user/.release-marker`
  - `upstreamBlocker.checkId=sub2`
  - `upstreamBlocker.repairAction=apply_openai_refresh_token_to_sub2_account`
  - `upstreamBlocker.sub2AccountId=2`
...
- `GET /api/admin/system-health`：
  ...
  - `resourceCredentials.metrics.activeApplicableCredentials=0`
- 负向创建请求验证：
  - 请求：`POST /api/admin/resources`，`applyCredentialToSub2=true`、`credentialRunSmokeTest=true`、提供初始凭据但不提供 `sub2AccountId`。
  - 响应：HTTP 400。
  - 错误码：`initial_credential_sub2_account_required`。
  - 资源数量：请求前 `1`，请求后 `1`，未创建半成品 Codex 资源。
...
- 生产仍没有 ready online production Codex shared resource。
- Sub2 默认 OpenAI 分组仍没有 active OpenAI account。
- 本次发布防止错误修复动作留下半成品资源；真实 `/v1/responses` 恢复仍依赖管理员提供有效 OpenAI refresh token 并绑定 Sub2 账号。

### 清理

- 远端 `/tmp/sub2share-user-6dedb84.tar` 已清理。
- 本地 `.deploy` 目录已删除。

## 2026-06-13 10:31 Codex 交付代理 smoke 门禁发布与线上复查
### 本次发布

- 代码提交：
  - `791ef1356cd144a15a170dd429d295a3502a5600 feat: gate codex delivery on proxy smoke`
  - `5f29e11de7e31e18d4a15348ad5d08e3cf698e52 fix: prioritize resource blocker in codex delivery`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - Codex 交付门禁现在会读取最近本地 OpenAI/Codex 代理 smoke 审计证据。
  - 当最新 smoke 在 24 小时新鲜窗口内，并且不是前置失败导致的跳过，同时 `modelsOk=false`、`responsesOk=false` 或 `localProxyOk=false` 时，`requireReadySupplierResourceForDelivery()` 会以 `codex_proxy_smoke_failed_for_delivery` 阻断真实 Codex 交付。
  - 公开商品目录与 Admin 商品列表新增/保留 `codexProxySmokeDeliveryReady` 和 `codexProxySmokeDeliveryLatest`，可直接看到最新 `/v1/responses` 失败路径、状态码与错误类型。
  - 商品/价格激活门禁同步接入最新 proxy smoke 失败证据；仍可用既有 `allowUnavailableDelivery=true` 显式覆盖特殊维护场景。
  - 当 ready production Codex shared resource 缺失与 proxy smoke 失败同时存在时，`deliveryBlockedReason` 优先返回 `codex_resource_not_ready_for_delivery`，同时保留 proxy smoke 失败作为并行证据，方便管理员先补资源再修上游。

### 本地验证

- `pnpm.cmd --filter @zyz/api typecheck`：通过。
- `pnpm.cmd --filter @zyz/api test -- --runInBand`：143/143 通过。
- `pnpm.cmd --filter @zyz/admin typecheck`：通过。
- `pnpm.cmd --filter @zyz/admin test`：16/16 通过。
- 后续阻断原因顺序修正：
  - `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/resource-delivery-readiness.test.ts`：13/13 通过。
  - `pnpm.cmd --filter @zyz/api typecheck`：通过。
  - `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。

### 生产部署

- 最终部署归档：`/tmp/sub2share-user-5f29e11.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-5f29e11.tar --commit 5f29e11de7e31e18d4a15348ad5d08e3cf698e52`
- 服务器侧部署脚本通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  - Admin typecheck：通过。
  - API tests：144/144 通过。
  - Admin tests：16/16 通过。
  - workspace build：通过。
  - `/health`、`/ready`、Web、Admin：均 200。
- release marker：
  - `commit=5f29e11de7e31e18d4a15348ad5d08e3cf698e52`
  - `deployed_at=20260613T023140Z`

### 线上复查

- `GET /api/admin/dashboard`：
  - `latestSystemHealth.status=error`
  - `deploymentRuntime.commit=5f29e11de7e31e18d4a15348ad5d08e3cf698e52`
  - `deploymentRuntime.deployedAt=20260613T023140Z`
  - `upstreamBlocker.checkId=sub2`
  - `upstreamBlocker.sub2AccountId=2`
  - `upstreamBlocker.accountErrorCode=token_invalidated`
  - `upstreamBlocker.evidenceResponsesStatusCode=503`
...
- `GET /api/admin/system-health`：
  - 总体 `status=error`，29 项检查：25 ok、1 warning、3 error。
  - `deploymentRuntime.status=ok`，当前 commit 为 `5f29e11de7e31e18d4a15348ad5d08e3cf698e52`。
  - `resources.status=warning`，`No online production Codex shared resource`。
  - `resources.metrics.readyOnlineCodexResources=0`。
  - `resourceCredentials.status=error`，`activeApplicableCredentials=0`。
  - `sub2.status=error`，`activeOpenAiAccounts=0`。
  - `localProxySmoke.status=error`，最新 evidence 仍为 `/v1/responses`：`HTTP 503 / api_error / Service temporarily unavailable`，且 `stale=false`。
- `GET /api/admin/products?page=1&pageSize=10&resourceType=codex`：
  - `total=2`。
  - 两个 Codex 商品当前均 `deliveryReady=false`。
  - `deliveryBlockedReason=codex_resource_not_ready_for_delivery`。
  - `readyDeliveryResources=0`。
  - `codexProxySmokeDeliveryReady=false`。
  - `codexProxySmokeDeliveryLatest.proxyRequestPath=/v1/responses`。
  - `codexProxySmokeDeliveryLatest.responsesStatusCode=503`。
  - `codexProxySmokeDeliveryLatest.responsesErrorType=api_error`。
- `GET /api/products`：
  - HTTP 200。
  - 当前公开 active 商品中 Codex 数量为 `0`。
...
- 生产仍没有 ready online production Codex shared resource。
- Sub2 默认 OpenAI 分组仍有 2 个 OpenAI account，但 active account 为 0。
- 最新本地代理 smoke 仍证明 `/v1/models=200`、`/v1/responses=503`。
- 本次发布新增的是“已知新鲜代理失败证据不得继续放行 Codex 交付”的保护；真实 `/v1/responses` 恢复仍依赖管理员提供有效 OpenAI refresh token、绑定 Sub2 账号、创建/修复生产 Codex 共享资源，并重新跑端到端 smoke 通过。

### 清理

- 远端 `/tmp/sub2share-user-791ef13.tar`、`/tmp/sub2share-user-5f29e11.tar` 已清理。
- 本地 `.deploy` 目录已删除。

## 2026-06-13 10:42 商品目录与售出交付 smoke 门禁巡检发布与线上复查
### 本次发布

- 代码提交：`a2d769e7fee6d51b3015c062cb7daa02b285356e feat: surface proxy smoke in delivery health`
- GitHub：`main` 已推送到 `git@github.com:carzygod/sub2share.git`
- 本次能力：
  - `productCatalog` 巡检现在复用 Codex 交付快照；当 active 且可购买的 Codex 商品存在 ready resource，但最新本地 proxy smoke 仍失败时，会产生 `active_codex_product_proxy_smoke_failed` warning。
  - `salesDelivery` 巡检现在复用同一 Codex 交付快照；当 active Codex 租赁受最新 proxy smoke 失败影响时，会产生 `active_codex_rental_proxy_smoke_failed` error。
  - 商品目录与售出交付 issue 会保留 `/v1/responses` smoke 阶段证据，包括模型、models/responses 状态码、错误类型、代理请求路径、请求 ID、上游请求 ID、新鲜度与 stale 时间。
  - Dashboard 预览继续保留 product catalog 的 smoke 证据字段，便于从首页阻断摘要进入共享资源或 Sub2 修复链路。

### 本地验证

- `pnpm.cmd --filter @zyz/api typecheck`：通过。
- `pnpm.cmd --filter @zyz/api exec node --import tsx --test tests/resource-delivery-readiness.test.ts`：14/14 通过。
- `pnpm.cmd --filter @zyz/api test -- --runInBand`：145/145 通过。
- `pnpm.cmd --filter @zyz/admin typecheck`：通过。
- `pnpm.cmd --filter @zyz/admin test`：16/16 通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。
...
- 部署归档：`/tmp/sub2share-user-a2d769e.tar`
- 部署脚本：`bash /opt/zhisuan-yizhan/user/scripts/deploy-production.sh --archive /tmp/sub2share-user-a2d769e.tar --commit a2d769e7fee6d51b3015c062cb7daa02b285356e`
- 服务器侧部署脚本通过：
  - Prisma generate：通过。
  - Prisma migrate deploy：无待应用迁移。
  - API typecheck：通过。
  ...
  - `/health`、`/ready`、Web、Admin：均 200。
- release marker：
  - `commit=a2d769e7fee6d51b3015c062cb7daa02b285356e`
  - `deployed_at=20260613T024214Z`

### 线上复查

- `GET /api/admin/dashboard`：
  - `latestSystemHealth.status=error`
  - `deploymentRuntime.commit=a2d769e7fee6d51b3015c062cb7daa02b285356e`
  - `deploymentRuntime.deployedAt=20260613T024214Z`
  - `upstreamBlocker.checkId=sub2`
  - `upstreamBlocker.sub2AccountId=2`
  - `upstreamBlocker.accountErrorCode=token_invalidated`
  - `upstreamBlocker.evidenceResponsesStatusCode=503`
  - `deliveryBlocker.checkId=resources`
  - `deliveryBlocker.primaryIssue.type=codex_online_resource_missing`
  - `deliveryBlocker.resourceType=codex`
  - `deliveryBlocker.resourceScope=production`
- `GET /api/admin/system-health`：
  - 总体 `status=error`，29 项检查：25 ok、1 warning、3 error。
  - `deploymentRuntime.status=ok`，当前 commit 为 `a2d769e7fee6d51b3015c062cb7daa02b285356e`。
  - `resources.status=warning`，`No online production Codex shared resource`。
  - `resourceCredentials.status=error`，`activeApplicableCredentials=0`。
  - `sub2.status=error`，`activeOpenAiAccounts=0`。
  - `localProxySmoke.status=error`，最新 evidence 仍为 `/v1/responses`：`HTTP 503 / api_error / Service temporarily unavailable`，且 `stale=false`。
  - `productCatalog.status=ok`，因为当前没有 active 公开 Codex 商品；metrics 已包含 `codexProductsBlockedByProxySmoke=0`。
  - `salesDelivery.status=ok`，因为当前没有 paid/provisioning/active 的待交付订单；metrics 已包含 `activeCodexRentalsBlockedByProxySmoke=0`。
...
- Sub2 默认 OpenAI 分组仍有 2 个 OpenAI account，但 active account 为 0。
- 最新本地代理 smoke 仍证明 `/v1/models=200`、`/v1/responses=503`。
- 本次发布让健康巡检在未来商品重新上架或已有租赁恢复时能继续反映最新 proxy smoke 失败，不会把“资源字段齐全但 `/v1/responses` 失败”的状态误报为可售或已交付正常。

### 清理

- 远端 `/tmp/sub2share-user-a2d769e.tar` 已清理。
- 本地 `.deploy` 目录已删除。
