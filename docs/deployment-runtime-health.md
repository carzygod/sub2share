...
## 2026-06-13 修复：Dashboard 使用实时发布态覆盖旧健康快照

线上复查发现，`GET /api/admin/system-health` 已经能实时显示当前 release，但 Dashboard 首页的 `latestSystemHealth` 默认读取数据库中的最新健康快照。部署后如果快照还没刷新，首页“当前发布”可能继续显示上一轮 commit。

已修复为：

- Dashboard 路由调用 `dashboardLatestSystemHealthPreview()` 时传入实时 `deploymentRuntimeHealthCheck()`。
- `deploymentRuntime` 摘要优先使用实时检查结果，只有缺失时才回退到快照中的旧值。
- `deploymentRuntime.metrics` 补齐 `cwd`、`releaseRootName`、`markerPath`，使 Dashboard 与完整系统健康都能显示 release marker 文件路径。

2026-06-13 部署 `5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6` 后，线上 Dashboard 显示：

- `deploymentRuntime.commit=5419ec56b9199e8b3956ee4ed2d5e15e3edbd1a6`
- `deploymentRuntime.deployedAt=20260613T015001Z`
- `deploymentRuntime.releaseRoot=/opt/zhisuan-yizhan/user`
...
生产系统总体仍为 `status=error`，原因是 Sub2/OpenAI 上游账号无 active account 且 `/v1/responses` 返回 503；这不是当前发布态检查失败。
